// Mermaid-based derivation graph rendering for the TQM derivation map.
...
window.tqmDerivationGraph = {
  ...
  render: async function (elementId, definition, dotnetRef) {
    ...
    const uniqueId = 'tqm-graph-' + Date.now();
    try {
  ...
  highlight: async function (elementId, nodeIds) {
    ...
    container.querySelectorAll('g.node').forEach((g) => {
      ...
      if (match && nodeIds.includes(match[1])) {
        g.classList.add('tqm-node-highlight');
      } else {
        g.classList.remove('tqm-node-highlight');
      }
    });
  },
  ...
  zoomBy: function (elementId, factor) {
    ...
    const svg = container.querySelector('svg');
    if (!svg) return;
    const current = svg.getAttribute('data-tqm-zoom');
    const base = current ? parseFloat(current) : 1;
    const next = Math.min(4, Math.max(0.15, base * factor));
    svg.setAttribute('data-tqm-zoom', String(next));
  ...
  resetZoom: function (elementId) {
    ...
    const svg = container.querySelector('svg');
    if (!svg) return;
    svg.removeAttribute('data-tqm-zoom');
