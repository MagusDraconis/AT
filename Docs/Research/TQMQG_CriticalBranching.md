# TQM-QG Phase 7 — Critical Branching

**Program:** TQM-QG (Unification)
**Phase:** 7 — why must actualization be critical (μ=1)?
**Status:** COMPLETED — 3/3 xUnit tests pass (24/24 TQM-QG)
...
### (a) μ=1 is the unique marginal point (TQMQG70)
...
### (b) Three criteria coincide at μ=1 (TQMQG71)
...
- Therefore criticality is **DERIVED**: uniquely selected by stability (non-extinction, non-runaway) +
  scale-freeness (renormalization invariance, TQM-F1) + maximum entropy (G4-RHO1).
- The single conditioning input is **scale-freeness** (renormalization invariance), which TQM-F1 reduced to
...
| Test | Verdict |
|---|---|
| TQMQG70 `TQMQG70_ExtinctionVsRunaway` | PASS (μ=1 unique marginal point) |
| TQMQG71 `TQMQG71_ThreeCriteriaCoincide` | PASS (scale-free = marginal = max-entropy at μ=1) |
| TQMQG72 `TQMQG72_Classification` | PASS (DERIVED) |

Code: `TQM.Core/ResearchXH/QEventBranching.cs` (added `ExtinctionProbability`, `TotalExpectedPopulation`);
tests `TQM.Tests/ResearchXH/TQMQG_Phase7_CriticalBranchingTests.cs`.
