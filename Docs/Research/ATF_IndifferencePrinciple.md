# AT-F Phase 1 — The Indifference Principle
...
### (a) Primitives are scale-covariant; the power law is the unique scale-covariant form (ATF10)
...
### (b) Renormalization invariance: power laws are the fixed points (ATF11)
...
the unique renormalization-invariant abundance law**, and the AT primitives (partial order + counting
...
| Test | Verdict |
|---|---|
| ATF10 `ATF10_ScaleCovariance` | PASS (power law = unique scale-covariant form) |
| ATF11 `ATF11_RenormalizationInvariance` | PASS (power law = RG fixed point) |
| ATF12 `ATF12_Classification` | PASS (PREFERRED, DERIVED-conditional) |

Code: `AT.Core/ResearchXH/RhoDynamics.cs` (added `CoarseGrain`, `GaussianAbundance`, `SuccessiveRatios`);
tests `AT.Tests/ResearchXH/ATF_Phase1_IndifferencePrincipleTests.cs`.
