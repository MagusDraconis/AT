...
| **Higgs** | calibration | QG169/176: M_H = σ_occ·(span/2) = 125.25 GeV, 0.003% match; blind reconstruction (hidden target, natural core rank 4) is the strongest anti-fit defense; σ_occ undefined in the monograph (reproducibility gap); GeV unit from the calibrated weak scale v (Validation Referee: "fit + calibration" too pessimistic) | "M_H = σ_occ·(span/2) = 125.25 GeV, matching observation within 0.003%" (Ch11 eq:higgs) | "M_H is a calibrated reconstruction via the anchor v (blind reconstruction, natural-core status); the factor σ_occ must be defined; the 0.003% match is a calibration, not a derived value" |
