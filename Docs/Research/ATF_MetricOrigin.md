...
### (a) The count and the metric volume are both measures (ATF30)
...
### (b) √(−g)=ρ is the unique consistent volume element (ATF31)
...
This **upgrades metric origin from PREFERRED (AT-F0) to DERIVED-in-form (unique)**, leaving only the
...
| Test | Verdict |
|---|---|
| ATF30 `ATF30_VolumePreservation` | PASS (count & volume are measures; equal under √(−g)=ρ) |
| ATF31 `ATF31_Uniqueness` | PASS (√(−g)=ρ unique; alternatives fail) |
| ATF32 `ATF32_Classification` | PASS (DERIVED form; PREFERRED identification) |

Code: `AT.Core/ResearchXH/MetricOrigin.cs`;
tests `AT.Tests/ResearchXH/ATF_Phase3_MetricOriginTests.cs`.
