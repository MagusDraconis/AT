# AT-QG Phase 3 — Dimension Selection
...
> conformal-complete dimension is d=2 (forbidden). See `ATQG_InformationDimension.md`.

**Program:** AT-QG (Unification)
**Phase:** 3 — search for a preferred spacetime dimension (is d=4 special?).
**Status:** COMPLETED — 3/3 xUnit tests pass (12/12 AT-QG)
...
### (a) All native scores are monotonic (ATQG30)
...
### (b) d=4 is the minimal propagating-gravity dimension (ATQG31)
...
- **PREFERRED (conditional):** d=4 is the lowest dimension with propagating graviton modes (2 polarizations),
  ...
  propagates (a GR input, not a native AT consequence: AT gravity is conformally-flat / scalar-only).
...
input — resolved only by adding a propagation requirement, which AT's scalar gravity does not itself supply.
...
| Test | Verdict |
|---|---|
| ATQG30 `ATQG30_MonotonicScores` | PASS (all scores monotonic, no d=4 extremum) |
| ATQG31 `ATQG31_MinimalPropagatingGravity` | PASS (d=4 minimal propagating dimension) |
| ATQG32 `ATQG32_Classification` | PASS (NOT SPECIAL native; PREFERRED conditional) |

Code: `AT.Core/ResearchXH/DimensionAnalysis.cs` (added `EinsteinRichness`, `FrozenFraction`,
`ComplexityPerDof`); tests `AT.Tests/ResearchXH/ATQG_Phase3_DimensionSelectionTests.cs`.
