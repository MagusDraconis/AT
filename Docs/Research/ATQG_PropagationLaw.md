# AT-QG Phase 28 — Derive the Propagation Law

**Program:** AT-QG (Unification)
**Phase:** 28 — which light-propagation rule follows from actualization dynamics?
**Status:** COMPLETED — 3/3 xUnit tests pass (87/87 AT-QG)
...
### (c) Classification (ATQG282)
- **NULL GEODESICS: DERIVED** (the native propagation law).
- **TRM EFFECTIVE MEDIUM: IMPORTED** (the temporal-only refractive index, not in AT's primitives).

---

## 4. Conclusion

Null geodesics are the **native (DERIVED)** propagation law of AT: the causal order fixes the light cone, and the
...
This closes the propagation-law question decisively: **no refractive medium emerges from actualization**; AT's
...
| Test | Verdict |
|---|---|
| ATQG280 `ATQG280_IndexAndConformalInvariance` | PASS (n=1 vs ρ^(1/d)) |
| ATQG281 `ATQG281_MechanismCensus` | PASS (4 native / 1 imported) |
| ATQG282 `ATQG282_Classification` | PASS (null geodesics DERIVED, TRM IMPORTED) |

Code: `AT.Core/ResearchXH/PropagationLaw.cs`;
tests `AT.Tests/ResearchXH/ATQG_Phase28_PropagationLawTests.cs`.
