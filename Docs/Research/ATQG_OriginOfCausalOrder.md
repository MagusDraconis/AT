# AT-QG Phase 11 — Origin of Causal Order

**Program:** AT-QG (Unification)
**Phase:** 11 — can causal order emerge from a more primitive actualization process?
**Status:** COMPLETED — 3/3 xUnit tests pass (36/36 AT-QG)
...
### (a) The ancestor relation is a partial order (ATQG110)
...
### (b) Temporal ordering + branching consistency (ATQG111)
...
- The **full causal order is DERIVED**: it is the transitive closure of the parent→child generation relation,
  which is automatically a strict partial order (ATQG110). Temporal ordering is the generation linear
  extension (ATQG111).
- The remaining **REAL-UNDERIVED primitive is the generation relation itself** — "an event generates
  descendants" — i.e., the actualization dynamics (critical branching, AT-QG1/QG7). This is more minimal than
...
| Test | Verdict |
|---|---|
| ATQG110 `ATQG110_AncestorRelationIsPartialOrder` | PASS (ancestor relation = partial order) |
| ATQG111 `ATQG111_TemporalOrderingAndConsistency` | PASS (linear extension + consistency) |
| ATQG112 `ATQG112_Classification` | PASS (DERIVED) |

Code: `AT.Core/ResearchXH/CausalOrder.cs`;
tests `AT.Tests/ResearchXH/ATQG_Phase11_OriginOfCausalOrderTests.cs`.
