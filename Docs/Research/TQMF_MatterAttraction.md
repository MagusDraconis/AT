# TQM-F Phase 2 — Matter Attraction

**Program:** TQM-F (Foundation)
**Phase:** 2 — can attraction itself be derived?
**Status:** COMPLETED — 3/3 xUnit tests pass (6/6 TQM-F)
...
### (a) Geodesic convergence derives the sign of gravity (TQMF20)
...
### (b) The deficit branch is the stable (clumping) one (TQMF21)
...
| Indifference (scale-freeness) | PREFERRED (renormalization invariance, TQM-F1) |
| **Matter attraction** | **DERIVED (geodesic convergence + stability, TQM-F2)** |
...
| Test | Verdict |
|---|---|
| TQMF20 `TQMF20_GeodesicConvergence` | PASS (R₀₀>0 at deficit, <0 at peak) |
| TQMF21 `TQMF21_StabilityDeficitAccumulation` | PASS (deficit converges, peak diverges) |
| TQMF22 `TQMF22_Classification` | PASS (DERIVED) |

Code: `TQM.Core/ResearchXH/PhysicalObservables.cs` (added `TimelikeConvergence`,
`AccelerationDivergence`); tests `TQM.Tests/ResearchXH/TQMF_Phase2_MatterAttractionTests.cs`.
