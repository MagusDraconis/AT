...
**Phase:** 1 — compare a curvature-sourced (TQM) vs density-sourced (GR) theory
...
| **TQM** | ΔΦ + ((d−2)/2)\|∇Φ\|² = −ρ^(2/d)R/(2(d−1)) (log-density **curvature**) |
...
GR has a linear field a = −ρ₀x in a uniform density; TQM has **zero** field (a = −∇lnρ = 0).
...
GR has the Newtonian **long-range** (1/r²) field outside a mass shell; TQM's field is **localized**
...
The TQM source is the **sign-changing** log-density curvature (positive at density minima, negative
...
| profile | GR vs TQM | classification |
|---|---|---|
...
shell-exterior regions**: TQM's field is proportional to ∇ρ (the actualization-density gradient), so it
vanishes wherever the density is uniform — including *outside* a mass shell, where Newtonian GR has its
signature 1/r² field. Equivalently, TQM's gravitational source is the sign-changing log-density curvature
(lnρ)″, not the positive-definite density value.

This is a concrete, in-principle-testable discrimination: **TQM predicts no Newtonian field in regions of
...
| Test | Verdict |
|---|---|
| G4-O10 `G4_O10_UniformDensityStrongDifference` | PASS (a_GR = −ρ₀x ≠ 0, a_TQM = 0) |
...
Code: `TQM.Core/ResearchXH/PhysicalObservables.cs` (added profiles + `GrSource/TqmSource/GrAcceleration/TqmAcceleration`);
tests `TQM.Tests/ResearchXH/G4O_Phase1_DiscriminatingPredictionTests.cs`.
