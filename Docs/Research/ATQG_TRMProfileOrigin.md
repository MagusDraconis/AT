# AT-QG Phase 36 — Derive the TRM Regular-Core Profile

**Program:** AT-QG (Unification)
**Phase:** 36 — can M_eff(r)=M(1−e^(−r³/r_c³)) be derived from a ψ-dynamics?
**Status:** COMPLETED — 3/3 xUnit tests pass (111/111 AT-QG)
...
## 2. The Poisson-saturation derivation (ATQG360)
...
- CAVEAT: r_c is not itself derivable — it is set by the critical density ρ_c, which is **supplied** (AT has
  bounds but no native cutoff value, QG14). The Poisson-independence assumption is the max-entropy counting model
  (AT-F Phase 1). So the profile is DERIVED, with r_c as the one free scale.
...
The only residual input is the critical density ρ_c (equivalently the core scale r_c), consistent with AT's
...
| Test | Verdict |
|---|---|
| ATQG360 `ATQG360_PoissonSaturationReproducesProfile` | PASS (exact, exponent 3) |
| ATQG361 `ATQG361_MechanismCensus` | PASS (only saturation works) |
| ATQG362 `ATQG362_Classification` | PASS (DERIVED) |

Code: `AT.Core/ResearchXH/TRMProfileOrigin.cs`;
tests `AT.Tests/ResearchXH/ATQG_Phase36_TRMProfileOriginTests.cs`.
