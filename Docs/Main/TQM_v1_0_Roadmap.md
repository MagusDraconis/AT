# TQM v1.0 Roadmap Audit

**Goal:** determine the **shortest path** from TQM v0.9 to v1.0.
**Inputs:** `TQM_Master_Reference.md`, `TQM_Encyclopedia.md`, `TQM_Completeness_Report.md`,
`Coverage_Report.md`, `TQM_VersionReadiness.md`.
...
| 4 | **Unified Action** | Low — TRM capstone roadmap, not a TQM result | Trivial (formal demotion) | 3 (needs $T$, $\vec A_T$, $\Theta$ first) | **Yes** — must be formally demoted to "roadmap" (removes the last OPEN chapter) |
...
2. **Demote Unified Action** — mark $S_{\rm eff}[T,\vec A_T,\Theta]$ as a **TRM roadmap,
   not a TQM result** (depends on the vector/theta sectors). This removes the last OPEN
   chapter (Part VII).
