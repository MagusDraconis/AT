...
**Goal:** turn `AT_v1_0_Paper.md` into a publication-ready paper by adding six sections
and fixing three wording issues identified by the hostile-review response.
**Inputs:** `AT_v1_0_Paper.md`, `HostileReview.txt`, `HostileReviewResponse.md`.
...
- **Dynamical layer** (the quantum postulates, AT-155): (1) $Q$ exists — topological
...
`AT_Master_Reference.md` §2, §17 (A-03).
...
- The graph Laplacian $L_Q = D - A$ (real symmetric, positive semi-definite, zero row
  sums); the tight-binding identity $H = t\,L_Q$ (AT-142, a mathematical identity, not
  an analogy).
...
- Observables from the spectrum: $m_{\rm eff}=1/\lambda_1$, $E=\mathrm{tr}(L)$,
  ...
  (AT-145).
- Ontology→physics bridge: $L_Q$ eigenvector basis = Hilbert space (AT-149); causal set
...
**Source:** `AT.Core/ResearchXE/ComplexityOptimumAnalyzer.cs`, X029/XE009,
`AT_Master_Reference.md` T-02.
...
- AT derives the **form** of structure and **classifies** the content; it does **not**
