...
ResearchY-M_015  ↔  Y_M_015_Tests.cs     (AT.Tests/ResearchY/M_Measurement/)
ResearchY-M_016  ↔  Y_M_016_Tests.cs     (AT.Tests/ResearchY/M_Measurement/)
...
ResearchY-G_007   ↔  Y_G_007_Tests.cs    (AT.Tests/ResearchY/G_GravitySource/)
...
ResearchY-G_012   ↔  Y_G_012_Tests.cs    (AT.Tests/ResearchY/G_GravitySource/)
...
ResearchY-G_016b  ↔  Y_G_016b_Tests.cs   (AT.Tests/ResearchY/G_GravitySource/)
...
ResearchY-G_030   ↔  Y_G_030_Tests.cs    (AT.Tests/ResearchY/G_GravitySource/)
| `Y_G_031_Tests` | 7 | PASS | Gravity Source | `AT.Tests/ResearchY/G_GravitySource/Y_G_031_Tests.cs` |
| `Y_G_032_Tests` | 8 | PASS | Gravity Source | `AT.Tests/ResearchY/G_GravitySource/Y_G_032_Tests.cs` |
| `Y_G_033_Tests` | 5 | PASS | Gravity Source | `AT.Tests/ResearchY/G_GravitySource/Y_G_033_Tests.cs` |
| `Y_G_034_Tests` | 6 | PASS | Gravity Source | `AT.Tests/ResearchY/G_GravitySource/Y_G_034_Tests.cs` |
| `Y_G_035_Tests` | 6 | PASS | Gravity Source | `AT.Tests/ResearchY/G_GravitySource/Y_G_035_Tests.cs` |
ResearchY-G_029   ↔  Y_G_029_Tests.cs    (AT.Tests/ResearchY/G_GravitySource/)
ResearchY-G_028   ↔  Y_G_028_Tests.cs    (AT.Tests/ResearchY/G_GravitySource/)
ResearchY-G_027   ↔  Y_G_027_Tests.cs    (AT.Tests/ResearchY/G_GravitySource/)
ResearchY-G_026   ↔  Y_G_026_Tests.cs    (AT.Tests/ResearchY/G_GravitySource/)
...
| ResearchY-M_015 (Score-Function Robustness Audit) | `AT.Tests/ResearchY/M_Measurement/Y_M_015_Tests.cs` | ACTIVE | 2026-09-08 | ✅ 32/32 PASSED — `Results/Y_M_015_Result.md` |
| ResearchY-M_016 (Criterion-Independence Audit) | `AT.Tests/ResearchY/M_Measurement/Y_M_016_Tests.cs` | ACTIVE | 2026-09-08 | ✅ 10/10 PASSED — `Results/Y_M_016_Result.md` |
...
| ResearchY-G_007 (Suppression Origin Audit) | `AT.Tests/ResearchY/G_GravitySource/Y_G_007_Tests.cs` | ACTIVE | 2026-09-12 | ✅ 9/9 PASSED — `Results/Y_G_007_Result.md` |
...
| ResearchY-G_012 (Local Rho Actuator Audit) | `AT.Tests/ResearchY/G_GravitySource/Y_G_012_Tests.cs` | ACTIVE | 2026-09-12 | ✅ 10/10 PASSED — `Results/Y_G_012_Result.md` |
...
| ResearchY-G_016b (Mass Independence Audit) | `AT.Tests/ResearchY/G_GravitySource/Y_G_016b_Tests.cs` | ACTIVE | 2026-09-12 | ✅ 7/7 PASSED — `Results/Y_G_016b_Result.md` |
...
| ResearchY-G_030 (No-Go Audit) | `AT.Tests/ResearchY/G_GravitySource/Y_G_030_Tests.cs` | ACTIVE | 2026-09-13 | ✅ 8/8 PASSED — `Results/Y_G_030_Result.md` (**NO-GO PROVED**: with A = sigma fixed, six of the eight constraints are inert, and the live pair {Cassini/deflection} vs {no-new-primitive} cannot both be satisfied. Two EXACT equivalences: **gamma = -1 <=> B = sigma** (the counting measure, AT's only derivation, excluded at **8.6957e4 sigma** with deflection 0) and **gamma = +1 <=> g_rr = 2 - rho^(2/d)** (underivable). The admitted band is ~5.5e-10 wide and sits at -sigma while the derived B = sigma \u2014 **opposite sides of zero**. The counting-measure departure grows 4.2e-9 (Earth) -> 2.44 (J0740+6620), reaching O(1) where it is tested hardest. A surviving spatial sector is **REFUTED as a derivation** and **AFFIRMED as a postulate**. Group G total 245/245.) |
| `Y_G_031_Tests` | 7 | PASS | Gravity Source | `ResearchY/G_GravitySource/Y_G_031_Tests.cs` |
| `Y_G_032_Tests` | 8 | PASS | Gravity Source | `ResearchY/G_GravitySource/Y_G_032_Tests.cs` |
| `Y_G_033_Tests` | 5 | PASS | Gravity Source | `ResearchY/G_GravitySource/Y_G_033_Tests.cs` |
| `Y_G_034_Tests` | 6 | PASS | Gravity Source | `ResearchY/G_GravitySource/Y_G_034_Tests.cs` |
| `Y_G_035_Tests` | 6 | PASS | Gravity Source | `ResearchY/G_GravitySource/Y_G_035_Tests.cs` |
| ResearchY-G_029 (Spatial Sector Closure Audit) | `AT.Tests/ResearchY/G_GravitySource/Y_G_029_Tests.cs` | ACTIVE | 2026-09-13 | ✅ 7/7 PASSED — `Results/Y_G_029_Result.md` (**EXACTLY ONE B SURVIVES: `g_rr = 2 - rho^(2/d) = 2 - |g00|`** — the unique member with gamma = +1 at ALL orders (e^(2B) = 2 - e^(2A)), the reflection of the temporal metric about 1; non-degenerate everywhere physical with g_rr in (1,2). B = sigma (the ONLY derivable member) is REFUTED at Cassini **8.6957e4 sigma** (gamma = -1, deflection 0, Shapiro 0), so the sector **CLOSES ON A POSTULATE** at a volume cost of 3.437585 — reproducing G_023's recorded value exactly. The requirements pin gamma, not B: Cassini bounds the quadratic coefficient only by |c2+2| <= 22.5. Group G total 237/237.) |
| ResearchY-G_028 (Clock Sector Closure Audit) | `AT.Tests/ResearchY/G_GravitySource/Y_G_028_Tests.cs` | ACTIVE | 2026-09-13 | ✅ 7/7 PASSED — `Results/Y_G_028_Result.md` (**CLOCK CLOSED on `dTau/dt = rho^(1/d)`.** The potential IS the time exponent (`A = sigma + psi`), so the source law holds IFF `grad psi = 0` — **psi CANNOT be nonzero without changing the source law**. `gamma = +1` fixes `psi = -4 sigma` uniquely, forcing `A = +3x > 0`: a blueshift at every body, z < 0 at compactness, **2000 sigma** at Earth, J0740+6620 z = -0.5233658 vs +0.2801817. rho-only satisfies ALL FIVE requirements; the spatial route is the only gamma = +1 that keeps the clock, at a 3.437585 volume cost. **CORRECTS G_024/QG212**: the GPS bound is 1.391e-12, **1.44e9x** tighter than the quoted 2e-3, and QG212's ‘physical sector’ is repulsive. Group G total 230/230.) |
| ResearchY-G_027 (Literal Verdict Audit) | `AT.Tests/ResearchY/G_GravitySource/Y_G_027_Tests.cs` | ACTIVE | 2026-09-13 | ✅ 5/5 PASSED — `Results/Y_G_027_Result.md` (ENFORCES the rule mechanically: no result path may let a classification depend on a literal. The scanner re-reads the AT.Core source at test time, so a NEW literal->verdict path FAILS the build until it is COMPUTED or adjudicated. 7 paths found, ALL BOUNDARY authored judgements/input taxonomies; exactly one REFUTED (`GdaggerOriginAnalyzer`: flags, ratio and score all literals, `Score` feeding the ranking, and a typed `RatioToA0 = 1.0` with a NaN numerator) and it is FIXED by deriving all four from the predicted g†, reproducing the typed values exactly. The registry structurally forbids a REFUTED entry. Group G total 223/223.) |
| ResearchY-G_026 (Authored-Verdict Audit) | `AT.Tests/ResearchY/G_GravitySource/Y_G_026_Tests.cs` | ACTIVE | 2026-09-12 | ✅ 8/8 PASSED — `Results/Y_G_026_Result.md` (GENERALISES the G_025 defect class and finds it SYSTEMIC: four QG audits with identical method names differing only in literals, sub-scores that never read their criteria, four suites asserting contradictory verdicts and all passing; the Born-rule "α = 2 UNIQUELY selected" classification was DEAD CODE; 7 RAR rows typed "DERIVED ✓"; G duplicated as unchecked literals; a parameter that silently answered for the defaults. All five now DERIVED with cited bases and Computed/Analytic disclosure. Verdicts unchanged; ψ corrected to canonical. Group G total 218/218.) |
