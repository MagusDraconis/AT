# AT Failure Analysis Framework

## Failure Modes, Survival Structure, and Experimental Kill-Shots

*AT Collaboration — Hostile Audit Document, August 2026*
...
A scientific theory that cannot define the conditions of its own falsification is not a scientific theory. The AT research program has produced a framework of interlocking claims. Many of these claims are testable. Some will be wrong.
...
This document does not defend AT. Every claim is treated as possibly false. Every prediction is assumed to be at risk. The goal is not to preserve the framework but to prepare for its failure.
...
| If this is falsified... | P1 (w) | P2 (a₀) | P3 (DM) | P4 (singular) | P5 (log-n) | P6 (ν) | P7 (M²) | AT core |
|------------------------|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| w(z) = −1.000 | ✗ | ✗ | ✓ | ✓ | ✓* | ✓ | ✓ | ✓ |
| a₀ ≠ f(H₀) | ✓ | ✗ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| WIMP detected | ✓ | ✓ | ✗ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
3. **Discovery of a simpler competing framework.** If a framework with fewer primitives or simpler axioms makes the same or better predictions, AT is superseded by Occam's razor.
...
**What happens.** Euclid and Roman together measure w(z) = −1.000 with uncertainty σ ≈ 0.01. The deviation predicted by AT (~1–4%) is not observed at >3σ.
...
- Distinguishing AT's specific functional form from other dark energy models.
...
**Impact on framework.** Minimal. The prediction was not unique to AT. It adds modest support but does not distinguish the framework from competitors.
...
### 7.1 Case A: Continued Null Results (Most Likely Under AT)
...
**What survives.** Tier 2 defect taxonomy survives — neutral defects exist but are not the dominant DM component. DM would require a separate explanation, possibly outside the current AT framework entirely.

**Required revision.** The defect ontology must be decoupled from the DM problem. A new DM candidate must be identified within AT, or the framework must accept that DM is beyond its current scope.

### 7.3 Case C: Axion Detected

**Consequence.** Similar to Case B. The specific defect DM candidate is wrong. However, AT's "hidden moduli excitation" candidate is axion-like, so an axion detection could be interpreted as support for the moduli DM model — depending on the axion's properties.
...
**Consequence.** The framework's DM prediction is wrong, and the true DM sector is beyond any current framework. This is the hardest scenario — no framework anticipated it. All DM predictions across all theories are simultaneously falsified. AT would be in the same position as all competitors.
...
"The causal set-to-GR bridge is not AT's achievement — it is causal set theory's achievement. AT provides a physical interpretation for causal set elements (they are Q-events), but the hard mathematical work — the BDG action, the continuum limit, the emergence of the Einstein equations — was done by Sorkin, Benincasa, Dowker, Glaser, and others. The framework claims to 'derive GR' but what it actually does is assert that Q-events form a causal set and then appeal to well-known results in causal set theory. This is not a derivation — it is an appropriation.
...
| WIMP detected at LHC/direct | >5σ | **Model Update** | Abandon neutral-vortex DM candidate. Investigate whether the detected particle can be identified with a different AT defect type. If not, accept external DM. |
| Axion detected | >5σ | **Model Update** | Compare to moduli-excitation prediction. If consistent: confirm AT-axion. If inconsistent: abandon moduli DM candidate. |
