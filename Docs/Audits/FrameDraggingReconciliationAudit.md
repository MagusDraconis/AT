# Frame Dragging Reconciliation Audit
...
## 1. Extracted vector equations

The vector sector is:

$$\vec A_T,\qquad \vec B_T = \nabla\times\vec A_T,$$

with a "weak-field frame-dragging candidate interpretation" and an effective coupling
$k_T$ whose normalization is claimed "derived non-fitted" (FD03). Guard blocks FD01–FD20
cover rotational source generation, spin-sign reversal, weak-field scaling, null/spin-zero
fallback, ablation/holdout/unit-scaling, and no-refit systematic-bias audits.
**Claim boundary (verbatim):** "tested-effective weak-field frame-dragging candidate
sector"; "**Not claimed:** … full quantitative GR-equivalence."
...
GR's gravitomagnetism is exactly the same structure: a gravitomagnetic potential
$\vec A_g$ and field $\vec B_g=\nabla\times\vec A_g$, producing Lense–Thirring
frame-dragging. → $\vec A_T,\ \vec B_T=\nabla\times\vec A_T$ is **structurally equivalent**
to GR's gravitomagnetism; only the label ($T$ vs $g$) and the coupling ($k_T$ vs the fixed
GR normalization) differ.
...
## 4. Observables, free parameters, benchmarks

| Item | Content |
|---|---|
| **Observable prediction** | Lense–Thirring frame-dragging precession (gyroscope precession) |
| **Free parameter** | $k_T$ (claimed derived; GR's gravitomagnetic coupling is fixed, no free parameter) |
| **Benchmark tests** | Gravity Probe B (~19% accuracy), LAGEOS/LARES, binary-pulsar spin–orbit |
