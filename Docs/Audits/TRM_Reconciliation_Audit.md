...
current TQM framework (`TQM_Master_Reference.md`, `TQM_Encyclopedia.md`, Phase 1–158).
...
derived in TQM), **Missing** (new mathematics absent from TQM), **Rejected**
...
| TRM concept | Status | TQM replacement | Migration? |
|---|---|---|---|
...
| Theta ($\Theta\to O_5\to\lambda_\Theta\to g_{\rm obs}$) | Missing | — (homonym only: TQM-128–133 information layer) | Y (TODO) |
| Memory Channel ($\phi^2|\dot\mu|$ invariant) | Missing | — (≠ TQM-130 information memory) | Y (TODO) |
...
| TRM Cosmology — CMB / cluster sub-models | Missing | — (TQM CMB & clusters = TODO) | Y (TODO) |
...
- **TQM:** gravity = phase-gradient phenomenon (QG-022, classification C STRONG
  ...
  differs: TQM's scalar is oscillation **phase** $\theta$, not a "time-rate".
...
- **TQM:** expansion = interpretation of FLRW (QG-081); the rate/event/structure
...
- **TQM:** the gradient→Newton step is already derived (QG-022, QG-007). The RAR is
  already derived with **zero free parameters**: $g_\dagger = cH_0/(2\pi) \approx
  1.05\times10^{-10}\ \mathrm{m/s^2}$. TRM's $a_0$ and TQM's $g_\dagger$ are **the same
  number** (both $\approx cH$); TQM explains it as an emergent cosmological scale
  (QG-086, Phase 143), and QG-084/085/097 audit the $2\pi$.
- **Verdict:** **Equivalent** (gradient→Newton) + **Integrated** (RAR/BTFR/a₀). No
  migration needed; TRM's "single scalar field" ambition is realized in TQM as
...
- **TQM:** the only "Theta" is the **information layer** in the proto-matter program
  (TQM-128–133) — an autonomous information field, *not* an observable chain to $g_{\rm obs}$.
...
- **Verdict:** **Missing** (new mathematics). The homonym "Theta" must not be confused
  with TQM's information-layer Θ.
...
- **TQM:** no transport invariant of this form. TQM-130 ("information memory: signals
...
- **TQM:** the "recurring integer 3" is fully resolved but by a *different* route —
  multiplicity: $N\ge3$ DERIVED (CP lower bound), $N\le3$ DRAWN (empirical),
  spatial 3 DERIVED (complexity) (Phases 150–151). TQM has **no** mode-locking /
  rational-band mechanism.
- **Verdict:** **Missing.** The $m=3$ *mode-locking* mathematics is absent; TQM's
...
- **TQM:** gravity is a scalar phase-gradient (QG-022); there is **no** vector /
...
- **TQM:** no unified effective action of this form. (`UnifiedTQMAnalyzer`/`Framework`
  in `Research/` concern the proto-matter TQM correspondence, not $S_{\rm eff}[T,A,\Theta]$.)
- **Verdict:** **Missing.** New mathematics.
...
- **TQM:** **no** CMB analysis and **no** cluster audit exist (both marked TODO in
  `Coverage_Report.md`/`TQM_Encyclopedia.md`). The drift-expansion mechanism itself is
  **Rejected** (QG-080–089, DATA-001/002).
- **Verdict:** sub-models **Missing**; mechanism **Rejected**. The cluster/CMB regimes
  are genuinely unpopulated in TQM — but TRM's own fits there are weak or unvalidated,
...
**The survival core of TRM is its gravity/RAR content**, which TQM already re-derives
...
mathematics with **no TQM counterpart** — it is **Missing**, not equivalent.

The TRM V3.0 documents themselves correctly disclaim theorem-level closure ("not GR
replacement", "tested-effective weak-field"). This is consistent with TQM's hostile-audit
...
specific TRM equation and a specific TQM phase/result.
