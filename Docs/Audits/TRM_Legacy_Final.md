...
produce the migration summary for `TQM_Encyclopedia.md`.
...
| Class | Meaning |
|---|---|
| **Absorbed** | content already present in TQM, or equivalent to established physics TQM already accepts (GR). No migration action. |
| **Rejected** | falsified/contradicted by a later audit, or non-viable (strictly worse than TQM's own machinery). Discard. |
...
| RAR | **Absorbed** | TRM $a_0$ ≡ TQM $g_\dagger=cH_0/(2\pi)\approx1.05\times10^{-10}$; BTFR/flat curves derived with **zero** free parameters (QG-084–086, DATA-003+). | None — already derived |
| Quantum Engine | **Rejected** | $D(x)=1/(1+x+bx^2+x^4)$, $e^{-p^2/\Lambda^2}$, Padé kernel solve **no** TQM problem: BDG layer operator already UV-finite/causal/unitary/stable; Gaussian cutoff is **non-unitary**, Padé causality-ambiguous, adds 2 fitted params. | None (only the "UV regularization" *job* overlaps the lattice) |
...
| Memory Channel | **Candidate Mathematics** | $\phi^2\lvert\dot\mu\rvert$ invariant genuinely new; **no observable, no fitted params, untestable**; "memory" = homonym of TQM-130. | Low — untestable as stated |
| Theta Chain | **Candidate Mathematics** | $\Theta\to O_5\to\lambda_\Theta\to g_{\rm obs}$ genuinely new math; **homonym** with TQM-128–133 information layer; $g_{\rm obs}$ unspecified, testability low. | Low |
...
**The survival core of TRM is already inside TQM.** Time Field and RAR are absorbed
...
TQM result.** Consistent with the TRM documents' own claim boundaries
