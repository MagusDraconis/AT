...
**Test file:** `TQM.Tests/ResearchXC/MetricOperatorTests.cs`
**Result:** **PASSED (4/4).**
...
1. **Weighted graph Laplacian $L_W=D_W-W$** is constructible and is a valid Laplacian
   (symmetric, zero row-sum, positive semi-definite). TQM's existing coupling matrix $K_{ij}$
...
2. **Uniform weights reduce to $L_Q$** — $L_W=L_Q$ exactly when all edges have weight 1. So
   the unweighted graph Laplacian already used by TQM is the *uniform-weight special case* of
   the metric operator.
...
| Construction | TQM ingredient already present | Missing piece |
|---|---|---|
...
**Minimal object:** the weighted graph Laplacian $L_W=D_K-K$, built from TQM's existing
spatial coupling $K_{ij}=K\exp(-d/\lambda)$. No new primitives, no new physics — it is a
standard construction over an ingredient TQM already possesses.
...
to couple a metric to a TQM operator is the **weighted graph Laplacian** $L_W=D_K-K$, and the
