...
- **Horizon corollary and the g₀₀-only boundary:** AT's `g₀₀ = −e^(2x)` **never vanishes** — at `y = 0.4999`,
  ...
  `y = 0.6/1.0/5.0` GR has **no real surface** while AT gives 0.8221188 / 1.718282 / 147.4132. **But that
  largest discrepancy is g₀₀-only**: AT supplies no spatial metric, so no light bending, Shapiro delay or
  shadow size follows — **the most distinctive consequence is the least derivable one.**
- **Verdicts: DERIVED** = the signature, the weak-field no-go, the horizon corollary · **BOUNDARY** = the
  ...
  route. Riders: one object class, and the requirement is a **joint** measurement on the same object (`x` to
  ≈3 %, `z` to ≈8 %).
