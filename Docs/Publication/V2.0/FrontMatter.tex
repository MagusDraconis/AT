...
\begin{center}
  ...
  {\large The Actualization Theory (AT) --- v2.0 (canonical)}\\[0.5cm]
  {\large DOI: 10.5281/zenodo.20681734}\\[0.5cm]
  {\large \today}\\[1.5cm]
  \vfill
\end{center}
\clearpage
...
formation). The derivations are dimensionless first; dimensional observables appear only
after calibration to measured anchors. The boundaries of the theory are documented
explicitly: Difference as the ontological boundary, the unresolved status of $\psi$, the
numerical boundary $\pi$, the Bekenstein $2\pi$ factor, and the hosted standard-model
dynamics. The theory claims no open derivation frontier; its frontier is the experimental
validation of its pre-registered predictions.
...
is a set of spectral reads and correspondences of the D96 moments: the $1+3+8$ dimensional
correspondence, the couplings (the fine-structure inverse a fitted readout), the CKM and
PMNS correspondences, the calibrated weak scale and Higgs mass, the neutrino masses, and
the precision predictions. Cosmology is a spectrum readout: the cosmological constant is the
...
\textbf{The units.} D96 yields dimensionless occupancies, ratios, and moments. Dimensional
observables are obtained only after calibration to the relevant measured anchor: the weak
scale $v$ for weak/Higgs masses, $m_e$ for the quark sector, and the usual unit conversion
for SI-gravity quantities. The unit system is fixed by measurement and convention, not by
the spectrum alone.
...
\section*{Origin of Physical Units}
\label{fm:abs:units}
\addcontentsline{toc}{chapter}{Origin of Physical Units}

D96 produces dimensionless structure first. The weak scale $v$ calibrates the weak/Higgs
sector, the electron mass $m_e$ calibrates the quark masses, and SI-gravity quantities
additionally import $\hbar$, $c$, and the GeV$\leftrightarrow$kg conversion. Those anchors
fix the units; they are not derived from the spectrum.
