...
(Chapter~\ref{c03:chap:actualization}). This chapter completes the foundational picture by
...
primitive \cite{c04:QG278,c04:QG282}. Second, the \emph{minimal theory}: the hierarchy
\begin{equation}
...
\cite{c04:QG267,c04:QG268,c04:QG278,c04:QG282,c04:QG293,c04:QG294}, consistent with the MONO006 resolution and the
MONO007 referee-ready architecture \cite{c04:MONO006,c04:MONO007}. Throughout we use only accepted
...
\label{c04:cor:closure-n96}
The fixed point of the actualization process is the $N=96$ network, whose eigenspectrum is
the inevitable D96 spectrum.
\end{corollary}

\begin{proof}
By Theorem~\ref{c04:thm:closure-holds}, the boundary is the converged attractor of the
dynamics; by Theorem~\ref{c03:thm:n96} of Chapter~\ref{c03:chap:actualization}, that attractor is
the $N=96$ network; and by Corollary~\ref{c03:cor:spectrum-eigenspectrum} of
Chapter~\ref{c03:chap:actualization}, its eigenspectrum is the inevitable D96 spectrum.
\end{proof}
...
established by Theorem~\ref{c03:thm:derived-not-primitive} of
Chapter~\ref{c03:chap:actualization}: the process is the face of Difference, and its fixed
...
\label{c04:lem:boundary-characterizes}
...
(Theorem~\ref{c04:thm:boundary-is-fixed}). It tells us where the process converges --- the
...
\label{c04:cor:closure-not-derive}
...
\begin{proof}
By Lemma~\ref{c04:lem:boundary-characterizes}, the closure is a characterization, not a
derivation. Moreover, by Theorem~\ref{c01:thm:fundamental-boundary} of
Chapter~\ref{c01:chap:difference}, Difference is the fundamental boundary: irreducible, with its
count-conservation law as its definitional identity. A derivation of the primitive would
presuppose a deeper notion, which the fundamental-boundary audit \cite{c04:QG278} shows does
...
Consistent with MONO007 \cite{c04:MONO007}, every statement in this chapter that invokes the
...
\label{c04:def:self-consistency}
...
and identity is the zero difference \cite{c04:QG268}. The conservation of the count, which is
the definitional identity of the primitive, is the universal conservation principle of which
all conservation laws are manifestations \cite{c04:QG267}. Hence self-consistency is not an
independent postulate but a consequence of the primitive's structure.
\end{proof}

\begin{corollary}[Conservation is universal]
\label{c04:cor:conservation-universal}
...
\begin{proof}
The conservation-principle audit \cite{c04:QG267} verifies that the six conservation laws (Born
...
Theorem~\ref{c03:thm:count-conservation} of Chapter~\ref{c03:chap:actualization}, count
...
the operator layer established in Chapter~\ref{c03:chap:actualization}
(Theorem~\ref{c03:thm:resonance-readout}). Each sector is a distinct pattern of difference in
...
\label{c04:def:difference-principle}
...
\label{c04:cor:difference-anchors}
...
\begin{proof}
By Theorem~\ref{c04:thm:difference-first}, every reduction returns to Difference; by
Theorem~\ref{c03:thm:process-face} of Chapter~\ref{c03:chap:actualization}, Actualization is the
process face of Difference; and the spectrum and physics are the outputs of that process
(Chapter~\ref{c03:chap:actualization}, Theorem~\ref{c03:thm:spectrum-inevitable}). Hence the
...
The minimal-theory audit \cite{c04:QG294} then re-derives every compressed layer from the
...
\cite{c04:QG293} establishes that removing Actualization destroys the count-producing process.
...
\label{c04:cor:minimal-architecture}
The canonical architecture of the theory is the minimal hierarchy
\eqref{c04:eq:minimal}: Difference → Actualization → Inevitable Spectrum → Physics, with
primitives $\{\text{Difference},\eta\}$, Actualization derived, the spectrum inevitable, and
physics emergent.
\end{corollary}

\begin{proof}
The minimal hierarchy is complete (Theorem~\ref{c04:thm:minimal-hierarchy}) and irreducible
(Theorem~\ref{c04:thm:minimal-irreducible}); the primitives are the pair
$\{\text{Difference},\eta\}$ (Theorem~\ref{c02:thm:minimal-foundation} of
Chapter~\ref{c02:chap:eta}); Actualization is derived (Theorem~\ref{c03:thm:derived-not-primitive}
of Chapter~\ref{c03:chap:actualization}); the spectrum is inevitable
(Theorem~\ref{c03:thm:spectrum-inevitable} of Chapter~\ref{c03:chap:actualization}). This is the
referee-ready structure of MONO007 \cite{c04:MONO007}.
\end{proof}

\begin{remark}[Referee-safe completeness]
Consistent with MONO007 \cite{c04:MONO007}, where the monograph claims completeness of the
...
\label{c04:lem:reduction-closes}
The reduction program closes at the minimal hierarchy \eqref{c04:eq:minimal}: all intermediate
layers of the fuller hierarchy are derivable from it, and none is actually required as an
input.
\end{lemma}

\begin{proof}
The minimal-theory audit \cite{c04:QG294} classifies the five compressed layers --- Closure,
...
closes at \eqref{c04:eq:minimal}.
\end{proof}
...
(Corollary~\ref{c04:cor:closure-n96}), which is a layer of the minimal hierarchy. Conversely,
the spectrum layer of the minimal hierarchy is the eigenspectrum of the converged network
(Chapter~\ref{c03:chap:actualization}, Corollary~\ref{c03:cor:spectrum-eigenspectrum}), so the
...
\label{c04:cor:anchor-bound-complete}
...
\begin{proof}
Difference anchors by the Difference Principle (Theorem~\ref{c04:thm:difference-first},
Corollary~\ref{c04:cor:difference-anchors}); the closure bounds by the Closure Principle
(Theorem~\ref{c04:thm:closure-holds}, Corollary~\ref{c04:cor:closure-n96}); the minimal theory
completes by Theorem~\ref{c04:thm:minimal-hierarchy} and Theorem~\ref{c04:thm:minimal-irreducible}.
...
\begin{enumerate}
\item \textbf{The Closure Principle} (Theorem~\ref{c04:thm:closure-holds},
Corollary~\ref{c04:cor:closure-n96}): the boundary of the theory is the fixed point of
Difference's own dynamics, closing at $N=96$;
\item \textbf{Boundary and fixed point} (Theorem~\ref{c04:thm:boundary-is-fixed},
Lemma~\ref{c04:lem:boundary-characterizes}, Corollary~\ref{c04:cor:closure-not-derive}): the
boundary is derived, characterizes the primitive, and --- decisively --- the Closure
Principle does \emph{not} derive Difference;
\item \textbf{Self-consistency} (Theorem~\ref{c04:thm:self-consistency-presupposes},
Corollary~\ref{c04:cor:conservation-universal}): self-consistency presupposes Difference, and
conservation is universal;
\item \textbf{Individuation} (Theorem~\ref{c04:thm:individuation}): the distinct sectors derive
from Difference;
\item \textbf{The Difference Principle} (Theorem~\ref{c04:thm:difference-first},
Corollary~\ref{c04:cor:difference-anchors}): Difference is the self-referential first concept,
anchoring the hierarchy;
\item \textbf{The minimal theory} (Theorem~\ref{c04:thm:minimal-hierarchy},
Theorem~\ref{c04:thm:minimal-irreducible}, Corollary~\ref{c04:cor:minimal-architecture}): the
minimal hierarchy is complete and irreducible;
\item \textbf{Consequences} (Lemma~\ref{c04:lem:reduction-closes},
Theorem~\ref{c04:thm:closure-minimality}, Corollary~\ref{c04:cor:anchor-bound-complete}): the
