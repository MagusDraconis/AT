...
\label{sec:difference-introduction}
...
\cite{QG278,QG282}; and that the two great physical sectors of the theory, the scalar
counting measure $\rho$ and the tensor field $\psi$, are not independent primitives but the
\emph{trace and traceless faces of the one Difference} \cite{QG286,QG301}.
...
\cite{QG268}. A Q-event is a unit of Difference.
\end{definition}
...
\label{cor:actualization-derived}
Actualization is \emph{not} a primitive. It is Difference's own count-producing process.
\end{corollary}

\begin{proof}
By the same removal test \cite{QG292,MONO006}, Actualization fails when Difference is removed
...
primitive. Its fixed point is the closure $N=96$ \cite{QG282}.
\end{proof}

\begin{remark}
The canonical core \eqref{eq:canonical-core} is therefore read: \emph{Difference} (the
...
\label{sec:fundamental-boundary}
...
\label{thm:fundamental-boundary}
Difference is the fundamental boundary of AT.
\end{theorem}

\begin{proof}
The boundary audits \cite{QG278,QG279} establish the three criteria. (i) \emph{Irreducible:}
...
\emph{Support:} by Theorem~\ref{thm:minimal-primitives}, removing Difference collapses every
...
actualization process, per the Closure Principle \cite{QG282}. Hence Difference satisfies the
three defining properties of a fundamental boundary.
\end{proof}

\begin{lemma}[Boundary is fixed point]
\label{lem:boundary-fixed-point}
...
same final geometry \cite{QG282}.
\end{lemma}
...
\cite{QG282}. The boundary is the closure of this dynamics.
\end{proof}
...
\cite{QG216}, the \emph{isotropic} difference from the uniform background --- the
\emph{count} difference. $\psi$ is the tensor field: the spin-2 Weyl content \cite{QG285},
the \emph{anisotropic} difference from conformal flatness --- the \emph{orientation}
difference. By the rank-2 decomposition theorem \eqref{eq:decomposition}, the trace and the
traceless part are determined by $A_{ij}$; neither is an independent input. Hence $\rho$ and
$\psi$ are dual projections of the same Difference \cite{QG286}.
\end{proof}

\begin{corollary}[Duality is invariant]
\label{cor:duality-invariant}
...
\emph{number}: every frozen prediction is reproduced exactly \cite{QG301}.
\end{corollary}
...
\cite{QG301}. The duality is therefore a reinterpretation of content, not a modification of
...
Theorem~\ref{thm:minimal-primitives} holds: $\eta$ is necessary exactly for the tensor
sub-sector.
\end{remark}
...
\emph{derived}, not primitive \cite{QG282}.
\end{theorem}
...
convergence (Lemma~\ref{lem:boundary-fixed-point}). The boundary is therefore not an
imported cut-off but the stable fixed point of the process itself; the closure principle
states exactly this --- the process converges, the boundary is its closure \cite{QG282}.
\end{proof}

\begin{corollary}[Boundary characterizes, does not derive]
\label{cor:closure-characterizes}
The Closure Principle \emph{characterizes} the fundamental boundary; it does not \emph{derive}
the primitive Difference.
\end{corollary}

\begin{proof}
Difference is the primitive: by Definition~\ref{def:difference} its count-conservation law
is its definitional identity, and by Theorem~\ref{thm:fundamental-boundary} it is
...
the referee-safe reading adopted by the canonical monograph \cite{MONO007}.
\end{proof}
...
the same primitive \cite{QG286,QG301}.
\end{lemma}
...
eigenspectrum of the converged network --- and not an additional primitive \cite{QG295}.
\end{theorem}
...
whose primitive is Difference \cite{QG295,QG282}.
\end{proof}
...
no further primitive is required \cite{QG292,QG294}.
\end{corollary}
...
\begin{enumerate}
\item \textbf{Primitive status} (Theorem~\ref{thm:minimal-primitives}): the minimal primitive
set is $\{\text{Difference},\eta\}$, and Actualization is derived from Difference
(Corollary~\ref{cor:actualization-derived}).
\item \textbf{Fundamental boundary} (Theorem~\ref{thm:fundamental-boundary}): Difference is
irreducible, supports the whole hierarchy, and is self-bounded by its own fixed point.
\item \textbf{The duality} (Theorem~\ref{thm:duality}): $\rho$ and $\psi$ are the trace and
traceless faces of the one Difference; the reinterpretation is invariant
(Corollary~\ref{cor:duality-invariant}).
\item \textbf{Closure} (Theorem~\ref{thm:closure}): the boundary is the fixed point of
Difference's own dynamics, characterizing rather than deriving the primitive
(Corollary~\ref{cor:closure-characterizes}).
\item \textbf{Consequences} (Theorem~\ref{thm:spectrum-inevitable},
Corollary~\ref{cor:core}): the spectrum is inevitable, and the canonical core is complete.
\end{enumerate}
...
\bibitem{QG279} AT-QG Phase 279, \emph{Boundary Legitimacy Audit}, Docs/Research.
