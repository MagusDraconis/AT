...
Higgs, and a set of precision predictions --- as reads of the D96 spectral moments
according to their documented claim status (theorem, correspondence, calibration, hosted,
fit) \cite{c11:QG149,c11:QG150,c11:QG157}.
...
Throughout, the hosted-standard-model caveat is retained: the standard-model
\emph{observables} are reconstructed according to their documented claim status (theorem,
correspondence, calibration, hosted, fit), while the standard-model \emph{Lagrangian} and
its dynamics are hosted --- a boundary, not a derivation gap, per the referee-safe
architecture of MONO007 \cite{c11:MONO007}.
...
$1+3+8$ gauge sector --- corresponds dimensionally to the D96 sector structure: the
degree-12 content of the $C_{96}$ generator ring. The gauge groups and their Lie algebras
are hosted, not derived.
\end{theorem}

\begin{proof}
The gauge-origin audit \cite{c11:QG161} establishes that the $1+3+8$ gauge sector is the
degree-12 structure of the $C_{96}$ sector: the gauge-generator counts are the D96
spectral content, giving one $U(1)$, three $SU(2)$, and eight $SU(3)$ generators as a
dimensional correspondence. Since the D96 spectrum is itself the derived output of the
attractor (Chapter~\ref{c06:chap:d96}, Theorem~\ref{c06:thm:d96-derived}), the
correspondence is a read of the derived spectrum; the gauge groups and the interaction
dynamics remain hosted.
\end{proof}
...
The gauge couplings are read as ratios of D96 spectral constants:
\begin{equation}
...
spectral ratio. Each coupling is a ratio of spectral constants ($\Sigma m$,
$\Sigma\sqrt{m}$, $\#d$), the constants themselves derived from the spectrum
(Chapter~\ref{c06:chap:d96}, Corollary~\ref{c06:cor:constants-derived}); the weak and
strong ratios correspond to the measured couplings, while the fine-structure inverse is a
fitted readout (137 = 95+42). None of the couplings carries a defined renormalization
scale.
\end{proof}
...
\label{c11:thm:ckm}
The CKM matrix is a spectral correspondence: the elements are ratios of doublet, octave,
and occupancy quantities, matched to the measured quark mixing.
\end{theorem}

\begin{proof}
The CKM-origin audit \cite{c11:QG165} establishes the constructions:
$V_{us} = \#d/(2\Sigma m)$, $V_{cb} = (\Omega_0/\Omega_2)^{\delta_d}$,
$V_{ub} = 2V_{cb}\cdot(\mathrm{occ}_0/\mathrm{occ}_2)$, with mean deviation $0.58\%$; here
$\Omega_0 = 0.924$ and $\Omega_2 = 3.385$ are the lowest and highest octave-family centers
(octave-family centers, not the Laplacian modes $\omega_k = \sqrt{\lambda_k}$) and
$\delta_d = 2.449$ is the down-sector effective dimension. The
...
\label{c11:thm:pmns}
The PMNS matrix is a spectral correspondence: the angles
...
T3-only access reads of the D96 content; hence the PMNS matrix is a spectral
correspondence, not an imported input.
\end{proof}

Both the quark (CKM) and the lepton (PMNS) mixing matrices are therefore spectral
correspondences, with no imported mixing input. Consistent with the state-phase lattice of
Chapter~\ref{c09:chap:qm} ($\theta_k = 2\pi k/N$), the mixing and CP phases
$\delta_{CP}$ and $\delta_\nu$ are spectral-ratio phases, $\delta = \arcsin(r)$ from the
chiral circulation: continuous functions of the spectral ratios, not lattice-restricted.
...
\end{equation}
with the $W$ and $Z$ masses following from the Weinberg projection. Here
$\sigma_{\mathrm{occ}} = 39.127$ is the collective occupation-density scalar --- the
standard deviation of the octave occupancies $[4,4,87]$,
$\sigma_{\mathrm{occ}} = \sqrt{\mathrm{Var}[4,4,87]}$.
\end{theorem}
...
The neutrino mass splittings correspond to closed-form D96 laws, calibrated to
$\mathrm{eV}^2$ by an implicit scale:
...
\end{equation}
Here $\alpha_2 = 0$, consistent with the real Majorana mass matrix derived from the D96
...
dynamics are hosted --- a boundary, not a derivation claim of this chapter. The
\emph{observables} presented here are reconstructed according to their documented claim
status (theorem, correspondence, calibration, hosted, fit); the Lagrangian form
...
are reconstructed according to their documented claim status (theorem, correspondence,
calibration, hosted, fit). The hosted SM Lagrangian is the single referenced
...
spectral correspondences (Theorem~\ref{c11:thm:ckm}, Theorem~\ref{c11:thm:pmns}); and the
...
observable is therefore a spectral read or calibrated construction drawn from the
spectrum, with the fine-structure inverse the single fitted readout
