...
the trace and traceless faces of the one Difference \cite{QG286}. That duality, however, is
...
necessity program \cite{QG289,QG290,QG291,QG292}: $\eta$ is the second primitive of the
...
\cite{MONO007}. Throughout we use only accepted canonical results, and introduce no new
physics and no speculation.

\section{Necessity of \texorpdfstring{$\eta$}{eta}}
\label{sec:eta-necessity}
...
and the Weyl content --- the difference from conformal flatness \cite{QG285} --- are all
...
\cite{QG291} classifies $\eta$ accordingly: not \emph{derived} (no count produces a metric,
...
triad enters the theory elsewhere --- and the metric ansatz \eqref{eq:metric-ansatz} takes
...
\begin{proof}
The metric ansatz \eqref{eq:metric-ansatz} is the accepted canonical form \cite{QG290}.
...
this factorization as the irreducible reading structure of the theory \cite{QG290}.
\end{proof}

\begin{remark}[The scalar face needs no reference; the tensor face does]
Consistent with Chapter~\ref{chap:difference}, the scalar face $\rho$ is defined without
reference to $\eta$, while the tensor face $\psi$ --- the Weyl content --- is defined
\emph{against} $\eta$. This asymmetry is precisely why Theorem~\ref{thm:minimal-primitives}
of Chapter~\ref{chap:difference} holds: $\eta$ is necessary exactly for the tensor
sub-sector.
\end{remark}

\section{\texorpdfstring{$\eta$}{eta} and the Tensor Sector}
\label{sec:eta-tensor}
...
\begin{proof}
The psi-reinterpretation audit \cite{QG285} establishes the link "$\psi$ as Difference": the
...
reference is $\eta$ (Definition~\ref{def:eta}). Hence $\psi$ is read against $\eta$. The
...
(Corollary~\ref{cor:tensor-requires-eta} of this chapter; Theorem~\ref{thm:minimal-primitives}
of Chapter~\ref{chap:difference}).
\end{proof}

\section{Difference, \texorpdfstring{$\eta$}{eta} and the Minimal Foundation}
\label{sec:minimal-foundation}

\begin{theorem}[The minimal foundation is \texorpdfstring{$\{\text{Difference},\eta\}$}{{Difference, eta}}]
\label{thm:minimal-foundation}
...
\begin{proof}
By the foundation stress test \cite{QG292}, removing Difference collapses all five layers
(the count-conservation identity is the primitive's definition, Chapter~\ref{chap:difference},
Theorem~\ref{thm:fundamental-boundary}); removing $\eta$ collapses only the tensor sub-sector
(Theorem~\ref{thm:tensor-read}, Corollary~\ref{cor:tensor-requires-eta}). Both are therefore
necessary. Neither is derivable from the other (Lemma~\ref{lem:eta-not-derived} and
Corollary~\ref{cor:eta-independent}). Hence the pair is irreducible and independent, and the
core hierarchy \eqref{eq:core} --- with Actualization derived from Difference per MONO006
\cite{MONO006} --- is the canonical minimal foundation.
\end{proof}

\begin{lemma}[Actualization is independent of \texorpdfstring{$\eta$}{eta}]
\label{lem:actualization-eta-free}
Actualization, the count-producing process of Difference, does not depend on $\eta$: counting
needs no metric reference.
\end{lemma}

\begin{proof}
The foundation stress test \cite{QG292} establishes that removing $\eta$ leaves Actualization
intact: counting is a graph identity (the handshake lemma), not a metric identity. Combined
with MONO006 \cite{MONO006}, which establishes that removing Difference collapses Actualization
...
\label{cor:canonical-architecture}
...
structure adopted by MONO007 \cite{MONO007}.
\end{corollary}
...
framework --- and not a primitive \cite{QG291}.
\end{definition}

\begin{theorem}[$\eta$ is a primitive; $\pi$ is a boundary]
\label{thm:eta-primitive-pi-boundary}
...
\begin{proof}
The framework necessity audit \cite{QG291} classifies the two items separately. $\eta$ is
\emph{necessary}: without it the trace/traceless duality and the conformal reading are
undefined (Theorem~\ref{thm:eta-necessary}). $\pi$ is \emph{redundant} as a theory input: no
...
factor that the framework cannot produce internally, an explicit boundary \cite{QG185}.
...
the two \cite{MONO007}.
\end{proof}

\begin{corollary}[No conflation]
\label{cor:no-conflation}
...
Per MONO007 \cite{MONO007}, the distinction between the primitive $\eta$ and the boundary
...
\label{sec:eta-consequences}

\begin{lemma}[One primitive pair, one hierarchy]
\label{lem:eta-consequence-1}
...
\eqref{eq:core}: the scalar face of Difference feeds the spectral readout of all observables;
...
derived quantity is used to define $\eta$ (Lemma~\ref{lem:eta-not-derived}), and no tensor
observable is used in the scalar sector that defines the count. The referee-ready structure
of MONO007 \cite{MONO007} confirms that the presentation introduces no circular
...
\section{Summary}
\label{sec:eta-summary}

This chapter has established the second member of the canonical architecture. We have proved:

\begin{enumerate}
\item \textbf{Necessity} (Theorem~\ref{thm:eta-necessary}): the tensor reference $\eta$ is
necessary --- without it no trace, no traceless part, no conformal flatness, no tensor face;
\item \textbf{Independence} (Lemma~\ref{lem:eta-not-derived},
Corollary~\ref{cor:eta-independent}): $\eta$ is not derivable from Difference;
\item \textbf{The conformal framework} (Theorem~\ref{thm:conformal}): the physical metric is
the conformal rescaling $g=\rho^{2/d}\,\eta$ of the tensor reference;
\item \textbf{The tensor sector} (Theorem~\ref{thm:tensor-read},
Corollary~\ref{cor:tensor-requires-eta}): $\psi$ is read against $\eta$, and tensor
observables require it while scalar observables do not;
\item \textbf{The minimal foundation} (Theorem~\ref{thm:minimal-foundation},
Lemma~\ref{lem:actualization-eta-free}, Corollary~\ref{cor:canonical-architecture}): the
minimal foundation is $\{\text{Difference},\eta\}$, with Actualization derived from
Difference alone;
\item \textbf{The $\eta$/$\pi$ distinction} (Theorem~\ref{thm:eta-primitive-pi-boundary},
Corollary~\ref{cor:no-conflation}): $\eta$ is a primitive, $\pi$ is a boundary constant,
never conflated;
\item \textbf{Consequences} (Theorem~\ref{thm:reference-safety},
Corollary~\ref{cor:eta-architecture}): the derivation graph is reference-safe and acyclic,
...
\bibitem{QG285} AT-QG Phase 285, \emph{Psi Reinterpretation Audit}, Docs/Research.
\bibitem{QG286} AT-QG Phase 286, \emph{Difference Duality Audit}, Docs/Research.
\bibitem{QG289} AT-QG Phase 289, \emph{Anchor Inventory Audit}, Docs/Research.
\bibitem{QG290} AT-QG Phase 290, \emph{Framework Inventory Audit}, Docs/Research.
