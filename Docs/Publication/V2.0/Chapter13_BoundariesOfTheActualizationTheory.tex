...
The standard-model \emph{Lagrangian} and its interaction dynamics are hosted: the framework
reconstructs the observables (masses, couplings, mixings) but carries the Lagrangian form as
an external structure. The hosted SM dynamics (the Lagrangian and its structure) is thus a
boundary, while the reconstructed SM observables (the values read from the D96 moments) are
derived physics.
\end{theorem}
...
propagators) is hosted; the SM dynamics is not complete as a derivation. The reconstructed
...
reconstructs: the physics observables established in the preceding chapters, with the
boundaries disclosed and the validation items awaiting experiment.
\end{theorem}
