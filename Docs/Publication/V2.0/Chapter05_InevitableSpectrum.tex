...
The canonical attractor graph is the 12-regular circulant ring
$C_{96}(\pm1,\ldots,\pm6)$: the adjacency is $A_{ij}=1$ iff
$(i-j)\bmod 96 \in \{\pm1,\ldots,\pm6\}$. Its graph Laplacian $L=D-A$ has the closed-form
eigenvalues
\[
\lambda_k \;=\; 2\sum_{d=1}^{6}\Bigl(1-\cos\frac{2\pi dk}{96}\Bigr), \qquad k=0,\ldots,95,
\]
with modes $\omega_k=\sqrt{\lambda_k}$ (the $\omega=\sqrt{\lambda}$ span convention). All
reported multiplicities, moments and span values are reproducible from the canonical
$C_{96}(\pm1,\ldots,\pm6)$ circulant graph.
