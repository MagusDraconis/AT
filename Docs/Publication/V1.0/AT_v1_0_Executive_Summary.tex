% THE Q-MODEL (AT) — Executive Summary
% Publication-quality condensed overview for Zenodo readers.
% Source: AT_v1_0_Monograph_Final.tex (no new physics, no changed conclusions).
% Build: pdflatex AT_v1_0_Executive_Summary.tex (twice)
...
\title{\bfseries THE Q-MODEL (AT) — Executive Summary\\[2mm]
...
(\texttt{AT\_v1\_0\_Monograph\_Final.tex}). It introduces no new physics, changes no
...
\textbf{What is AT.} THE Q-MODEL (AT) is a theory of \emph{structure} and \emph{content}
...
\textbf{Why AT was created.} AT began from a single question: are matter, quantum
...
\textbf{The TRM $\to$ AT transition.} The immediate predecessor was the \emph{Temporal
...
oscillator experiments} (AT-001--008) established that synchronization emerges
...
\texttt{AT.Tests/ResearchXC} (or \texttt{ResearchQG}). The complete inventory is
...
verified: the standard $g\to G_{\mu\nu}$ chain works, but AT has no native metric (the
...
\begin{enumerate}
\item \textbf{The G4 metric $\to$ operator coupling.} AT imports $g_{\mu\nu}$ via
...
yet separates AT from SM$+\Lambda$CDM. The RAR $g_\dagger=cH_0/2\pi$ matches $a_0$ but its
