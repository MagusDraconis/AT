...
% Build: pdflatex TQM_v1_0_Monograph_Final_DE.tex (zweimal)
%
...
\vspace{0.3em}
\noindent
Diese Monographie ist die Forschungsmonographie-Ausgabe von THE Q-MODEL (TQM), einer
...
sind importiert (bewiesen, aber nicht aus TQM abgeleitet), $G=\ell^2c^3/\hbar$ ist
Dimensionsanalyse, und keine eindeutige, scharfe Vorhersage unterscheidet TQM bisher von
SM$+\Lambda$CDM. Jede als \textbf{verifiziert} markierte Aussage wird durch einen
ausführbaren xUnit-Test in \texttt{TQM.Tests/ResearchXC} (oder \texttt{ResearchQG})
...
die Einstein-Tensor-Kette), gerade um zu zeigen, wo die TQM-Ableitung endet.
...
\chapter{Der Ursprung von TQM}
\label{ch:origin}

\section{Motivation}

THE Q-MODEL (TQM) begann aus einer einzigen, bewusst einfachen Frage: Sind Materie,
...
TQM entstand nicht im Vakuum. Ihr unmittelbarer Vorgänger war das \textbf{Temporal
...
Feldes --- die Idee, die nach dem Übergang zu TQMs Phasengradienten-Gravitationsbild wurde.
...
Die Beziehung zwischen TRM und TQM wird am besten durch die \emph{TRM Legacy Final
...
Invariante $\varphi^2|\dot\mu|$ ohne Observable) und die Theta Chain (ein Homonym zu TQMs
eigenem Informationsfeld). \emph{Eines} blieb offen: die Unified-Action-Roadmap.

\section{Der Übergang TRM $\to$ TQM}
...
\subsection{Frühe Oszillatorexperimente (TQM-001--TQM-008)}
...
hervorgeht --- Substanz hat. TQM-001 zeigte, dass ein Ordnungsparameter $R$ einer
Population gegen $1$ strebt: Kohärente Synchronisation wird nicht von Hand eingesetzt,
sie erscheint. TQM-002 schloss dann eine verlockende Sackgasse: Zufallsmatrizen liefern
Wigner-artige Spektren, erzeugen aber \emph{nicht} die Art strukturierter Emergenz, die
das Programm brauchte --- Zufall allein genügt also nicht. TQM-003 zeigte, dass auch
strukturierte Topologie allein unzureichend ist --- die Dynamik, nicht der statische Graph,
trägt das Gewicht. TQM-004 zeigte, dass eine feldvermittelte Wechselwirkung für sich allein
...
TQM-005 fand stabile Resonanzcluster aus drei Oszillatoren, die sich bildeten und über den
...
Cluster das Feld noch nicht dominierten. TQM-006 fand die erste scharfe Schwelle, eine
...
des Programms. TQM-007 berichtete fünf Resonanzfamilien. TQM-008 zerstörte dann diese
...
Seed-Artefakte entlarvt wurden. Die Lektion von TQM-008 --- scheinbare Vielfalt kann ein
...
Resonanzfamilien-Vielfalt von TQM-007 (Artefakte eines einzigen Seeds); dynamische Graphen
...
$\to$ Operator-Kopplung} --- die ``G4''-Lücke: TQM importiert $g_{\mu\nu}$ über den Satz
...
Der Weg, der scheiterte, ist die \emph{native} Metrikdynamik. TQM erzeugt $g_{\mu\nu}$
...
TQM-007-Resonanz-``Familien'' kollabierten in einer Reproduzierbarkeitsstudie mit 400
...
keine Emergenz (TQM-002); statische Topologie allein war unzureichend (TQM-003); die
scheinbare Vielfalt der Resonanzfamilien kollabierte auf einen einzigen reproduzierbaren
Modus (TQM-008); dynamische Graphen erzeugten keine offene Innovation (X002--X004); und
...
\section{Die Tight-Binding-Identität (TQM-142)}
...
\section{Schr\"odinger aus Reversibilität (TQM-149--151)}
...
{Akzeptiert als Satz (TQM-149--151), weil er exakt ist und weil sein Kontinuumsgrenzwert
dann unabhängig durch \texttt{GraphLaplacianContinuumTests} verifiziert wird.}

\section{Observablen aus dem Spektrum (TQM-145)}
...
\begin{enumerate}
\item $L_Q$-Eigenvektorbasis $=$ Hilbertraum (TQM-149); und
...
Vorhersagen.}
{Mathematisch (TQM-142 Tight-Binding-Identität; TQM-149--151 Reversibilitätssatz); die
...
\begin{remark}
Dies ist die wichtigste strukturelle Tatsache des Kontinuumsprogramms: TQM besitzt
...
Aufnahme besteht darin zu zeigen, \emph{genau wo} sie bricht, wenn man versucht, ihr TQMs
...
{Es ist Standard-Differentialgeometrie, keine TQM-Ableitung; es erzeugt für sich genommen
kein $3{+}1$.}

\begin{remark}[Wo die Kette in TQM bricht]
Die Standardkette ist voll funktionsfähig. Aber TQMs eigene Analysatoren
...
bricht bei Schritt~1 — \emph{Metrik} $\to$ \emph{Christoffels} —, weil TQM kein natives
...
{Er ist importiert, nicht TQM-abgeleitet, und benötigt
...
Glied & Status & Natur \\
\midrule
Q-Ereignisse & \textbf{NATIV} & TQM-abgeleitetes Primitiv \\
...
Operator-Kopplung (die ``G4''-Lücke): TQM erzeugt $g_{\mu\nu}$ nicht aus Q-Ereignissen,
es importiert es.
\end{remark}
...
\begin{remark}
TQM leitet die Eichgruppen-\textbf{Struktur} ab (welche Gruppe), \textbf{nicht} die
...
Gravitation ist in TQM ein Phasengradienten-Phänomen. Dieses Kapitel hält den ehrlichen
...
während sie die beiden Stellen isoliert, an denen TQM wirklich abweicht ($\Lambda(t)$ und
Singularitätsauflösung).}
...
(Phase 140).
\item \textbf{Pantheon+SH0ES}: TQM und $\Lambda$CDM sind beim aktuellen Signal
...
$g\to G_{\mu\nu}$ funktioniert, aber TQM hat keine native Metrik (Malament-Import), und
...
Ist TQM journalreif? & NEIN \\
Ist TQM whitepaperreif? & JA \\
\bottomrule
\end{tabular}
\end{center}
...
\item Parameterkompression SM+GR ($\sim$26) $\to$ TQM ($2{+}3$), $\sim$5--8$\times$
Reduktion.
...
\midrule
TQM-001 & Entsteht Synchronisation? & Ja: Ordnungsparameter $R\to1$. \\
TQM-002 & Ergeben Zufallsmatrizen Emergenz? & Nein: Wigner-Spektrum, keine strukturierte Emergenz. \\
TQM-003 & Ist statische Topologie ausreichend? & Nein: Dynamik überwiegt statische Struktur. \\
TQM-004 & Synchronisiert ein Feld zwei asymmetrische Oszillatoren? & Nein: Feld wirkt als Resonanzmedium. \\
TQM-005 & Bilden sich Resonanzcluster? & Ja: zwei Cluster aus drei Oszillatoren, $\sim5\%$ Energiebeteiligung. \\
TQM-006 & Gibt es eine kritische Resonanzdichte? & Ja: $\rho_c\approx0.09$, perkolationsartiger Übergang. \\
TQM-007 & Wie viele Resonanzfamilien existieren? & Fünf gemeldet. \\
TQM-008 & Welche Familien sind reproduzierbar? & Nur F4 (74\%, Score 0,721); der Rest sind Keim-Artefakte. \\
\midrule
