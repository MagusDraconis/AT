...
- The central derivation claim (Einstein recovery from Q-events) remains **logical, not
  ...
  AT-derived), $G=\ell^2c^3/\hbar$ is *dimensional analysis*, and no unique sharp
  prediction yet discriminates AT from SM + $\Lambda$CDM.
...
THE Q-MODEL; AT; emergence; quantum foundations; causal set theory; graph Laplacian;
...
| File | Purpose | Status |
|---|---|---|
| `AT_v1_0.tex` | LaTeX source of the paper | **created** |
| `AT_v1_0.pdf` | compiled PDF | **compiled** (5 pages) |
...
| `AT_v1_0_PublicationPackage.md` | this overview | **created** |

---

## PDF checklist

- [x] Compile `AT_v1_0.tex`: `pdflatex AT_v1_0.tex` (run twice for the table of contents).
...
- [x] Name the PDF `AT_v1_0.pdf` and place it in this directory.
...
1. `AT_v1_0.tex` — LaTeX paper (with title-page caveat).
2. `AT_v1_0.pdf` — compiled white paper.
