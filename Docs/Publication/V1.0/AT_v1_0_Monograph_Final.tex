...
% Build: pdflatex AT_v1_0_Monograph_Final.tex (twice)
%
...
\vspace{0.3em}
\noindent
This monograph is the research-monograph edition of THE Q-MODEL (AT), a theory of
...
the BDG action are imported (proven but not AT-derived), $G=\ell^2c^3/\hbar$ is
dimensional analysis, and no unique sharp prediction yet discriminates AT from
SM$+\Lambda$CDM. Every claim marked \textbf{verified} is backed by an executable xUnit
test in \texttt{AT.Tests/ResearchXC} (or \texttt{ResearchQG}); see Part~V and Appendix~A.
\end{minipage}}
\end{center}
...
\chapter{The Origin of AT}
\label{ch:origin}

\section{Motivation}

THE Q-MODEL (AT) began from a single, deliberately simple question: are matter, quantum
...
AT did not appear in a vacuum. Its immediate predecessor was the \textbf{Temporal
...
after the transition, became AT's phase-gradient gravity picture. The \emph{Radial
...
The relationship between TRM and AT is best summarized by the \emph{TRM Legacy Final
...
homonym of AT's own information field). \emph{One} remained open: the Unified Action
roadmap.

\section{The TRM $\to$ AT transition}
...
\subsection{Early oscillator experiments (AT-001--AT-008)}
...
teeth. AT-001 showed that an order parameter $R$ of a population climbs toward $1$:
coherent synchronization is not put in by hand, it appears. AT-002 then closed a tempting
blind alley: random matrices give Wigner-like spectra but do \emph{not} produce the kind
of structured emergence the program needed, so randomness alone is not enough. AT-003
showed that structured topology alone is also insufficient --- the dynamics, not the
static graph, carries the weight. AT-004 showed that a field-mediated interaction does
...
AT-005 found stable resonance clusters of three oscillators forming and persisting over
...
the field. AT-006 found the first sharp threshold, a critical resonance density
$\rho_c\approx0.09$, above which persistent coherent modes appear: a percolation-like
transition, and the program's first well-defined critical point. AT-007 reported five
resonance families. AT-008 then demolished that apparent diversity with a
400-simulation reproducibility study, leaving a single universal coherent mode (F4) and
exposing the others as seed artifacts. The lesson of AT-008 --- apparent diversity can be
...
topology as a sufficient condition; the apparent resonance-family diversity of AT-007
...
The path that failed is the \emph{native} metric dynamics. AT does not produce
...
\item \textbf{Apparent diversity can be a single seed.} The AT-007 resonance ``families''
...
produce emergence (AT-002); static topology alone was insufficient (AT-003); the
apparent diversity of resonance families collapsed to a single reproducible mode (AT-008);
...
\section{The tight-binding identity (AT-142)}
...
\section{Schr\"odinger from reversibility (AT-149--151)}
...
{Accepted as a theorem (AT-149--151) because it is exact, and because its continuum limit
is then independently verified by \texttt{GraphLaplacianContinuumTests}.}

\section{Observables from the spectrum (AT-145)}
...
\begin{enumerate}
\item $L_Q$ eigenvector basis $=$ Hilbert space (AT-149); and
...
{Mathematical (AT-142 tight-binding identity; AT-149--151 reversibility theorem); the
...
\begin{remark}
This is the single most important structural fact of the continuum program: AT possesses
...
show \emph{exactly where} it breaks when one tries to feed it AT's own objects.
...
selection argument.}
{It is standard differential geometry, not a AT derivation; it does not by itself
produce $3{+}1$.}

\begin{remark}[Where the chain breaks in AT]
The standard chain is fully functional. But AT's own analyzers
...
Step~1 — \emph{metric} $\to$ \emph{Christoffels} — because AT possesses no native metric
...
{It is imported, not AT-derived, and it needs past-and-future-distinguishing spacetimes;
...
Link & Status & Nature \\
\midrule
Q-events & \textbf{NATIVE} & AT-derived primitive \\
...
(the ``G4'' gap): AT does not produce $g_{\mu\nu}$ from Q-events, it imports it.
\end{remark}
...
\begin{remark}
AT derives the gauge-group \textbf{structure} (which group), \textbf{not} the gauge
...
Gravity in AT is a phase-gradient phenomenon. This chapter records the honest status:
...
two places where AT genuinely departs ($\Lambda(t)$ and singularity resolution).}
...
\item \textbf{Pantheon+SH0ES}: AT and $\Lambda$CDM are indistinguishable at the current
...
CAMB-class Boltzmann solver --- standard $\Lambda$CDM physics, not new AT physics.
\end{remark}
...
G_{\mu\nu}$ chain works, but AT has no native metric (Malament import), and the BDG action
is imported.
...
Is AT journal-ready? & NO \\
Is AT white-paper-ready? & YES \\
\bottomrule
\end{tabular}
\end{center}
...
\item Parameter compression SM+GR ($\sim$26) $\to$ AT ($2{+}3$), $\sim$5--8$\times$
reduction.
...
\midrule
AT-001 & Does synchronization emerge? & Yes: order parameter $R\to1$. \\
AT-002 & Do random matrices give emergence? & No: Wigner spectrum, no structured emergence. \\
AT-003 & Is static topology sufficient? & No: dynamics outweigh static structure. \\
AT-004 & Does a field synchronize two asymmetric oscillators? & No: field acts as a resonance medium. \\
AT-005 & Do resonance clusters form? & Yes: two clusters of three oscillators, $\sim5\%$ energy participation. \\
AT-006 & Is there a critical resonance density? & Yes: $\rho_c\approx0.09$, percolation-like transition. \\
AT-007 & How many resonance families exist? & Five reported. \\
AT-008 & Which families are reproducible? & Only F4 (74\%, score 0.721); the rest are seed artifacts. \\
\midrule
