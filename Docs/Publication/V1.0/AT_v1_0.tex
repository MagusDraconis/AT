...
% Build: pdflatex AT_v1_0.tex   (twice for references, if any)
...
mathematical}: the metric and the BDG action are imported (proven but not AT-derived),
$G=\ell^2c^3/\hbar$ is dimensional analysis, and no unique sharp prediction yet
discriminates AT from SM$+\Lambda$CDM. See
...
\noindent\textbf{Keywords:} THE Q-MODEL; AT; emergence; quantum foundations; causal set
...
\emph{values} unexplained. THE Q-MODEL (AT) addresses this with a single organizational
...
The dynamical content of AT is the graph Laplacian $L_Q=D-A$ (adjacency $A$, degree $D$):
real symmetric, positive semi-definite, zero row sums. Its eigenvectors form the
\emph{Hilbert space} of the theory. The tight-binding identity (AT-142) is
$(L_Q)_{ij}=2\delta_{ij}-\delta_{i,j+1}-\delta_{i,j-1}$, giving $H=tL_Q$.

\textbf{Schr\"odinger from reversibility (AT-149--151).} For linear evolution
...
unitary curved-space Schr\"odinger equation. (See \texttt{AT.Tests/ResearchXC}.)
...
$SU(3)$ structure are REAL-UNDERIVED (emergent); the color count $n=3$ is DRAWN. AT
...
AT derives the \emph{form} of structure and \emph{classifies} content. It does \emph{not}
