...
THE Q-MODEL (AT) is a theory of structure and content that compresses observable
...
THE Q-MODEL (AT) addresses this with a single organizational claim: **structure is
...
### 2.2 Dynamical layer (quantum postulates, AT-155)
...
The dynamical content of AT is the graph Laplacian and its consequences.
...
**The tight-binding identity (AT-142).** For a 1D chain,
...
**Schrödinger from reversibility (AT-149–151).** For linear evolution
...
**Observables from the spectrum (AT-145).** $m_{\rm eff}=1/\lambda_1$, $E=\mathrm{tr}(L)$,
$\Delta=\lambda_2-\lambda_1$, $\xi=1/\sqrt{\lambda_1}$, $D=\lambda_1$, $C=\log_2 N$.

**Ontology→physics bridge.** $L_Q$ eigenvector basis = Hilbert space (AT-149); the causal
...
**Radial acceleration relation.** AT derives the RAR with **zero free parameters**:
...
**Pantheon+SH0ES.** AT and $\Lambda$CDM are indistinguishable at the current signal
...
| Unified Action | **TRM roadmap only** | $S_{\rm eff}[T,\vec A_T,\Theta]$, not a AT result |
...
**Scope.** AT derives the **form** of structure and **classifies** the content. It does
