...
public static class PredictionPriorityAnalyzer
{
    ...
    static string BuildA()
    {
        ...
        sb.AppendLine("    power, and uniquely distinguishes TQM from MOND and ΛCDM.");
        return sb.ToString();
    }
    ...
    static string BuildB(RankedPrediction[] ranking)
    {
        ...
        sb.AppendLine("    - 0.90 (g†(z)): if g† does NOT evolve as c·H(z)/2π, TQM's");
        sb.AppendLine("      central RAR derivation (DATA-004) is WRONG. Devastating.");
        sb.AppendLine("    - 0.85 (anti-matter): if anti-H falls up, TQM + GR + EP all");
        ...
        sb.AppendLine("    - 0.70 (w(z)): if η=0 exactly, the TQM dark-energy signal");
    ...
    static string BuildD()
    {
        ...
        sb.AppendLine("      collapses. TQM's STRONGEST prediction falsified.");
        ...
        sb.AppendLine("      fails. GR + EP + TQM all collapse.");
        ...
        sb.AppendLine("    - QG-004 (Λ(t) from N(t)) is wrong. Dark energy not from TQM.");
    ...
    static string BuildE(RankedPrediction[] ranking)
    {
        ...
        sb.AppendLine("    - Uniquely distinguishes TQM from MOND (constant g†) and");
    ...
    static string BuildF()
    {
        ...
        sb.AppendLine("    - OUTCOME: confirms/refutes TQM's STRONGEST prediction.");
        sb.AppendLine();
        ...
        sb.AppendLine("    - OUTCOME: tests the TQM Λ(t) form.");
        sb.AppendLine();
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.AppendLine("HOSTILE REVIEW: ASSUME TQM IS WRONG");
        sb.AppendLine();
        sb.AppendLine("  Which experiment reveals TQM is wrong FASTEST?");
        sb.AppendLine();
        sb.AppendLine("  ANSWER: the evolving RAR (g†(z)).");
        sb.AppendLine("    - If TQM's RAR derivation is wrong, g† does NOT evolve as");
        ...
        sb.AppendLine("    TQM's core (Q, Randomness, triple) is UNFALSIFIABLE (they");
        sb.AppendLine("    are primitives). The FALSIFIABLE content is the DERIVED");
        sb.AppendLine("    predictions (g†, anti-matter, w(z), Koide). So TQM is");
    ...
    static string BuildH()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  2. TQM'S FALSIFIABILITY IS REAL BUT CONCENTRATED:");
        ...
        sb.AppendLine("    matter, w(z), Koide). So TQM stands or falls on its derived");
        ...
        sb.AppendLine("    TQM is MODERATELY testable (B): its derived predictions are");
    ...
    static string BuildI()
    {
        ...
        sb.AppendLine("  Q4: TQM depends on: g†(z) (CORE), anti-matter (CORE), neutrino");
        sb.AppendLine("      Koide (refinement), w(z) (QG-004, already partial).");
        sb.AppendLine("  Q5: Confirming g†(z) would STRONGLY validate TQM (a derived");
        sb.AppendLine("      number, uniquely distinguishing from MOND and ΛCDM).");
        sb.AppendLine("  Q6: g†(z) failing would MOST STRONGLY damage TQM (core RAR).");
        ...
        sb.AppendLine();
        sb.AppendLine("    TQM is MODERATELY testable: its DERIVED predictions are");
        ...
        sb.AppendLine("      derived number, uniquely distinguishes TQM from MOND/ΛCDM).");
        sb.AppendLine("    - If funded: this ONE experiment provides the strongest and");
        sb.AppendLine("      fastest test of TQM.");
        sb.AppendLine();
