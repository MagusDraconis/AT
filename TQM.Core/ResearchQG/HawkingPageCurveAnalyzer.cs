...
public static class HawkingPageCurveAnalyzer
{
    public static HPResult RunFullAnalysis()
    {
        ...
        var pc = new[]{new ParaComp("Hawking 1976","NO — never decreases.","N/A","Information DESTROYED.","TQM REJECTS — Q-events preserve info."),
            new ParaComp("Page 1993","YES — turns at t_Page.","NO","Information PRESERVED.","TQM SUPPORTS — natural from Q-events."),
            new ParaComp("Complementarity","AMBIGUOUS — observer-dependent.","NO","PRESERVED (non-local).","PARTIALLY — non-local but no cloning."),
            new ParaComp("Firewall/AMPS","YES but with firewall.","YES","PRESERVED (at cost of structure).","TQM REJECTS — causal boundary = no firewall."),
            new ParaComp("ER = EPR","YES — geometry = entanglement.","NO","PRESERVED (geometric).","TQM SUPPORTS — geometry from Q-event entanglement."),
            new ParaComp("TQM","YES — natural from Q-event entropy.","NO — causal boundary only.","ALWAYS PRESERVED.","THIS FRAMEWORK."),
        };
    ...
    static string BuildB(ThermAnalysis[] t){
        ...
        sb.AppendLine("  Aspect                TQM Mechanism                          Status");
        sb.AppendLine("  --------------------  -------------------------------------  ------");
        foreach(var x in t) sb.AppendLine(string.Format(CultureInfo.InvariantCulture,"  {0,-21} {1,-38} {2}",x.Aspect,x.TqmMechanism,x.Status));
    ...
    static string BuildD(PagePhase[] p){
        ...
        sb.AppendLine("  TQM naturally produces this from Q-event entanglement evolution.");
        return sb.ToString();
    }
    ...
    static string BuildF(ParaComp[] p){
        ...
        sb.AppendLine("  Approach            Page Curve?   Firewall?    Information        TQM Position");
        sb.AppendLine("  ------------------  ------------  -----------  -----------------  -----------");
        foreach(var x in p) sb.AppendLine(string.Format(CultureInfo.InvariantCulture,"  {0,-19} {1,-13} {2,-12} {3,-18} {4}",x.Approach,x.PageCurve,x.Firewall,x.InfoOutcome,x.TqmPosition));
        return sb.ToString();
    }

    static string BuildG()=>"HOSTILE REVIEW\n\n1. THE PAGE CURVE IS NOT DERIVED — IT IS ASSERTED:\n   TQM says 'S_ent rises then falls' but does not derive t_Page ≈ M^3.\n   This is the Page curve from QFT — TQM imports it, not derives it.\n\n2. HAWKING TEMPERATURE IS IMPORTED:\n   T_H = hbar/(8πGM) comes from QFT in curved spacetime.\n   TQM provides the MECHANISM (Q-event pairs) but not the FORMULA.\n\n3. INFORMATION ENCODING IS QUALITATIVE:\n   'Correlations carry information' is true for ALL unitary theories.\n   TQM does not explain HOW the correlations encode specific information\n   (e.g., which Q-events carry which bits).\n\n4. THE PAGE CURVE IS THE STANDARD QM PREDICTION:\n   Any unitary theory produces the Page curve. TQM inherits it from\n   unitarity (which TQM derives from Q-event conservation, QM-002).\n   This is not a TQM prediction — it's a QM prediction.\n\n5. TQM ADDS NO NEW QUANTITATIVE PREDICTIONS:\n   The Page time, information return rate, and entropy evolution\n   are all inherited from standard QM + GR. TQM provides the\n   ONTOLOGICAL FOUNDATION but no new numbers.";
    ...
    static string BuildI(){
        ...
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        sb.AppendLine("  TQM PROVIDES A COMPLETE QUALITATIVE PICTURE of black hole evaporation.");
        sb.AppendLine();
        sb.AppendLine("  WHAT TQM EXPLAINS:");
        ...
        sb.AppendLine();
        sb.AppendLine("  WHAT TQM DOES NOT EXPLAIN (yet):");
        ...
        sb.AppendLine("    TQM provides the ONTOLOGICAL FOUNDATION for why the Page curve");
        sb.AppendLine("    must be true. But the quantitative predictions remain imported");
        sb.AppendLine("    from standard QFT+GR. TQM is an explanation, not a replacement.");
        sb.AppendLine();
        ...
        sb.AppendLine("  remains the defining challenge of the TQM quantum gravity program.");
        return sb.ToString();
    }
}
