...
public static class DimensionlessStructureAnalyzer
{
    public static DimResult RunFullAnalysis()
    {
        var dq = new[]{new DimlessQ("c*tau/l","c*tau/l = 1","1.000","DEFINITION — c = l/tau (QG-010).","NOT A CONSTRAINT — it's the definition of c."),
            ...
            new DimlessQ("l_P/l","l_P/l = 1","1.000","l IS the Planck length in TQM.","IDENTITY — l = l_P by definition."),
            new DimlessQ("tau/t_P","tau/t_P = 1","1.000","tau IS the Planck time in TQM.","IDENTITY — tau = t_P by definition."),
        ...
        var pa = new[]{new PiAppear("g† = c*H0/(2*pi)","RAR/DATA-004 to 007",4,"omega = 2*pi*nu — frequency conversion.","MATHEMATICAL IDENTITY — not coincidence.","2*pi IS THE omega<->nu CONVERSION."),
            ...
            new PiAppear("8*pi in Einstein eqs","QG-001: G_uv = 8*pi*G*T_uv",1,"From Newtonian limit of GR.","INHERITED FROM GR — not TQM-derived.","8*pi = 4*pi * 2 from spherical area."),
        };
    ...
    static string BuildI(){
        ...
        sb.AppendLine();
        sb.AppendLine("  THE DEEPEST TRUTH OF TQM:");
        sb.AppendLine("    TQM provides STRUCTURE — how things relate.");
        sb.AppendLine("    TQM does not provide SCALE — why things have specific values.");
