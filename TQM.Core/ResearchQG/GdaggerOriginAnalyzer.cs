...
public static class GdaggerOriginAnalyzer
{
    ...
    private static string BuildF()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  - g†(z) evolution: TQM/time-scale predict g† ∝ H(z) (rising); MOND a0 is constant.");
        ...
        sb.AppendLine();
        sb.AppendLine("  None of these is yet decisive; they all converge on g† ∝ H (the TQM prediction).");
        return sb.ToString();
    }
    ...
    private static string BuildG(CouplingMechanism[] ranked)
    {
        ...
        sb.AppendLine("            survivors converge on g† ∝ H (TQM's existing, mass-limited test)");
        sb.AppendLine();
