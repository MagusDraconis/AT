...
/// <summary>
/// QG-079 Detectability Limit Audit. Computes the TQM evolution signal amplitude
...
public static class DetectabilityLimitAnalyzer
{
    ...
    public static DetectabilityReport Run(string largeSampleCsv, string outDir)
    {
        ...
        // Signal leverage: TQM prediction deviation about its own sample mean.
        ...
        // Plots.
        PlotSignalVsNoise(Path.Combine(outDir, "TQM_Signal_vs_Noise.png"), zGrid, sigmaObs, IntrinsicRARScatterDex);
    ...
    private static string BuildA(double[] zGrid)
    {
        ...
        sb.AppendLine("TQM evolution signal amplitude (Δlog g† = ½ log₁₀[Ωm(1+z)³+ΩΛ]).");
        sb.AppendLine();
        ...
        foreach (double z in zGrid)
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
                ...
                RARPhysics.LogGdaggerTqm(z) - Math.Log10(RARPhysics.GdaggerLocal())));
        sb.AppendLine();
    ...
    private static string BuildB(double sigmaObs, double intrinsic, double stdDelta, double snrObs, int n)
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"  TQM signal leverage (std of Δlog g† across sample): {stdDelta:F3} dex");
    ...
    private static string BuildF(double snrObs, double stdDelta, double intrinsic)
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"  TQM signal leverage = {stdDelta:F3} dex vs intrinsic RAR scatter {intrinsic:F2} dex");
        sb.AppendLine($"  (ratio {stdDelta / intrinsic:F1}×). Current observed S/N = {snrObs:F2} (marginal 2σ).");
        sb.AppendLine();
        sb.AppendLine("  CENTRAL QUESTION ANSWERED: the TQM prediction is NOT fundamentally too small —");
    ...
    private static void PlotSignalVsNoise(string path, double[] zGrid, double sigmaObs, double intrinsic)
    {
        ...
        for (int i = 0; i < m; i++)
        {
            ...
            d[i] = RARPhysics.LogGdaggerTqm(zs[i]) - Math.Log10(RARPhysics.GdaggerLocal());
        }
