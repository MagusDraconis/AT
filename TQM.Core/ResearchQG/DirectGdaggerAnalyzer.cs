...
/// stacked in redshift bins, compared against TQM (g† ∝ H(z)) vs MOND (constant).
/// </summary>
public static class DirectGdaggerAnalyzer
{
    ...
    // Local acceleration scale (TQM): c*H0/(2pi).
    ...
    // ---------------------------------------------------------------------
    // Per-galaxy g† fit (log-space grid search, TQM RAR form)
    // ---------------------------------------------------------------------
    ...
    private static TheoryComparison[] CompareModels(RARFit[] fits)
    {
        ...
        int n = d.Length;
        double chi2Tqm = 0, chi2Mond = 0, chi2Null = 0;
        foreach (var f in d)
        {
