...
public static class CouplingConstantAnalyzer
{
    ...
    static Coupling[] BuildCouplings()
    {
        return new Coupling[]
        {
            new Coupling("Fine structure (alpha_EM)",1.0/137.036,"e^2/(4*pi*eps0*hbar*c) = g^2/(4*pi)",
                "TQM: g = charge quantum = coupling of phase winding (n) to the U(1) gauge field. alpha_EM = g^2/4pi. The winding-coupling g is the elementary charge ratio.",
                "NO. The value 1/137.036 is EMPIRICAL. TQM explains WHAT alpha is (winding-gauge coupling) but not WHY 1/137.",
                "A: Empirical. The deepest mystery of physics (why 1/137?)."),
            new Coupling("Strong (alpha_s @ MZ)",0.118,"g_s^2/(4*pi)",
                "TQM: g_s = coupling of tri-winding confinement (n=3 color structure, QG-038). Strong force binds the 3 vortex substructures of hadrons.",
                "NO. alpha_s = 0.118 is empirical. Its RUNNING (asymptotic freedom) is a QCD property not derived from TQM.",
                "A: Empirical. Confinement and running not derived."),
            new Coupling("Weak (alpha_W @ MZ)",1.0/30.0,"g^2/(4*pi)",
                "TQM: g = coupling of binary sector transitions (n<->-n, QG-038 SU(2)). Weak force = flavor-changing phase transitions.",
                "NO. alpha_W = 1/30 empirical. Electroweak unification (alpha_W ~ alpha_EM/sin^2(theta_W)) is SM structure, not TQM-derived.",
                "A: Empirical. sin^2(theta_W) = 0.231 not derived."),
            new Coupling("Yukawa (top, y_t)",0.99,"m_t*sqrt(2)/v",
                "TQM: y_t = overlap integral between top-quark architecture and the amplitude mode (Higgs, QG-037). Measures how strongly the top's frequency architecture 'feels' the VEV.",
            ...
            new Coupling("Higgs self (lambda)",0.13,"m_H^2/(2*v^2)",
                "TQM: lambda = amplitude stiffness (curvature of Mexican hat at VEV, QG-040). Sets Higgs frequency.",
            ...
            new Coupling("Theta_QCD",0.0,"CP-violating term coefficient",
                "TQM: theta_QCD would violate CP in strong sector. Measured |theta| < 10^-10 (neutron EDM).",
                "NO. Why theta ~ 0 (strong CP problem) is unexplained by TQM and SM.",
    ...
    static NumerAttempt[] BuildNumerologies()
    {
        return new NumerAttempt[]
        {
            ...
            new NumerAttempt("TQM (this work)","2026","alpha = g^2/4pi where g = winding-gauge coupling","~1/137","HONEST: TQM does NOT derive the value. g remains a free parameter. No numerology attempted."),
        };
    }
    ...
    static string BuildB()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  WHAT alpha IS (TQM interpretation):");
        sb.AppendLine("    alpha = g^2/(4*pi), where g = dimensionless charge.");
        sb.AppendLine("    In TQM (QG-035): charge Q = g·n, n = winding number.");
        ...
        sb.AppendLine();
        sb.AppendLine("  WHY alpha IS DIMENSIONLESS (TQM answer):");
        ...
        sb.AppendLine("    TQM does NOT derive the value of g.");
        sb.AppendLine("    The winding-gauge coupling strength is a FREE PARAMETER.");
        sb.AppendLine("    No known theory derives 1/137 (SM, GUT, string, LQG, TQM).");
        sb.AppendLine();
        ...
        sb.AppendLine("    TQM's honesty: it remains a mystery in TQM too.");
        return sb.ToString();
    }
    ...
    static string BuildC()
    {
        ...
        sb.AppendLine("    beta function, NOT from TQM.");
        sb.AppendLine();
        sb.AppendLine("  TQM CORRESPONDENCE (weak):");
        sb.AppendLine("    Strong force = tri-winding confinement (QG-038).");
        sb.AppendLine("    alpha_s = coupling of the 3 vortex substructures.");
        sb.AppendLine("    But TQM does NOT derive alpha_s or its running.");
        sb.AppendLine("    The beta function (asymptotic freedom) comes from SU(3)");
        sb.AppendLine("    gauge theory — an external input to TQM.");
        sb.AppendLine();
        ...
        sb.AppendLine("    The Weinberg angle is NOT derived (SM or TQM).");
        sb.AppendLine();
        sb.AppendLine("  VERDICT: alpha_s and alpha_W (and theta_W) are empirical.");
        sb.AppendLine("  TQM provides qualitative mappings (tri-winding = strong,");
    ...
    static string BuildD()
    {
        ...
        sb.AppendLine("    - The OVERLAP INTEGRALS are not computed. TQM cannot");
        sb.AppendLine("      calculate the specific architecture shapes.");
        sb.AppendLine();
        sb.AppendLine("  THE YUKAWA HIERARCHY REMAINS UNEXPLAINED");
        sb.AppendLine("  by both the SM and TQM. It is the single largest");
    ...
    static string BuildE()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  TQM PERSPECTIVE:");
        sb.AppendLine("    TQM does NOT explain unification either.");
        ...
        sb.AppendLine("    - No derivation of alpha_GUT from TQM.");
        ...
        sb.AppendLine("  but TQM does not yet derive the unified coupling.");
        return sb.ToString();
    }
    ...
    static string BuildF(NumerAttempt[] num)
    {
        ...
        sb.AppendLine("    TQM offers no decisive evidence either way.");
        sb.AppendLine();
        sb.AppendLine("  TQM's POSITION (honest):");
        sb.AppendLine("    TQM does NOT attempt numerology.");
    ...
    static string BuildG()
    {
        ...
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        sb.AppendLine("  Q1: Coupling constant in TQM = strength of interaction between");
        ...
        sb.AppendLine("      (TQM explains dimensionlessness, not values.)");
        ...
        sb.AppendLine("    TQM: ~19 free parameters (NO reduction in couplings).");
        sb.AppendLine("    TQM's parameter reduction is ONTOLOGICAL (primitives),");
        ...
        sb.AppendLine("    TQM does NOT derive any coupling constant.");
        sb.AppendLine("    alpha = 1/137 remains as mysterious in TQM as in");
        sb.AppendLine("    the Standard Model. This is the largest numerical");
        sb.AppendLine("    gap in the entire TQM program.");
        sb.AppendLine();
