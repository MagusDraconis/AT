...
public static class CausalitySpeedAnalyzer
{
    ...
    static string BuildA(Cmeaning[] c){
        ...
        sb.AppendLine("WHAT IS c IN TQM?");sb.AppendLine();
        sb.AppendLine("  Aspect               Standard View              TQM View");
        sb.AppendLine("  -------------------  -------------------------  -------------------------");
        foreach(var x in c) sb.AppendLine(string.Format(CultureInfo.InvariantCulture,"  {0,-20} {1,-26} {2}",x.Aspect,x.StandardView,x.TqmView));
    ...
    static string BuildI(){
        ...
        sb.AppendLine("    TQM: c, τ, ħ (3 fundamental constants). UNCHANGED.");
        ...
        sb.AppendLine("    Then TQM would have FEWER parameters than standard physics.");
