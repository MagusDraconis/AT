...
public static class PredictionInventoryAnalyzer
{
    ...
    static Prediction[] BuildPredictions()
    {
        return new Prediction[]
        {
            ...
            new Prediction("TQM ≈ QM experimentally","QM-005","no deviation at current precision","CONSISTENT (equivalence audit)","EQUIVALENCE (not prediction)"),
        };
    }
    ...
    static Negative[] BuildNegatives()
    {
        return new Negative[]
        {
            new Negative("Anti-gravity (matter falls up)","QG-029/035","FORBIDDEN: T_μν ~ (∂θ)² sign-blind. If observed, TQM falsified."),
    ...
    static string BuildA()
    {
        ...
        sb.AppendLine("    - COMPATIBILITIES: TQM ≈ QM, TQM ≈ GR (just match).");
        ...
        sb.AppendLine("    TQM has ~5 GENUINE predictions (2 confirmed), ~5 negative");
    ...
    static string BuildB()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  TQM DERIVED a large amount of STRUCTURE (QG-065):");
        ...
        sb.AppendLine("    content of TQM — real progress, but not prediction.");
        sb.AppendLine();
        sb.AppendLine("  VERDICT: TQM derives LOTS of structure (strong ontology),");
    ...
    static string BuildC()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  TQM REINTERPRETS many known phenomena:");
        ...
        sb.AppendLine();
        sb.AppendLine("  VERDICT: The reinterpretations are TQM's CORE VALUE (ontology),");
    ...
    static string BuildD(Prediction[] preds)
    {
        ...
        sb.AppendLine("       This is TQM's STRONGEST prediction — it DERIVED a number.");
    ...
    static string BuildE(Negative[] negs)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  TQM PROHIBITS several phenomena (falsifiable):");
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
        ...
        sb.AppendLine("    would FALSIFY TQM. The anti-gravity prohibition was already");
        ...
        sb.AppendLine("  data. They give TQM real falsification risk.");
        return sb.ToString();
    }
    ...
    static string BuildF()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  Experiments that could falsify TQM DIRECTLY:");
        sb.AppendLine();
        sb.AppendLine("  1. ANTI-MATTER FREE-FALL (GBAR/AEGIS, ongoing):");
        sb.AppendLine("     If anti-hydrogen falls UP, TQM is FALSIFIED (QG-035).");
        ...
        sb.AppendLine("     Deviation from constant g† would falsify MOND, test TQM.");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("  VERDICT: TQM has ~4 DIRECT falsification tests, 2 already");
    ...
    static string BuildH(Scorecard[] score)
    {
        ...
        sb.AppendLine("    TQM is EXPLANATORY (strong ontology) and MODERATELY");
    ...
    static string BuildI()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine("  TQM IS EXPLANATORY + MODERATELY PREDICTIVE (HONEST SCORECARD)");
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        ...
        sb.AppendLine("  Q7: Contradicted: NONE (no observation contradicts TQM).");
        ...
        sb.AppendLine();
        sb.AppendLine("    TQM's GENUINE predictions:");
        ...
        sb.AppendLine();
        sb.AppendLine("    TQM's NON-predictions (honestly excluded):");
        ...
        sb.AppendLine("    TQM is a STRONG ONTOLOGY (explains WHY) with MODERATE");
