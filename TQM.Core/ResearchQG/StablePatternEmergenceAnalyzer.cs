...
public static class StablePatternEmergenceAnalyzer
{
    public static SPResult RunFullAnalysis()
    {
        var ns = new[]{new NoiseVsStruct("Randomness ONLY (no Q, no M^2)","WHITE NOISE — no patterns, no persistence.","Randomness alone = uncorrelated events. No structure.","MATTER CANNOT EXIST."),
            ...
            new NoiseVsStruct("Q + Randomness + M^2","STABLE PATTERNS EMERGE.","M^2 (nonlinearity) creates attractors. Q provides structure.","THIS IS TQM — the minimal set for matter."),
        };
        ...
        var am = new[]{new AttractorMech("Q individuation","Distinct Q-events with causal relations.","Q primitive.","FOUNDATION — without Q, no structure."),
            ...
            new AttractorMech("Topological protection","Defects with non-zero winding number are topologically stable.","M^2 + Q-event field topology.","PARTICLE STABILITY — winding number conserved (TQM-113-122)."),
    ...
    static string BuildI(){
        ...
        sb.AppendLine();
        sb.AppendLine("  TQM identifies WHY matter exists but does not COMPUTE");
        sb.AppendLine("  specific particle properties — those come from standard physics.");
        sb.AppendLine("  TQM provides the ONTOLOGICAL FOUNDATION, not the computational");
