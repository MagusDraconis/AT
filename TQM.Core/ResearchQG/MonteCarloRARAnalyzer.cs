...
public static class MonteCarloRARAnalyzer
{
    ...
    /// <summary>Analytic discrimination signal-to-noise² between TQM (rising) and the
    /// best-fit constant (MOND) at a given gas precision: Σ (TQM_pred − <TQM_pred>w)²/σ_i².</summary>
    public static double Snr2(GalaxyGasSystematics[] gals, double sigmaGasDex)
    {
        ...
        foreach (var g in gals)
        {
            ...
            double wi = 1.0 / (s * s);
            sumW += wi;
            sw += wi * RARPhysics.LogGdaggerTqm(g.Z);
        }
        double mean = sw / sumW;
        double snr2 = 0;
        foreach (var g in gals)
        {
            double s = TotalSigma(g, sigmaGasDex);
            double wi = 1.0 / (s * s);
            double d = RARPhysics.LogGdaggerTqm(g.Z) - mean;
            snr2 += wi * d * d;
        }
        return snr2;
    }
