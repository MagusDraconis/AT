...
public static class PhaseGradientGravityAnalyzer
{
    public static GR22Result RunFullAnalysis()
    {
        ...
        var mp = new[]{new MassPh("Mass = energy/oscillation","E = hbar*omega. Mass = E/c^2.","Mass IS oscillation energy (QM-002 + QM-001).","Mass is a fundamental property.","OSCILLATION GROUNDS MASS."),
            new MassPh("Mass curves spacetime","Energy-momentum -> curvature via GR.","Oscillation density concentrates Q-events -> curvature.","G_uv = 8*pi*G*T_uv assumes T_uv.","TQM EXPLAINS T_uv from oscillation."),
            new MassPh("Inertial mass","Resistance to acceleration. m*a = F.","Resistance to phase change. Phase defects resist displacement.","Inertial = gravitational mass (equivalence principle).","TQM: both from phase structure."),
            new MassPh("Gravitational mass","Source of gravity. Active gravitational mass.","Phase gradients create causal density -> curvature.","G_uv = 8*pi*G*T_uv.","TQM: gravity from phase gradients."),
        };
    ...
    static string BuildI(){
        ...
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        sb.AppendLine("  GRAVITY IS A PHASE-GRADIENT PHENOMENON IN TQM.");
        sb.AppendLine();
