...
public static class NegativePhaseAnalyzer
{
    ...
    static PhaseStructure[] BuildStructures()
    {
        return new PhaseStructure[]
        {
            ...
            new PhaseStructure("Particle as winding","∮∇θ·dl = 2π (confined)","Particle = compact region with internal phase winding. Winding number = topological charge = particle identity.",
                "STABLE: topological protection + architectural binding. This IS what particles ARE in TQM (QG-027/028).","ATTRACTIVE: net positive ∇θ in far field. Mass = winding energy.","C→D: SPECULATIVE BUT COMPELLING. Particle = trapped phase winding. Explains stability and identity."),
            new PhaseStructure("Skyrmion (3D winding)","π₃(S²)=Z","3D topological texture. Phase wraps around internal S². Topological charge = baryon number candidate.",
                "STABLE: homotopy protection. π₃(S²)=Z → integer conserved charge.","ATTRACTIVE: net positive phase gradient.","C: STABLE ARCHITECTURE. Candidate for baryons (Skyrme model, 1961). TQM natural host."),
    ...
    static string BuildC(PhaseStructure[] structures)
    {
        ...
        sb.AppendLine("    Core size ξ ~ ℓ (Planck, in TQM).");
        ...
        sb.AppendLine();
        sb.AppendLine("  VERDICT: Phase winding is THE stable architecture in TQM.");
    ...
    static string BuildE()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  TQM phase structures form a hierarchy:");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("  THE REAL 'NEGATIVE SECTOR' OF TQM:");
    ...
    static string BuildF(PhaseStructure[] structures)
    {
        ...
        sb.AppendLine("  Q10: YES — Phase winding is the true 'negative sector' of TQM.");
        sb.AppendLine();
