...
public static class BlackHoleEmergenceAnalyzer
{
    public static BHResult RunFullAnalysis()
    {
        ...
        var eh = new[]{new EntHorizon("Horizon entanglement","Q-events across horizon are entangled (QM-003).","S_ent = (Area)/(4ℓ²). Bekenstein-Hawking.","CONSISTENT — matches GR prediction."),
            new EntHorizon("Firewall?","If horizon Q-events disentangle → firewall.","NO firewall in TQM: entanglement is fundamental.","RESOLVED — Q-events maintain entanglement."),
            new EntHorizon("ER = EPR?","Einstein-Rosen bridge = entangled Q-event pairs.","Geometry from entanglement (QG-001).","SUPPORTED — TQM naturally has ER = EPR."),
        };
        ...
        var pr = new[]{new ParaResolution("Hawking (original)","Information destroyed.","TQM rejects: Q-events cannot be destroyed.","REJECTED — Q-event substrate preserves information."),
            new ParaResolution("Complementarity","Information at horizon AND inside (no-cloning).","TQM supports: Q-event correlations are non-local.","PARTIALLY — non-local but no cloning."),
            new ParaResolution("Firewall","Horizon is high-energy barrier → burns observer.","TQM rejects: horizon is causal boundary, not physical.","REJECTED — causal boundary has no firewall."),
            new ParaResolution("Fuzzball","String-theory microstates at horizon.","TQM similar: Q-event microstates at horizon.","SIMILAR — but Q-events replace strings."),
            new ParaResolution("TQM","Information = Q-event correlations. Never destroyed.","THIS IS THE TQM RESOLUTION.","RESOLVED — information = Q-event entanglement."),
        };
    ...
    static string BuildA(HorizStep[] h){
        ...
        sb.AppendLine("BLACK HOLE IN TQM");sb.AppendLine();
    ...
    static string BuildD(InfoFlow[] i){
        ...
        sb.AppendLine("  Aspect                   TQM Mechanism                    Outcome");
        sb.AppendLine("  -----------------------  -------------------------------  -------");
        foreach(var x in i) sb.AppendLine(string.Format(CultureInfo.InvariantCulture,"  {0,-24} {1,-32} {2}",x.Aspect,x.TqmMechanism,x.Outcome));
        return sb.ToString();
    }
    ...
    static string BuildI(){
        ...
        sb.AppendLine("  Q9:    Information PARADOX RESOLVED in TQM:");
        ...
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        sb.AppendLine("  TQM RESOLVES THE BLACK HOLE INFORMATION PARADOX.");
        sb.AppendLine();
        ...
        sb.AppendLine("  it's a consequence of what TQM IS: Q-events are information.");
