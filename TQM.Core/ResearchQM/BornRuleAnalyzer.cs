...
public static class BornRuleAnalyzer
{
    public static BornResult RunFullAnalysis()
    {
        ...
        var bc = new[]{new BornCandidate("Frequency convergence","Repeated Q-events produce frequencies → P in large-N limit.","Q-events + law of large numbers.",2,"MODERATE","Derives P as frequency limit. Does NOT specify WHICH measure."),
            ...
            new BornCandidate("Defect counting","Q-event actualization rate ∝ defect density ∝ |ψ|^2.","Defect dynamics + M^2.",3,"WEAK — heuristic.","Plausible TQM mechanism. Not derived."),
        ...
        var aa = new[]{new AssumptionAudit("Random actualization","YES","NO","YES","PRIMITIVE — cannot be derived."),
            ...
            new AssumptionAudit("Hilbert space structure","NO","PARTIAL","PARTIAL","OPEN — TQM must derive."),
    ...
    static string BuildA(ProbSource[] p){
        ...
        sb.AppendLine("PROBABILITY IN TQM");sb.AppendLine();
    ...
    static string BuildC(FreqModel[] f){
        ...
        sb.AppendLine("  TQM provides the mechanism (random actualization). The question is WHICH measure.");
        return sb.ToString();
    }
    ...
    static string BuildE(ExpConstraint[] e){
        ...
        sb.AppendLine("  Any TQM deviation must be below this threshold.");
        return sb.ToString();
    }
    ...
    static string BuildI(BornCandidate[] b){
        ...
        sb.AppendLine("         TQM must predict deviations below this level.");
        sb.AppendLine();
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine("  OVERALL VERDICT");
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        sb.AppendLine("  THE BORN RULE IS NOT YET FULLY DERIVED IN TQM.");
        sb.AppendLine();
        sb.AppendLine("  WHAT TQM CONTRIBUTES:");
        ...
        sb.AppendLine("  TQM: Born emerges from Q-events + field structure + Gleason.");
