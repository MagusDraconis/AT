...
/// <summary>
/// Audits the origin of the 2π factor and RAR scatter within TQM.
...
public static class RarScatterAnalyzer
{
    ...
    public static PiFactorAudit AuditPiFactor()
    {
        var candidates = new List<PiFactorCandidate>
        {
            ...
            new("De Sitter horizon circumference",
                ...
                2.0 * Math.PI,
                false, true, 3,
                "MODERATE: Horizon thermodynamics naturally yields 2π. Consistent with TQM's causal set horizon structure."),
            ...
            new("Defect winding number",
                ...
                2.0 * Math.PI,
                true, false, 4,
                "STRONG: Topological charge quantization intrinsically contains 2π. TQM defects naturally have this factor."),
        ...
        string synAnswer =
            "The factor 2π appears in TQM through THREE independent channels:\n" +
        ...
        string verdict = best.StrengthScore >= 4
            ? $"2π ORIGIN ESTABLISHED. Best candidate: '{best.Origin}' (score {best.StrengthScore}/5). " +
              "The factor is a MATHEMATICAL INEVITABILITY in TQM's causal geometry."
    ...
    public static VarianceModel PropagateVariance(double observed)
    {
        ...
        sb.AppendLine("  TQM contribution ~0.01 dex. Bulk from astrophysics (M/L, env).");
    ...
    public static RarScatterResult RunFullAnalysis(string dataPath)
    {
        ...
        sbG.AppendLine("     with circles has 2π. Uniqueness to TQM is unproven.");
        sbG.AppendLine("  2. Defect Poisson noise: N_def~100 is UNCONSTRAINED — free parameter.");
        sbG.AppendLine("  3. M/L variations: Shared with ΛCDM, not unique to TQM.");
        ...
        sbH.AppendLine("  [4] M/L ratios — astrophysics, not TQM-specific.");
        sbH.AppendLine("  [5] Non-circular motions — observational noise beyond TQM scope.");
        sbH.AppendLine("  [6] Environmental dependence — cluster/field differences not modeled.");
        sbH.AppendLine();
        sbH.AppendLine("  FUNDAMENTAL gaps: [1],[2],[3] — require TQM extension.");
        sbH.AppendLine("  ASTROPHYSICAL gaps: [4],[5],[6] — not TQM-specific.");
        ...
        sbI.AppendLine("         MATHEMATICALLY INEVITABLE in TQM's compact dimensions.");
        sbI.AppendLine();
        ...
        sbI.AppendLine("  TQM explains: SCALE ✓ FORM ✓ 2π ✓ LIMITS ✓");
        sbI.AppendLine("  TQM partially explains: SCATTER (~60%)");
        sbI.AppendLine("  TQM doesn't yet: ISOTHERMAL PROFILE from Q-events, DEFECT COUNT");
