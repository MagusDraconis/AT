...
public static class RarNovelPredictionAnalyzer
{
    ...
    public static NovelResult RunFullAnalysis()
    {
        ...
        var ev = new RedshiftEvolution(pts,"g†(z) INCREASES — unique to TQM.","a₀ CONSTANT — no evolution. Critical discriminant.","No theoretical prediction for g†(z).");
    ...
    static string BuildA(NovelPrediction[] p){
        ...
        foreach(var x in p) sb.AppendLine(string.Format(CultureInfo.InvariantCulture,"  [{0}] {1}: {2}",x.Status,x.Aspect,x.Tqm));
    ...
    static string BuildC(ScatterForecast[] s){
        ...
        sb.AppendLine();sb.AppendLine("  TQM: σ(z) ~ 1/sqrt(V(z)): fewer defects → higher Poisson noise.");
        return sb.ToString();
    }
    ...
    static string BuildD(GalaxyTypePred[] g){
        ...
        sb.AppendLine("  Type              σ_TQM   g†       Distinctive?  Mechanism");
        sb.AppendLine("  ----------------  ------  -------  ------------  ---------");
        foreach(var x in g) sb.AppendLine(string.Format(CultureInfo.InvariantCulture,"  {0,-17} {1,-7} {2,-8} {3,-13} {4}",x.Type,x.Scatter,x.Gdagger,x.Distinctive,x.TqmMech));
        return sb.ToString();
    }
    ...
    static string BuildI(NovelPrediction[] p,RedshiftEvolution e,FailCond[] f){
        ...
        sb.AppendLine("    TQM: g†(z)=c·H(z)/(2π) — INCREASES with redshift.");
        ...
        sb.AppendLine("    TQM: g† modulated by environment. MOND: Universal.");
        sb.AppendLine();
        sb.AppendLine("  #3 MEDIUM: Scatter(z) evolution");
        sb.AppendLine("    TQM: σ_RAR increases ~1/sqrt(V(z)).");
        sb.AppendLine();
        sb.AppendLine("  WHAT WOULD FALSIFY TQM FASTEST?");
        ...
        sb.AppendLine("  TQM makes at least ONE unique, testable, falsifiable prediction");
