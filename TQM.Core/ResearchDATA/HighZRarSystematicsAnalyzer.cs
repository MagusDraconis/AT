...
public static class HighZRarSystematicsAnalyzer
{
    ...
    public static SysResult RunFullAnalysis()
    {
        ...
        var rs = new[]{new RobustScore("Beam smearing correctable",7,10,"Forward modeling reduces bias to <0.05 dex."),
            ...
            new RobustScore("Overall robustness",6.7,10,"TQM signal survives IF systematics corrected. NOT guaranteed."),
        };
    ...
    static string BuildF(FalsePosResult[] f){
        ...
        sb.AppendLine("  Inject TRUE TQM g†(z), apply systematics, attempt recovery.");
        sb.AppendLine();
        double tqmDelta=Gdagger(2.0)-g0, sysBias=0.20;
        double snr=tqmDelta/Math.Sqrt(sysBias*sysBias+0.04);
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,"  TQM signal at z=2:   +{0:F3} x1e-10 (Δg†/g0 = {1:P0})",tqmDelta,tqmDelta/g0));
        ...
        sb.AppendLine();
        sb.AppendLine("  z<1.0:  TQM signal RECOVERABLE. Systematics < signal.");
        sb.AppendLine("  z=1-2:  TQM signal MARGINALLY recoverable. Need corrections.");
        sb.AppendLine("  z>2:    TQM signal DOMINATED by systematics. Not recoverable.");
        return sb.ToString();
    }
    ...
    static string BuildI(FalsePosResult[] f,RobustScore[] r){
        ...
        sb.AppendLine("  The TQM prediction is NOT an artifact of systematics —");
