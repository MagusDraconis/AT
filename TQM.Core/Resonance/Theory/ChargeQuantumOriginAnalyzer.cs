...
1. WHAT WE KNOW:
   ...
   TQM-121 proved Q ∈ ℕ (quantized). But it didn't explain
   ...
   Why is there no Q = 0.5? → TQM-121: fractional β₀ impossible.
   Why is Q = 1 the MINIMAL non-zero value? → TQM-122: THIS question.
...
3. THE MINIMALITY THEOREM:
   ...
   PROOF SKETCH:
   (a) Q must be integer (TQM-121) → Q ∈ {0,1,2,...}
...
";
        ...
        MinimalChargeStructure.QuantumOriginReport report)
    {
        ...
        sb.AppendLine("  YES. Energy E[Q] = E₀ + Q·ΔE is linear in Q. Q=0 has " +
                      ...
                      $"corresponds to the nucleation barrier (TQM-118).");
        sb.AppendLine();
    ...
    public static Dictionary<string, string> ValidateAgainstPriorExperiments()
    {
        ...
        {
            ["TQM-113"] = "Q = #{R>0.5 domains} is the condensate count. TQM-122 " +
                          "explains WHY this is the minimal definition: β₀=1 is the " +
                          "smallest non-zero Betti number. No finer definition exists.",

            ["TQM-115"] = "Q=1 plateau spans T∈[0.10,0.85]. TQM-122 explains: the " +
            ...
            ["TQM-120"] = "Q=1 is indivisible. TQM-122 explains WHY: sub-Q structures " +
                          "would require width < w_c → unstable. Incompressibility " +
                          "of the condensate enforces indivisibility.",

            ["TQM-121"] = "Q ∈ ℕ is quantized. TQM-122 completes the story: not only " +
