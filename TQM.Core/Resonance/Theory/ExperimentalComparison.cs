...
/// against known results. Tests whether TQM works beyond its construction set.
///
...
public static class ExperimentalComparison
{
    public static List<PredictionCandidate.ExternalPrediction> RunExternalTests()
    {
        ...
        // ══════════════════════════════════════════════════════════════
        // SYSTEMS WHERE TQM SHOULD WORK (graph Laplacian governs dynamics)
        // ══════════════════════════════════════════════════════════════

        // 1. 1D Coupled Harmonic Oscillators
        // TQM: λ_k = 2-2cos(πk/(N+1)) → ω_k ∝ sin(πk/(2(N+1)))
        ...
        // TQM: E_k = -2t·cos(ka) with k=πn/(N+1)
        ...
        tests.Add(new PredictionCandidate.ExternalPrediction(
            ...
            false, "TQM fails — Heisenberg gap ∝ 1/N"));

        // ══════════════════════════════════════════════════════════════
        // SYSTEMS WHERE TQM FAILS (different physics)
        // ══════════════════════════════════════════════════════════════

        // 7. Percolation cluster — conductivity
        // TQM: predicts nothing (graph Laplacian doesn't capture percolation)
        tests.Add(new PredictionCandidate.ExternalPrediction(
            "2D Percolation (p=p_c)", "Conductivity exponent t",
            "Not predictable from L_Q", "t ≈ 1.3 (numerical)",
            false, "TQM has no prediction — outside graph Laplacian domain"));
        ...
        tests.Add(new PredictionCandidate.ExternalPrediction(
            ...
            false, "TQM predicts uniform case only"));
