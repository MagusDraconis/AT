...
1. THE HYPOTHESIS:
   ...
   Information species (TQM-133/138/139) are NOT arbitrary attractors.
...
3. SPECTRAL PREDICTIONS:
   ...
   b) 5-6 spectral families = 5 graph components (TQM-139)
   c) Low-k modes (k<2) = hub species (TQM-139)
   d) High-k modes (k≥6) = bottleneck species (TQM-139)
...
";
    ...
    public static ThetaEigenmode.SpectralLandscapeReport Analyze()
    {
        ...
        // Step 3: Get TQM-139 attractors for comparison.
        var (tqm139Basins, tqm139Transitions, _, _) = AttractorGraph.MapLandscape(500);

        // Step 4: Map species to modes.
        var mappings = SpectralSpeciesMap.MapSpeciesToModes(tqm139Basins, eigenmodes);

        // Step 5: Get TQM-139 graph for comparison.
        var tqm139Graph = AttractorGraph.ComputeTopology(
            tqm139Basins, tqm139Transitions);

        // Step 6: Compare spectral predictions with TQM-139.
        var (predictedCount, familiesMatch, hubsMatch, bottlenecksMatch) =
            SpectralSpeciesMap.CompareWithTQM139(
                eigenmodes, families, mappings, tqm139Graph);
        ...
        bool spectralConfirmed = mappedSpecies >= 5
            && meanOverlap > 0.3
            && Math.Abs(predictedCount - tqm139Basins.Count) <= 5;
        ...
        string verdict = spectralConfirmed
            ? $"SPECTRAL ORIGIN CONFIRMED. {totalModes} eigenmodes computed, "
              ...
              + $"{mappedSpecies}/{tqm139Basins.Count} TQM-139 species mapped to eigenmodes "
              + $"(mean overlap {meanOverlap:F3}). "
              + $"Predicted species count: {predictedCount} (analytic: {analyticCount}). "
              + $"TQM-139 observed: {tqm139Basins.Count}. "
    ...
    public static string HostileReview(ThetaEigenmode.SpectralLandscapeReport report)
    {
        ...
        sb.AppendLine("  → TQM-133 species include uniform, standing wave, anti-phase.");
        ...
        sb.AppendLine($"  → TQM-138 observed: ~19 species (15 unique)");
