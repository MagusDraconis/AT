...
/// that explain all major TQM findings (TQM-044 through TQM-082).
///
/// TQM-083: Minimal TQM Physics
/// </summary>
public static class TheoryCompressionAnalyzer
{
    ...
    // ══════════════════════════════════════════════════════════════════
    // Known facts and causal chains from TQM-044 to TQM-082
    // ══════════════════════════════════════════════════════════════════

    public static readonly string[] KnownCausalChains =
    {
        "Memory(β) → Curvature (TQM-059, r=0.932) [Curvature does NOT drive motion, TQM-068]",
        "Memory(β) is EXTERNAL, not emergent (TQM-061)",
        "No memory-curvature feedback (TQM-060)",
        "Identity ⟂ Energy (TQM-047, r=0.06) — independent dimensions",
        "Identity survives ±25% energy band (TQM-048), fully recoverable (TQM-049)",
        "Identity does NOT transfer (TQM-050) — identity exclusion",
        "Coherence IS conserved (TQM-052), emergent consequence not causal root (TQM-053)",
        "Single continuous attractor landscape (TQM-056)",
        "Near-geodesic recovery trajectories (TQM-057, 89.4% repeatability)",
        "Curvature exists (TQM-058), memory-generated (TQM-059)",
        "F_net = Alignment × ⟨f⟩ (TQM-074, R²=0.989) — universal force law",
        "Alignment ≈ R² (TQM-075, R²=0.942) — zero-parameter model",
        "dR/dt = f(R, M) (TQM-081, R²=0.758) — M is dominant topology descriptor",
        "M compresses 97.7% of topology info (TQM-081)",
        "dM/dt = f(M, R, M², R², MR) (TQM-082, Adj R²=0.299) — M is effective field",
    ...
    {
        // Generate topology ensemble (reuse TQM-081 approach).
    ...
    public static List<PhysicsCandidate> GenerateCandidates()
    {
        ...
        candidates.Add(new PhysicsCandidate(
            ...
            "Expected: REDUNDANT (S ∝ M, TQM-081 corr>0.99)."));
        ...
        candidates.Add(new PhysicsCandidate(
            ...
            "Expected: β is EXTERNAL parameter, not state (TQM-061)."));
