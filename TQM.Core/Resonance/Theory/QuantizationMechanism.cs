...
1. THE MYSTERY:
   ...
   TQM-120 proved Q is indivisible (fundamental charge quantum = +1).
   TQM-116 proved Q is conserved under PDE evolution.
...
3. THE COMPLETE PROOF:
   ...
   THEOREM: Under the TQM PDE ∂R/∂t = c₀·M·R·(1−R²) + D_R·∇²R
   ...
   somewhere (finite-N fluctuation, TQM-118).
...
5. UNIVERSALITY:
   ...
   is created (TQM-118, TQM-119), not THAT it is quantized.
";
    }
    ...
    public static string ResearchQuestions(ChargeSectorProfile.QuantizationReport report)
    {
        ...
        sb.AppendLine("  YES. The quantization mechanism depends only on the PDE structure " +
                      ...
                      "created (TQM-118, TQM-119), not the fact that it is quantized. " +
    ...
    public static Dictionary<string, string> ValidateAgainstPriorExperiments()
    {
        ...
        {
            ["TQM-113"] = "Topological charge identification: Q = #{R>0.5 domains}. " +
                          "TQM-121 explains WHY this definition works: it's the Betti number " +
                          "β₀, which is inherently integer. The definition is not arbitrary — " +
                          "it's the unique choice that produces a conserved integer.",

            ["TQM-115"] = "Charge robustness: plateau of Q=1 spans T∈[0.10, 0.85]. " +
                          "TQM-121 explains WHY: the plateau is the homotopy class. " +
            ...
            ["TQM-116"] = "Charge dynamics: dQ/dt = 0 under PDE. " +
                          "TQM-121 provides the MATHEMATICAL PROOF: the one-way barrier " +
                          "c₀·M·R·(1−R²) > 0 prevents downward crossing of T=0.5. " +
                          "This is a RIGOROUS theorem, not just empirical observation.",

            ["TQM-117"] = "Origin of Q: derived from PDE, not defined. " +
                          "TQM-121 completes the derivation by proving Q ∈ ℕ from " +
                          "first principles (β₀ + reaction barrier). The origin is now " +
                          "FULLY UNDERSTOOD: Q emerges from the mathematical structure " +
                          "of the PDE.",

            ["TQM-120"] = "Minimal charge quantum: Q=+1 is indivisible. " +
                          "TQM-121 explains WHY it's indivisible: fractional Q would " +
