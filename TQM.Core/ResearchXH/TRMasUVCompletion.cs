...
/// <summary>
/// TQM-QG Phase 33 — interpret TRM as a UV completion. The TRM kernel is the ψ (non-conformal) sector, which QG32
/// showed is a clean extension (only the Einstein sector changes). Here we test whether TRM is purely a
/// high-density/UV extension of TQM, a separate theory, or a partial extension. Using the ψ-perturbation profile
/// ψ = b·x (MetricAnsatzAudit): the g_00 correction is e^{2ψ}, which → 1 in the weak-field limit (x→0) — so TRM
/// reduces to TQM in the IR — and departs from 1 in the strong-field (high-density) regime. But the graviton
...
public static class TRMasUVCompletion
{
    ...
    /// <summary>g_00 correction factor e^{2ψ} = TRM g_00 / TQM conformal g_00.</summary>
    public static double G00Correction(double x, double b = 0.3) => Math.Exp(2.0 * Psi(x, b));

    /// <summary>Weak-field (x→0) correction = e^{2ψ(0)} = 1 (TRM reduces to TQM).</summary>
    ...
    /// <summary>TRM reduces to TQM iff the correction ≈ 1 (ψ → 0).</summary>
    public static bool ReducesToTqm(double correction, double tol = 1e-6) => Math.Abs(correction - 1.0) < tol;
    ...
    /// <summary>Core density ρ(0) for the TQM conformal metric — finite (regular core).</summary>
    ...
    /// <summary>Does the extension reduce EXACTLY to TQM in the weak-field/IR limit? Yes.</summary>
    public static bool IrReductionHolds() => ReducesToTqm(WeakFieldCorrection());
}
