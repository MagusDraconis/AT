...
/// <summary>
/// Identifies observable deviations between TQM and GR+ΛCDM.
/// TQM-X062: Observable Deviations from GR and ΛCDM
/// </summary>
public static class ObservableDeviationAnalyzer
{
    public static List<ObservableDeviationMetrics.DeviationSignature> IdentifySignatures()
    {
        return new List<ObservableDeviationMetrics.DeviationSignature>
        {
            ...
            new("Growth of structure fσ₈(z)",
                ...
                "TQM: Λ was larger in past → growth suppression\n"
            ...
            new("Gravitational wave propagation speed",
                ...
                "LIGO, Virgo, KAGRA",
                false,
                "Exact agreement. TQM's emergent c = GR's c.\n"
                + "GW170817 constraint c_g/c = 1 ± 10⁻¹⁵ — TQM passes."),
            ...
            new("Galaxy rotation curves (dark matter)",
                ...
                "SPARC, current data",
                true,
                "TQM predicts correlation-induced 'extra gravity'\n"
            ...
            new("Solar System tests (Cassini, MESSENGER)",
                ...
                + "TQM = GR to 10⁻⁴⁰ precision. Untestable."),
    ...
    public static List<ObservableDeviationMetrics.CosmologyForecast> ForecastCosmology()
    {
        ...
        foreach (double z in zVals)
        {
            ...
            // TQM: H² ≈ H₀²[0.3(1+z)³ + 0.7·(1+z)^(-0.15)]
            // (Λ decays slightly faster than ΛCDM)
            double omegaLambda = 0.7 * Math.Pow(1 + z, -0.15);
            double hTqm = Math.Sqrt(0.3 * Math.Pow(1 + z, 3) + omegaLambda);

            double wLcdm = -1.0;
            double wTqm = -1.0 + 0.015 * Math.Pow(1 + z, 1.5);
            double deltaW = wTqm - wLcdm;

            forecast.Add(new ObservableDeviationMetrics.CosmologyForecast(
                z, hLcdm, hTqm, wLcdm, wTqm, deltaW));
        }
        ...
        List<ObservableDeviationMetrics.DeviationSignature> signatures)
    {
        ...
        sb.AppendLine("FALSIFICATION RANKING — Strongest TQM Tests First");
        sb.AppendLine();
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.AppendLine("COSMOLOGICAL FORECAST — TQM vs ΛCDM");
        sb.AppendLine();
        sb.AppendLine("  z      H/H₀ (ΛCDM)  H/H₀ (TQM)  Diff%   w (ΛCDM)  w (TQM)    Δw");
        sb.AppendLine("  " + new string('─', 70));

        foreach (var f in forecast)
        {
            double diffPct = 100 * (f.H_H0_TQM - f.H_H0_GR) / f.H_H0_GR;
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
                "  {0,4:F1}  {1,11:F4}   {2,10:F4}  {3,6:F2}  {4,9:F3}  {5,8:F4}  {6,8:F4}",
                f.Redshift, f.H_H0_GR, f.H_H0_TQM, diffPct,
                f.WLcdm, f.WTqm, f.DeltaW));
        }

        sb.AppendLine();
        sb.AppendLine("  TQM predicts systematically HIGHER H(z) at z>0 (less dark energy).");
...
THE HONEST ANSWER: TQM and GR+ΛCDM are nearly identical at all
...
THE STRONGEST FALSIFICATION TEST:
  ...
  TQM predicts w ≈ -0.99 (not exactly -1).
  If Euclid finds w = -1.00 ± 0.01: TQM's time-varying Λ is FALSIFIED.
  If Euclid finds w = -0.98 ± 0.02: TQM is CONSISTENT (but not uniquely confirmed).

CLASSIFICATION C: Strong deviations exist but are subtle (~1-3%).
  Two unique TQM signatures: time-varying Λ and correlation DM.
