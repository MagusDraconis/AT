...
/// <summary>
/// Performs a complete audit of the TQM framework:
...
public static class TheoryAuditAnalyzer
{
    public static AssumptionRegistry.AuditReport PerformAudit()
    {
        ...
        string verdict = biased
            ? $"TQM FRAMEWORK IS BIASED. {critical.Length} critical assumptions with low test coverage. "
    ...
    private static List<AssumptionRegistry.TrackedAssumption> RegistryAssumptions()
    {
        ...
        {
            new("Pairwise interactions", "Q charges interact only in pairs", "All TQM",
                false, "Not tested", 9, 9, 1, 7,
                "CRITICAL: Test 3-body+ interactions via hypergraph Laplacian"),

            new("Local interactions", "J_ij = exp(-|x_i-x_j|/r_c)", "TQM-142, 143",
                true, "Tested across 10 topologies (TQM-143)", 8, 7, 6, 5,
                "Well-tested: long-range breaks discrete species"),

            new("Static graph", "Q positions fixed → L_Q constant", "All TQM",
                false, "Not tested", 10, 10, 1, 9,
                "HIGHEST PRIORITY: Dynamic graphs → L_Q(t) → open-ended innovation?"),

            new("Graph Laplacian L_Q", "L_Q = D - A as fundamental operator", "TQM-142+",
                false, "Alternatives not tested", 10, 10, 2, 8,
                "Test alternatives: normalized, magnetic, fractional, nonlinear"),

            new("Linearity", "L(ψ₁+ψ₂)=L(ψ₁)+L(ψ₂)", "TQM-140, 149",
                false, "Not tested", 9, 9, 1, 8,
                "Nonlinear L(ψ) could enable solitons, new physics"),

            new("Reversibility", "d/dt ||ψ||²=0 (norm conserved)", "TQM-149-152",
                true, "Equivalent to unitarity (TQM-152)", 10, 10, 8, 3,
                "Well-understood: equivalent to unitarity"),

            new("Euclidean distance", "Coupling depends on Euclidean |x_i-x_j|", "TQM-142",
                false, "Not tested", 5, 3, 1, 6,
                "Test graph distance instead of Euclidean"),

            new("Symmetric coupling", "J_ij = J_ji", "All TQM",
                false, "Not tested", 8, 8, 1, 7,
                "Directed graphs with J_ij≠J_ji unexplored"),

            new("1D primary focus", "Most TQM tested on 1D chains", "TQM-117-154",
                true, "2D/3D partially tested (TQM-143)", 6, 7, 4, 6,
                "Higher dimensions have richer spectra"),

            new("Theta field", "Θ as intermediary between Q and information", "TQM-128-133",
                false, "Optionality not tested", 4, 5, 3, 4,
                "Q→L_Q may suffice without Θ"),

            new("Spectral decomposition", "Species = eigenmodes of L_Q", "TQM-133, 140",
