...
/// TQM-X047: Emergence of Particles from Q-Event Topology
/// </summary>
    ...
    public static List<ParticleEmergenceMetrics.ParticleCandidate> IdentifyCandidates()
    {
        return new List<ParticleEmergenceMetrics.ParticleCandidate>
        {
            new("Q-condensate (soliton)",
                ...
                + "Each condensate = Q=+1 topological charge (TQM-010,116).",
                "Q = β₀({R>0.5}) ∈ ℕ (Betti number).",
                true, true, true,
                "PRIMARY PARTICLE CANDIDATE. Experimentally validated (TQM-010-116).\n"
                + "Effective mass m_eff = 4(1+M₀²)/(3w) from TQM-111.\n"
                + "Stable against perturbations up to threshold T_c (TQM-115).\n"
            ...
            new("Causal loop (closed timelike curve candidate)",
                ...
                + "Causal loops would require time travel. TQM excludes them.\n"
            ...
            new("Attractor in actualization dynamics",
                ...
                + "Attractors = 'species' (TQM-133) at the dynamical level."),
        };
    }
    ...
    public static List<ParticleEmergenceMetrics.TopologicalProperty> MapProperties()
    {
        return new List<ParticleEmergenceMetrics.TopologicalProperty>
        {
            ...
            new("Charge (Q)",
                ...
                "Q = β₀({R>0.5}) — Betti number of superlevel set (TQM-116).\n"
...
CHALLENGE 1: TQM produces a CONTINUOUS family of topological
species (TQM-114), not discrete particles. The Standard Model
has discrete particles (electron, muon, tau) with specific masses.
TQM predicts a continuum — wrong.
...
may emerge from additional structure not yet in TQM:
...
This is a gap — TQM has topological defects but not the
...
NEXT LAYER beyond pure Q-topology. TQM needs to be extended
with internal symmetry structure.

CHALLENGE 3: The derivation uses the PDE (TQM-010-116), which
...
VERDICT: Classification C — Stable structures found. TQM
