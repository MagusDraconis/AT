...
/// <summary>
/// Catalogs unique predictions of zero-parameter TQM.
...
public static class UniquePredictionAnalyzer
{
    ...
    public sealed record TQMPrediction(
    ...
    public static List<TQMPrediction> CatalogPredictions()
    {
        return new List<TQMPrediction>
        {
            new("Time-varying dark energy w(z) ≠ -1",
                ...
                "Euclid measures w = -1.00 ± 0.01 → TQM FALSIFIED at >3σ."),
            ...
            new("Derived acceleration scale a₀ ≈ cH₀",
                ...
                "If a₀ is shown to vary with cosmic time (not follow H₀), TQM's Λ→a₀ link is broken."),
            ...
            new("Log-normal abundance distributions",
                ...
                "If α is shown to be CONSTANT to higher precision than log-normal width allows → TQM's statistical interpretation constrained."),
    ...
    public static string FalsificationRanking(List<TQMPrediction> predictions)
    {
        var sb = new System.Text.StringBuilder();
        sb.AppendLine("FALSIFICATION RANKING — Fastest TQM Kill Shots");
        sb.AppendLine();
        ...
        {
            var p = ranked[i];
            string uniq = p.IsUnique ? "✓ TQM" : "~ shared";
    ...
    {
        return @"
TQM FALSIFICATION TREE — DECISION FLOW

┌─────────────────────────────────────────────┐
│  EXPERIMENT 1: Euclid measures w(z)         │
│  ┌─ w = -1.00 ± 0.01 → TQM FALSIFIED        │
│  └─ w ≠ -1 at >3σ → TQM SURVIVES            │
├─────────────────────────────────────────────┤
│  EXPERIMENT 2: JUNO/DUNE neutrino ordering  │
│  ┌─ INVERTED at >5σ → Model A wrong         │
│  └─ NORMAL → TQM consistent                  │
├─────────────────────────────────────────────┤
│  EXPERIMENT 3: SPARC rotation curves         │
│  ┌─ a₀ does NOT track H₀ → Λ→a₀ link broken │
│  └─ a₀ ∝ H₀ → TQM a₀ derivation holds       │
├─────────────────────────────────────────────┤
│  EXPERIMENT 4: Direct DM detection           │
│  ┌─ WIMP detected (not defect) → TQM DM wrong│
│  └─ Null → TQM consistent (no WIMP miracle)  │
└─────────────────────────────────────────────┘
