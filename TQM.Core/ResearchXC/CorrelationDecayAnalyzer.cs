    ...
    public static List<CorrelationDecayModel.CorrelationSource> SourceAudit()
    {
        return new List<CorrelationDecayModel.CorrelationSource>
        {
            ...
            new("S2: Common-cause correlations",
                ...
                "exponential",
                true,
                "IMPORTANT — spacelike events CAN be correlated via common ancestors. This is the Bell-type correlation in TQM. However, the correlation decays as the common ancestor recedes into the past. After ~ℓ_c events, the memory of the common cause is lost."),
    ...
    public static List<CorrelationDecayModel.MixingEstimate> SpectralGapAnalysis()
    {
        return new List<CorrelationDecayModel.MixingEstimate>
        {
            ...
            new("Worst-case: linear chain (bottleneck)",
                2.0, 0.01, 100, 100,
                "If the Q-graph degenerates to a 1D chain (⟨k⟩≈2), mixing is SLOW — power-law, not exponential. Spectral gap ~0.01 → mixing time ~100. But TQM is in 3+1D with ⟨k⟩≈5 — far from 1D. NOT REALIZED."),
...
STATUS:
  ...
  A2 and A3 are THEOREMS within TQM.
    ...
    public static List<CorrelationDecayModel.WorstCaseTopology> WorstCaseTopologies()
    {
        return new List<CorrelationDecayModel.WorstCaseTopology>
        {
            new("T1: Linear chain (1D causal set)",
                ...
                100, true, false,
                "REALIZED ONLY IN 1D SPACETIME. TQM is 3+1D (X042). The Q-graph has ⟨k⟩≈5 ≫ 2. NOT POSSIBLE in 3+1D — minimal degree in 3+1D is ~4."),
            ...
            new("T3: Fractal causal set (self-similar at all scales)",
                ...
                double.PositiveInfinity, true, false,
                "Requires self-similarity of the Q-graph at all scales. No evidence for this in TQM. Defects create LOCAL (compact) structure, not fractal hierarchy. The Q-graph at large scales is approximately homogeneous (cosmological principle). NOT REALIZED."),
...
CHALLENGE 4: Power-law correlations from quantum entanglement.
  ...
  RESPONSE: This is the most serious challenge. In TQM, quantum
  ...
  The key question: what IS the decay of C_ij in TQM? This depends
...
DEPENDENCE ON OTHER TQM RESULTS:
