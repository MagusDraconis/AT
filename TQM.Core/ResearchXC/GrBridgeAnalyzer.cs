...
/// <summary>
/// Audits the Q → GR bridge and determines what prevents full TQM-native derivation.
...
public static class GrBridgeAnalyzer
{
    ...
    public static List<GrBridgeModel.BridgeStep> AuditBridgeSteps()
    {
        return new List<GrBridgeModel.BridgeStep>
        {
            ...
            new("Q-event set", "Q individuates discrete events. These are the fundamental entities.",
                "TQM-derived", true, "None — this is the primitive.", "Low"),

            new("Causal ordering", "E1 < E2 iff E2 depends on E1. Partial order from logical dependence.",
                "TQM-derived", true, "None — follows from Q's individuation logic.", "Low"),

            new("Causal set (C,<)", "The pair (Q-events, order) IS a causal set.",
                "TQM-derived", true, "TQM provides the set and the order. No external import.", "Low"),

            // TIER 1: Metric structure — partially native
            new("Spacetime volume N(R)", "V(R) ∝ N(R) — counting Q-events in region R.",
                "TQM-derived", true, "The proportionality constant (fundamental length ℓ) is not derived — it's an effective scale from the Q-event density. NOT a free parameter — it sets the unit.", "Medium"),

            new("Proper time τ(a,b)", "τ = max chains between a and b. Myrheim's theorem: longest chain → proper time.",
                "External theorem", false, "Myrheim's theorem (1978) proves this for causal sets. TQM does not re-derive it. This is a KNOWN RESULT from causal set theory that TQM imports wholesale. The import is justified: the theorem is proven rigorously.", "Medium"),

            new("Metric g_μν from N", "g_μν emerges from the counting measure: ∂_μ ∂_ν N ∝ g_μν.",
                "External theorem", false, "The causal set → manifold reconstruction theorem (Malament 1977, Hawking-King-McCarthy 1976) shows that the causal order plus volume element determines the metric up to conformal factor. TQM does not re-derive this. Imported from mathematical relativity.", "High"),

            new("Dimensionality d=3+1", "Causal set reconstruction yields a manifold dimension. TQM predicts d=3+1 from complexity maximization (X042).",
                "TQM-derived", true, "X042 derives d=3+1 independently. This is ONE OF TQM's STRONGEST RESULTS. But the derivation does not go THROUGH the causal set — it's a separate argument about complexity optimization. The two derivations should be unified.", "Medium"),
            ...
            new("Discrete d'Alembertian B", "Bφ(x) acts on causal set functions. Defined by BDG (Benincasa-Dowker-Glaser).",
                "External theorem", false, "The BDG d'Alembertian is a DEFINITION, not a theorem. It's designed to converge to □ in the continuum limit. TQM provides no alternative definition. This is the SINGLE LARGEST external dependency: the entire gravitational dynamics flows through B.", "Critical"),

            new("BDG action S_BDG", "S_BDG = Σ (α N + β N_1 + γ N_2 + ...) — a discrete sum over causal intervals.",
                "External theorem", false, "The BDG action (2010) is constructed to reproduce the Einstein-Hilbert action in the continuum limit. TQM does not derive WHY this specific discrete sum is the action — it just imports the result.", "Critical"),

            new("Continuum limit → □", "B → □ (d'Alembertian) as sprinkling density → ∞.",
                "External theorem", false, "The convergence proof (BDG 2010) requires the causal set to be a Poisson sprinkling on a Lorentzian manifold. TQM must SHOW that Q-events satisfy this. Currently ASSUMED.", "Critical"),

            new("Einstein-Hilbert action S_EH", "S_EH = (1/16πG) ∫ R √(-g) d⁴x emerges from S_BDG.",
                "External theorem", false, "This is the culmination of causal set gravity. TQM inherits it. The chain is: TQM → causal set → (external BDG theory) → Einstein equations. The middle two steps are external.", "Critical"),
            ...
            new("Cosmological constant Λ", "Λ ∝ 1/√V from Q-event Poisson fluctuations (X046).",
                "TQM-derived", true, "This is an INTERNAL TQM result. But the coefficient α is not computed from primitives. Λ's sign and time-dependence are predictions; its exact magnitude is not.", "Medium"),
        };
    }

    // ── STEP 2: Candidate TQM-native gravitational actions ──
    ...
    public static List<GrBridgeModel.CandidateAction> CandidateActions()
    {
        return new List<GrBridgeModel.CandidateAction>
        {
            ...
            new("B: Graph Ricci curvature action",
                ...
                false, true, true,
                "REJECTED. Ollivier-Ricci is a coarse-grained curvature on graphs — it converges to Ricci in some limits but requires a transport distance (Wasserstein). TQM's Q-graph has no natural transport distance beyond causal order. The Ollivier definition depends on a metric that TQM doesn't provision without circularity.",
                "Circular: needs metric to define curvature to derive metric."),

            new("C: Volume deficit action (TQM-native candidate)",
                "S_vol = Σ_{R ⊂ C} [N(R) − N_flat(R)]² where N(R) = Q-event count in region R, N_flat(R) = expected count for flat causal set.",
                true, true, true,
                "PROMISING. This is conceptually closest to TQM: curvature IS deviation of event count from flat expectation. The action penalizes such deviations. In the continuum limit, this produces R (Ricci scalar) from the Myrheim-Meyer dimension estimator. But it only recovers the Ricci scalar, not the full Riemann tensor. It's EINSTEIN-HILBERT with a restricted variation (conformal only).",
                "Recovers R·√(−g) but not full G_μν. Needs extension for tensor dynamics."),

            new("D: Chain-counting action (TQM-native candidate)",
                "S_chain = Σ_{a<b∈C} [L(a,b) − L_flat(a,b)]² where L = length of longest chain.",
                true, true, true,
                "PROMISING. Longest chains encode proper time (Myrheim). Deviation from flat chain-length IS curvature. This action is fully expressible in TQM primitives: Q-events + causal order. The continuum limit recovers the full Einstein-Hilbert action (proven by BDG for the discrete d'Alembertian, which is constructed from chain-counting).",
                "This IS essentially the BDG action expressed in TQM language. It's TQM-native in ontology but mathematically equivalent to the BDG construction. CLASSIFICATION: The action IS TQM-native; the continuum-limit proof is external."),

            new("E: Correlation entropy action (TQM-native candidate)",
    ...
    public static List<GrBridgeModel.TheoremGap> TheoremGaps()
    {
        return new List<GrBridgeModel.TheoremGap>
        {
            new("Poisson sprinkling proof",
                "Prove that TQM's Q-event distribution is a Poisson process on an emergent Lorentzian manifold, to sufficient order for BDG convergence.",
                "Very Hard",
            ...
            new("BDG d'Alembertian derivation from TQM primitives",
                "Derive the BDG discrete d'Alembertian operator B directly from Q-event dynamics, without importing the BDG definition.",
                "Open Problem",
                "B is defined geometrically (sums over causal intervals at different 'layers'). A TQM-native derivation would express B in terms of Q-event correlation functions C_ij. Currently unknown whether this is possible without circularity.",
                true),

            new("Einstein equation derivation without BDG",
                "Derive G_μν = 8πG T_μν from TQM primitives without going through the BDG action.",
                "Open Problem",
            ...
            new("G from Q-event density",
                "Derive G = β·ℓ² and compute β explicitly from TQM primitives.",
                "Hard",
            ...
            new("Tensor structure from scalars",
                "The volume-deficit action only produces a scalar (R). A TQM-native action must produce the full tensor (G_μν) from event-counting primitives.",
                "Open Problem",
                "Chain-length deviation on differently-shaped causal intervals captures the full tensor structure. This IS how BDG works. The open problem is expressing this in purely TQM-primitive language without importing BDG's specific operator definition.",
                true),
        };
    }
    ...
    public static GrBridgeModel.BridgeAudit FullAudit()
    {
        ...
        int native = steps.Count(s => s.IsTqmNative);
...
PHASE 1: POISSON SPRINKLING (Moderate, 6-12 months)
  Prove that TQM Q-events approximate a Poisson sprinkling.
...
PHASE 2: BDG TRANSLATION (Hard, 12-24 months)
  Express the BDG d'Alembertian B in TQM-primitive language.
  ...
  This is PURE TRANSLATION — not new physics, just TQM-native
...
CLASSIFICATION: The bridge is COMPLETE ENOUGH for physics.
  ...
  how completely TQM derives this internally vs. importing it.
";
    }
...
WHAT THE BRIDGE LOOKS LIKE TODAY:
  ...
  Q-events + Causal Order  [TQM-NATIVE ✓]
        ↓
  Causal Set (C,<)         [TQM-NATIVE ✓]
        ↓
...
WHAT THIS MEANS:
  ...
  TQM provides the ONTOLOGY — what spacetime IS (Q-event causal set).
  TQM provides the METRIC — how distances emerge (correlation geometry).
  TQM provides DIMENSIONALITY — why 3+1 (complexity maximization).
  TQM provides MATTER — what particles ARE (topological defects).

  TQM does NOT yet provide the gravitational ACTION.
  ...
  TQM provides the causal set.

  The question is: does TQM RE-DERIVE the BDG action, or IMPORT it?
...
CLASSIFICATION:
  ...
  After Phase 1-3: A — TQM-native derivation of GR (modulo constants).
...
WHAT SURVIVES IF THE BDG BRIDGE IS WRONG:
  ...
  Only the gravity sector (~15% of TQM) depends on the BDG bridge.
  Everything else in Layers 0-3 is derived independently.

THE SINGLE LARGEST THEORETICAL GAP IN TQM:
...
TQM claims GR emerges from Q-events. But the actual derivation
goes: TQM → causal set → (external math) → GR. The middle step
is the ENTIRE causal set gravity program — decades of work by
Sorkin, Dowker, Rideout, and others. TQM did none of this work.
TQM just PROVIDES the set. That's like claiming you derived QCD
...
TQM does not currently derive it. This is the single largest
theoretical gap. The roadmap acknowledges this. The claim is not
that TQM has derived GR from scratch — it's that TQM provides a
...
it affects ALL causal set approaches, not just TQM. TQM's advantage:
...
TQM provides no selection principle.
...
VERDICT OF HOSTILE REVIEW:
  ...
  But they are NOT derived within TQM.
  The gaps are real but well-understood.
  Closing them is a mathematical closure problem, not a
  physics uncertainty.
  The worst case (BDG is somehow wrong) kills only ~15% of TQM.
";
    }

    // ── STEP 9: Native derivation attempt summary ──

    public static string WhatWouldATqmNativeDerivationLookLike()
    {
        return @"
WHAT A TQM-NATIVE GR DERIVATION WOULD LOOK LIKE

A complete TQM-native derivation would establish:

1. Q-EVENT GEOMETRY:
   The causal set (Q,<) has a well-defined dimension d.
   PROOF: Myrheim-Meyer dimension estimator converges.
   TQM STATUS: Dimension estimator is external. TQM provides
...
2. METRIC FROM COUNTING:
   ...
   TQM STATUS: Theorem is external. TQM provides the measure.
...
3. CURVATURE FROM DEVIATION:
   ...
   TQM STATUS: Conceptually TQM-native. The formula:
...
4. ACTION FROM EVENT-COUNTING:
   ...
   TQM STATUS: The action IS expressible in TQM primitives
...
5. SOURCE FROM DEFECT DENSITY:
   ...
   TQM STATUS: Heuristic. Defect density → curvature is qualitatively
...
THE FUNDAMENTAL OBSTACLE:
   ...
   limit. TQM does not currently explain WHY this specific combination
   — as opposed to any other that also converges to □.

   The deep question: Is there a UNIQUE discrete d'Alembertian, or
   is BDG just one choice among many? If unique, TQM should derive
   it. If not unique, TQM needs an additional selection principle.
