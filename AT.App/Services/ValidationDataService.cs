...
/// <summary>
/// Loads the validation single-source-of-truth JSON files (Docs/ATQG_PhysicsCoverage.json and
/// Docs/ATQG_Predictions.json, mirrored into wwwroot/data for the app). Cached per HTTP client;
...
public class ValidationDataService
{
    ...
    public async Task<PhysicsCoverageFile> GetCoverageAsync()
    {
        if (_coverage is null)
            _coverage = await _http.GetFromJsonAsync<PhysicsCoverageFile>("data/ATQG_PhysicsCoverage.json", _json);
        return _coverage!;
    }
    ...
    public async Task<PredictionRegistryFile> GetRegistryAsync()
    {
        if (_registry is null)
            _registry = await _http.GetFromJsonAsync<PredictionRegistryFile>("data/ATQG_Predictions.json", _json);
        return _registry!;
    }
    ...
    public async Task<PredictionOutcomeFile> GetOutcomesAsync()
    {
        if (_outcomes is null)
            _outcomes = await _http.GetFromJsonAsync<PredictionOutcomeFile>("data/ATQG_PredictionOutcomes.json", _json);
        return _outcomes!;
    }
}
