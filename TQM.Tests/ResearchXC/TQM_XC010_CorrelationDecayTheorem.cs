...
public class TQM_XC010_CorrelationDecayTheorem : ResearchTestBase
{
    public TQM_XC010_CorrelationDecayTheorem(ITestOutputHelper o) : base(o) { }
    ...
    private void RunTest()
    {
        ...
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "  {0,-40} {1,8} {2,-12} {3,-12}",
            "Topology", "t_mix", "Breaks Exp?", "In TQM?"));
        sb.AppendLine("  " + new string('-', 80));
        foreach (var t in assessment.Topologies)
        {
            ...
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
                ...
                t.RealizedInTqm ? "POSSIBLE ⚠" : "no ✓"));
        }
        sb.AppendLine();
        int badTopologies = assessment.Topologies.Count(t => t.RealizedInTqm);
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "  Topologies realized in TQM: {0}/{1}. All 5 rejected for 3+1D causal sets.",
        ...
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "  Bad topologies in TQM:  {0}/{1}", badTopologies, assessment.Topologies.Count));
        sb.AppendLine();
