...
public class MetricOriginTests : ResearchTestBase
{
    ...
    public void MetricOrigin_NativeOrImported()
    {
        ...
        sb.AppendLine($"Q-events            : {(qEvents.IsTqmNative ? "NATIVE" : "IMPORTED")}  (\"{qEvents.DerivationStatus}\")");
        sb.AppendLine($"Causal order        : {(order.IsTqmNative ? "NATIVE" : "IMPORTED")}  (\"{order.DerivationStatus}\")");
        sb.AppendLine($"Conformal class     : {(metric.IsTqmNative ? "NATIVE" : "IMPORTED")}  (\"{metric.DerivationStatus}\" — Malament, PROVEN)");
        sb.AppendLine($"Conformal factor    : NATIVE   (counting measure, MetricEmergenceProgram)");

        Assert.True(qEvents.IsTqmNative);
        Assert.True(order.IsTqmNative);
        Assert.False(metric.IsTqmNative);
