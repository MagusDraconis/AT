...
/// <summary>
/// Determines whether TQM already contains enough information to generate g_μν from Q-events.
...
public class MetricGenerationTests : ResearchTestBase
{
    ...
    public void DistanceStructure_DefinesMetricCandidate()
    {
        ...
        sb.AppendLine($"GrBridge step \"{metricStep.Name}\": status = \"{metricStep.DerivationStatus}\", native = {metricStep.IsTqmNative}");
    ...
    public void MetricCandidate_IsCoordinateInvariant()
    {
        ...
        sb.AppendLine("      TQM's candidate is a conformal class — invariant by construction, but no full tensor exists to transform.");
        Output.WriteLine(sb.ToString());
    }
    ...
    public void MetricGeneration_PresentOrMissing()
    {
        ...
        bool tensorComputed = metricStep.IsTqmNative;
        ...
        Assert.False(tensorComputed, "g_μν unexpectedly marked TQM-native");

        sb.AppendLine();
        sb.AppendLine("VERDICT: TQM does NOT generate g_μν from Q-events.");
