...
/// no metric-dependent Schrödinger operator (Laplace-Beltrami Δ_g) exists in TQM.Core.
///
...
public class CurvedSpaceBridgeTests : ResearchTestBase
{
    ...
    public void MetricDependentOperator_Exists()
    {
        ...
        PrintHeader("Test 1: does a metric-dependent operator exist in TQM.Core?");
        ...
        sb.AppendLine($"Occurrences of 'Beltrami' in TQM.Core: {beltrami}");
        sb.AppendLine($"Occurrences of 'curved-space Schrödinger' in TQM.Core: {curvedSchrodinger}");
    ...
    public void CurvedSpaceBridge_PresentOrAbsent()
    {
        ...
        sb.AppendLine($"Laplace-Beltrami Δ_g occurrences in TQM.Core: {beltrami}");
    ...
    private static string FindRepoRoot()
    {
        ...
        while (dir != null && !Directory.Exists(Path.Combine(dir.FullName, "TQM.Core")))
            dir = dir.Parent;
        return dir?.FullName ?? throw new DirectoryNotFoundException("TQM.Core not found");
    }

    /// <summary>Count occurrences of a substring across all TQM.Core .cs files.</summary>
    private static int CountOccurrencesInCore(string needle)
    {
        string root = FindRepoRoot();
        int count = 0;
        foreach (string file in Directory.GetFiles(
            Path.Combine(root, "TQM.Core"), "*.cs", SearchOption.AllDirectories))
        {
