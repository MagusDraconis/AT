...
/// TQM already has a weighted coupling matrix K_ij (TemporalMatrix), but no weighted Laplacian.
/// </summary>
public class MetricOperatorTests : ResearchTestBase
{
    ...
    public void CausalSetDAlembertian_HasNoMetricData()
    {
        ...
        // layers — metric-independent. No metric-dependent d'Alembertian exists in TQM.
    ...
    {
        string path = Path.Combine(FindRepoRoot(), "TQM.Core", Path.Combine(parts));
