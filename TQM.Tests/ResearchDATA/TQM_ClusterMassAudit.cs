...
public class TQM_ClusterMassAudit : ResearchTestBase
{
    public TQM_ClusterMassAudit(ITestOutputHelper o) : base(o) { }
    ...
    private void Run()
    {
        ...
        if (!File.Exists(comaPath)) comaPath = @"D:\Coding\Test\TQM\Data\coma_v3344_ready.csv";
        if (!File.Exists(acceptPath)) acceptPath = @"D:\Coding\Test\TQM\Data\Coma_Cluster_Chandra_temperature_all_profiles.dat";
        ...
        sb.AppendLine("  TQM defect model        : M ~ 8.8e14 M_sun  -> MATCHES (defect DM = collisionless)");
        sb.AppendLine("  NOTE: TQM modified gravity (g_dagger / RAR) is INSUFFICIENT at cluster scale");
        ...
        sb.AppendLine("  CONTINGENT : exact Omega_DM (TQM X065: 0.27 not derivable, initial conditions).");
        sb.AppendLine();

        // ═══ 6. SUMMARY ═══
        Sec(sb, "Section 6 — Summary");
        sb.AppendLine("  Clusters are ~6.7x more massive than their baryons -> a dark component is REQUIRED.");
        sb.AppendLine("  TQM defect model == Lambda-CDM at the mass-profile level (both need ~85% dark mass).");
