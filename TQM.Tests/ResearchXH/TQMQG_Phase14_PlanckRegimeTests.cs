...
/// bounds).
///
/// Tests: TQMQG140 (curvature divergence at ρ=0), TQMQG141 (branching saturation + minimum cell), TQMQG142 (classification).
/// </summary>
public class TQMQG_Phase14_PlanckRegimeTests : ResearchTestBase
{
    public TQMQG_Phase14_PlanckRegimeTests(ITestOutputHelper o) : base(o) { }

    // ── TQMQG140: curvature diverges at ρ=0 (metric degeneracy) ──────────────────────

    [Fact]
    public void TQMQG140_CurvatureDivergence()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG140: curvature diverges as ρ → 0 (metric degeneracy at the horizon)");
    ...
    // ── TQMQG141: branching saturation (μ=1 max sustained) + minimum cell ────────────

    [Fact]
    public void TQMQG141_BranchingSaturationAndMinimumCell()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG141: critical μ=1 is the max sustained branching; ℓ is set by a free ρ_max");
    ...
    // ── TQMQG142: classification ───────────────────────────────────────────────────────

    [Fact]
    public void TQMQG142_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG142: does actualization imply a native minimum length / maximum density?");

        sb.AppendLine("CLASSIFICATION: PARTIAL — native BOUNDS, but no native minimum length.");
        sb.AppendLine();
        sb.AppendLine("  • NATIVE lower bound ρ &gt; 0: the metric √(−g)=ρ degenerates and curvature diverges at ρ=0 (TQMQG140)");
        ...
        sb.AppendLine("    supercritical diverges, subcritical dies (TQMQG141). This bounds the rate, not the density.");
        sb.AppendLine("  • NO native minimum length ℓ: the minimum cell size ℓ = ρ_max^(−1/d) is set by a FREE maximum density");
        sb.AppendLine("    (TQMQG141). The physical Planck length ℓ = √(Għ/c³) involves G (native as deficit mass, QG6) and ħ");
        sb.AppendLine("    (a free parameter) — so the cutoff scale is not derived.");
        sb.AppendLine("  • Therefore TQM has native BOUNDS (ρ&gt;0, μ=1) but NO native minimum length or maximum density: the");
