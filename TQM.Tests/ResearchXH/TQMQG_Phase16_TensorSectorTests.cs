...
///
/// Tests: TQMQG160 (tensor d.o.f. exist for d≥3), TQMQG161 (ψ-mode activation), TQMQG162 (classification).
/// </summary>
public class TQMQG_Phase16_TensorSectorTests : ResearchTestBase
{
    public TQMQG_Phase16_TensorSectorTests(ITestOutputHelper o) : base(o) { }

    // ── TQMQG160: the tensor sector EXISTS for d≥3 ──────────────────────────────────

    [Fact]
    public void TQMQG160_TensorSectorExists()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG160: the tensor (Weyl + graviton) sector exists for d≥3");
    ...
    // ── TQMQG161: the ψ-mode activates the non-conformal (tensor) sector ─────────────

    [Fact]
    public void TQMQG161_PsiModeActivation()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG161: the ψ-perturbation activates the non-conformal (tensor) mode");
    ...
    // ── TQMQG162: classification ───────────────────────────────────────────────────────

    [Fact]
    public void TQMQG162_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG162: is tensor gravity ABSENT, FROZEN, or EMERGENT?");
        ...
        sb.AppendLine("    d=3 (3+1), independent of the conformal factor ρ (conformal invariance) (TQMQG160).");
        sb.AppendLine("  • Conformal flatness (g = ρ^(2/d)η, i.e. reference h = η) FROZES it to zero: ψ=0 → Weyl=0, so the");
        sb.AppendLine("    tensor modes do not appear in TQM's metric (TQMQG161).");
        ...
        sb.AppendLine("  • Therefore tensor gravity is FROZEN, not ABSENT: it is a genuine, countable sector that TQM's");
        sb.AppendLine("    conformal-flatness assumption sets to zero. This closes the QG10/QG15 arc: TQM is scalar gravity");
