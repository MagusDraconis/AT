...
/// <summary>
/// TQM-QG Phase 36 — derive the TRM regular-core profile. Tests whether M_eff(r)=M(1−e^(−r³/r_c³)) follows from a
/// ψ-dynamics. Classify: DERIVED / PREFERRED / ANSATZ.
///
/// Tests: TQMQG360 (Poisson saturation reproduces the form), TQMQG361 (mechanism census), TQMQG362 (classification).
/// </summary>
public class TQMQG_Phase36_TRMProfileOriginTests : ResearchTestBase
{
    public TQMQG_Phase36_TRMProfileOriginTests(ITestOutputHelper o) : base(o) { }

    // ── TQMQG360: Poisson saturation reproduces the profile exactly ───────────────────

    [Fact]
    public void TQMQG360_PoissonSaturationReproducesProfile()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG360: 1 - e^(-r^3/rc^3) = Poisson saturation in 3D volume");
    ...
    // ── TQMQG361: mechanism census ─────────────────────────────────────────────────────

    [Fact]
    public void TQMQG361_MechanismCensus()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG361: which dynamics yields the profile?");
    ...
    // ── TQMQG362: classification ───────────────────────────────────────────────────────

    [Fact]
    public void TQMQG362_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG362: DERIVED / PREFERRED / ANSATZ?");
        ...
        sb.AppendLine("    QG14, TQM has bounds but no native value for the cutoff). The Poisson-independence assumption is the");
        sb.AppendLine("    max-entropy counting model (TQM-F Phase 1). So the profile is DERIVED, with r_c as the one free scale.");
        Output.WriteLine(sb.ToString());
