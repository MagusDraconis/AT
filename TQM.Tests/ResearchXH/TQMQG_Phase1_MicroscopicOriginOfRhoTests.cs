...
/// <summary>
/// TQM-QG Phase 1 — derive ρ directly from microscopic Q-event actualization dynamics. Models actualization as
...
///
/// Tests: TQMQG10 (branching → α; critical μ=1 → α=0 → log deficit), TQMQG11 (branching-generated ρ reproduces
///        the gravity requirements), TQMQG12 (criticality = unique scale-free point + classification).
/// </summary>
public class TQMQG_Phase1_MicroscopicOriginOfRhoTests : ResearchTestBase
{
    public TQMQG_Phase1_MicroscopicOriginOfRhoTests(ITestOutputHelper o) : base(o) { }

    private const double LAMBDA = 1.5;
    private const int K = 16;

    // ── TQMQG10: branching ratio → α; critical μ=1 → α=0 → log deficit ──────────────

    [Fact]
    public void TQMQG10_BranchingToAlpha()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG10: Q-event branching → α; criticality (μ=1) → α=0 → log deficit");
    ...
    // ── TQMQG11: branching-generated ρ reproduces the gravity-required density ───────

    [Fact]
    public void TQMQG11_ReproducesGravityDensity()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG11: branching-generated ρ equals the gravity-required density exactly");
    ...
    // ── TQMQG12: criticality = unique scale-free point + classification ──────────────

    [Fact]
    public void TQMQG12_CriticalityClassification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG12: criticality (μ=1) is the unique scale-free branching point");
        ...
        sb.AppendLine("  • Scale-freeness (TQM-F1: renormalization invariance) therefore selects μ=1, i.e. α=0, uniquely.");
        sb.AppendLine("  • The microscopic actualization rule — critical Q-event branching — generates EXACTLY the ρ the");
        sb.AppendLine("    gravity program requires (TQMQG10/11).");
        sb.AppendLine("  • The single remaining input is scale-freeness itself (the criticality of the branching), which is");
        sb.AppendLine("    the renormalization-invariance requirement already reduced in TQM-F1.");
        Output.WriteLine(sb.ToString());
