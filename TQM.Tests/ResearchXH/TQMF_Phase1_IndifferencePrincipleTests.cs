...
/// <summary>
/// TQM-F Phase 1 — derive or justify the indifference principle. G4-RHO3 traced indifference → maximum
...
///
/// Tests: TQMF10 (scale covariance of primitives), TQMF11 (renormalization invariance), TQMF12 (classification).
/// </summary>
public class TQMF_Phase1_IndifferencePrincipleTests : ResearchTestBase
{
    public TQMF_Phase1_IndifferencePrincipleTests(ITestOutputHelper o) : base(o) { }

    private const int K = 8;
    private const double LAMBDA = 1.5;

    // ── TQMF10: scale covariance — power law is the unique scale-covariant abundance ──

    [Fact]
    public void TQMF10_ScaleCovariance()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMF10: primitives are scale-covariant; the power law is the unique scale-covariant form");
    ...
    // ── TQMF11: renormalization invariance — power laws are the fixed points ──────────

    [Fact]
    public void TQMF11_RenormalizationInvariance()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMF11: renormalization (coarse-graining) preserves power laws and washes out scale-setting profiles");
    ...
    // ── TQMF12: classification ───────────────────────────────────────────────────────

    [Fact]
    public void TQMF12_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMF12: is scale-freeness DERIVED, PREFERRED, or POSTULATED?");
        ...
        sb.AppendLine("  • Scale-freeness is the UNIQUE renormalization-invariant (self-similar) abundance form (TQMF11):");
        sb.AppendLine("    power laws are the RG fixed points; scale-setting profiles flow away under coarse-graining.");
        sb.AppendLine("  • The primitives carry no intrinsic scale (TQMF10): the causal order is a scale-invariant partial");
