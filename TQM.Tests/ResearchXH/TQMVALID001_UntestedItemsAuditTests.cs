...
/// <summary>
/// TQM-VALID001 — Remaining Untested Items Audit. Analyze the S,T,U oblique parameters, the electron
/// g-2 (a_e), and the Majorana character (0νββ) against the accepted TQM derivations. No new physics,
/// no parameter fitting, no speculation. For each item determine A) derived prediction, B) partial
/// derivation, C) no derivation.
/// </summary>
public class TQMVALID001_UntestedItemsAuditTests : ResearchTestBase
{
    public TQMVALID001_UntestedItemsAuditTests(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void TQMVALID0010_StatusClassification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMVALID0010: the status classification of the three items");

        sb.AppendLine("ASSUMPTIONS:");
        sb.AppendLine("  - use only accepted TQM derivations [QG162/167/171/172/174/178/179/180];");
    ...
    [Fact]
    public void TQMVALID0011_DependencyChains()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMVALID0011: the dependency chains");
    ...
    [Fact]
    public void TQMVALID0012_ClassificationAndPriority()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMVALID0012: the category and priority determination");
