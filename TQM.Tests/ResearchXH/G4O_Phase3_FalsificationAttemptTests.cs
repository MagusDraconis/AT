...
public class G4O_Phase3_FalsificationAttemptTests : ResearchTestBase
{
    ...
    public void G4_O30_SignConventionIsFixed()
    {
        ...
        // TQM ρ=1+ax² (density MINIMUM at origin): a = −(1/d)∇lnρ < 0 (points toward the minimum).
        double aMin = PhysicalObservables.GeodesicAcceleration(0.4, A, 3);
        sb.AppendLine($"TQM ρ=1+ax² (min at x=0): a = −Γ = {aMin:F4} (points toward the density minimum)");

        // TQM Gaussian (density PEAK at origin): a = −(1/d)∇lnρ > 0 (repulsive, away from the peak).
        double aPeak = PhysicalObservables.TqmAcceleration(u => PhysicalObservables.Gaussian(u), 0.4, 3);
        sb.AppendLine($"TQM Gaussian (peak at x=0): a = −∇lnρ = {aPeak:F4} (repulsive, away from the peak)");

        sb.AppendLine();
        sb.AppendLine($"Newtonian formula is attractive (−GM/r²) with the SAME a=−Γ convention; TQM's field");
    ...
    public void G4_O31_SignatureAndWeakField()
    {
        ...
        double aGauss = PhysicalObservables.TqmAcceleration(u => PhysicalObservables.Gaussian(u), x, 3);
