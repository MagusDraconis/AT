...
/// <summary>
/// TQM-F Phase 2 — audit the matter-attraction postulate. Matter = deficit is uniquely selected once
...
/// POSTULATED.
///
/// Tests: TQMF20 (geodesic convergence), TQMF21 (stability / deficit accumulation), TQMF22 (classification).
/// </summary>
public class TQMF_Phase2_MatterAttractionTests : ResearchTestBase
{
    public TQMF_Phase2_MatterAttractionTests(ITestOutputHelper o) : base(o) { }
    ...
    // ── TQMF20: geodesic convergence — attraction is derived from the metric ─────────

    [Fact]
    public void TQMF20_GeodesicConvergence()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMF20: geodesic convergence — R₀₀ selects the deficit (attractive) branch");
        ...
        double aVoid = PhysicalObservables.TqmAcceleration(Void, 0.4, D);
        double aPeak = PhysicalObservables.TqmAcceleration(Peak, 0.4, D);
    ...
    // ── TQMF21: stability — the deficit branch clumps, the peak branch disperses ─────

    [Fact]
    public void TQMF21_StabilityDeficitAccumulation()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMF21: the attractive (deficit) branch is the stable, clumping one");
    ...
    // ── TQMF22: classification ───────────────────────────────────────────────────────

    [Fact]
    public void TQMF22_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMF22: is matter-attraction DERIVED, PREFERRED, or POSTULATED?");
        ...
        sb.AppendLine("    (deficit) branch supports stable clumping (TQMF21); the peak branch disperses.");
