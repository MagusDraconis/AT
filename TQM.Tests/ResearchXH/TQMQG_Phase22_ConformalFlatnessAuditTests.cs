...
/// <summary>
/// TQM-QG Phase 22 — audit conformal-flatness consequences. Tests whether the failures (no lensing, no tensor
/// GWs, no Hawking T) are consequences of conformal flatness (ψ=0) or fundamental TQM results. Classify:
/// CONFORMAL-FLATNESS ARTIFACT / FUNDAMENTAL TQM RESULT.
///
/// Tests: TQMQG220 (lensing is a conformal-flatness artifact), TQMQG221 (tensor modes are a conformal-flatness
///        artifact), TQMQG222 (classification).
/// </summary>
public class TQMQG_Phase22_ConformalFlatnessAuditTests : ResearchTestBase
{
    public TQMQG_Phase22_ConformalFlatnessAuditTests(ITestOutputHelper o) : base(o) { }

    // ── TQMQG220: "no lensing" is a conformal-flatness artifact ─────────────────────

    [Fact]
    public void TQMQG220_LensingIsConformalFlatnessArtifact()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG220: light bending turns ON when conformal flatness is relaxed (ψ≠0)");
        ...
        sb.AppendLine("Relaxing it (ψ≠0) restores light bending — so it is an ARTIFACT of the assumption, not a fundamental TQM result.");
    ...
    // ── TQMQG221: "no tensor GWs" is a conformal-flatness artifact ──────────────────

    [Fact]
    public void TQMQG221_TensorModesConformalFlatnessArtifact()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG221: the tensor (graviton) sector is frozen by conformal flatness, not absent");
    ...
    // ── TQMQG222: classification ───────────────────────────────────────────────────────

    [Fact]
    public void TQMQG222_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG222: are the failures artifacts of conformal flatness, or fundamental?");
        ...
        sb.AppendLine("    Relaxing flatness (ψ≠0) restores lensing (TQMQG220).");
        sb.AppendLine("  • NO TENSOR GWs: a DIRECT artifact of conformal flatness (ψ=0 → graviton frozen). Relaxing it");
        sb.AppendLine("    restores the +/× modes (TQMQG221).");
        ...
        sb.AppendLine("  • Therefore the three failures are NOT fundamental TQM results: they are consequences of the");
