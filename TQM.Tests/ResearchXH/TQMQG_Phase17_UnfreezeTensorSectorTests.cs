...
/// <summary>
/// TQM-QG Phase 17 — unfreeze the tensor sector. Tests whether the scalar actualization (ρ) can source the
/// tensor/graviton (ψ) mode, or whether a non-scalar (tensor) primitive is required. Classify: ABSENT / FROZEN /
/// EMERGENT.
///
/// Tests: TQMQG170 (scalar source → trace only), TQMQG171 (tensor source required), TQMQG172 (classification).
/// </summary>
public class TQMQG_Phase17_UnfreezeTensorSectorTests : ResearchTestBase
{
    public TQMQG_Phase17_UnfreezeTensorSectorTests(ITestOutputHelper o) : base(o) { }

    // ── TQMQG170: a scalar source generates only trace (scalar) modes ────────────────

    [Fact]
    public void TQMQG170_ScalarSourceTraceOnly()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG170: a scalar (ρ) source generates only trace modes — never tensor");
    ...
    // ── TQMQG171: the tensor sector requires a non-scalar (tensor) source ────────────

    [Fact]
    public void TQMQG171_TensorSourceRequired()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG171: the Weyl tensor requires a non-scalar source (ρ has only 1 d.o.f.)");
    ...
    // ── TQMQG172: classification ───────────────────────────────────────────────────────

    [Fact]
    public void TQMQG172_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG172: does actualization source ψ? ABSENT / FROZEN / EMERGENT?");
        ...
        sb.AppendLine("    deficit gradients or branching asymmetries (TQMQG170).");
        sb.AppendLine("  • The Weyl sector has d(d+1)(d+2)(d−3)/12 d.o.f. (10 at d=3) that require a NON-SCALAR (tensor)");
        sb.AppendLine("    source; a scalar (1 d.o.f.) is structurally insufficient (TQMQG171).");
        ...
        sb.AppendLine("    / dynamical ψ-field) — beyond the current TQM primitives.");
