...
public class G4ME_Phase2_LongRangeGravityTests : ResearchTestBase
{
    ...
    private static double A3D(Func<double, double> rho, double r) => DeficitCollective.TqmAcceleration3D(rho, r, D);
    ...
    public void G4_ME22_AbundanceLawContinuumLimit()
    {
        ...
        // smooth power-law deficit ρ = ρ̄ − m0/(1 + r/r0). Its TQM field is a = −(1/d) m′/ρ ∝ −1/r².
        double m0 = 0.5, r0 = 0.5;
        double asymptote = m0 * r0 / (D * 1.0);   // M_eff → m0·r0/(d·ρ̄) as r → ∞

        sb.AppendLine($"{"r",6} {"a_TQM",12} {"M_eff=-a r^2",14} {"a_Newton(M_eff)",16}");
        ...
        sb.AppendLine($"a_TQM(12) = {aLast:F6}, Newtonian point-mass M_eff/r² = {DeficitCollective.NewtonianPointMass(mEffLast, 12.0):F6}");
        sb.AppendLine();
