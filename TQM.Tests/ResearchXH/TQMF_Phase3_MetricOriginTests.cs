...
/// <summary>
/// TQM-F Phase 3 — derive the metric origin √(−g)=ρ. Tests whether the identification of the counting measure
/// with the metric volume element emerges uniquely from counting-measure consistency. Classify: DERIVED /
/// PREFERRED / ASSUMED.
///
/// Tests: TQMF30 (volume preservation / additivity), TQMF31 (uniqueness of √(−g)=ρ), TQMF32 (classification).
/// </summary>
public class TQMF_Phase3_MetricOriginTests : ResearchTestBase
{
    public TQMF_Phase3_MetricOriginTests(ITestOutputHelper o) : base(o) { }

    // ── TQMF30: the count and volume are both measures; √(−g)=ρ makes them identical ──

    [Fact]
    public void TQMF30_VolumePreservation()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMF30: the counting measure and the metric volume are both measures");
    ...
    // ── TQMF31: √(−g)=ρ is the UNIQUE volume element consistent with counting ────────

    [Fact]
    public void TQMF31_Uniqueness()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMF31: √(−g)=ρ is the unique volume element making volume = count for all regions");
    ...
    // ── TQMF32: classification ───────────────────────────────────────────────────────

    [Fact]
    public void TQMF32_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMF32: is metric origin DERIVED, PREFERRED, or ASSUMED?");
        ...
        sb.AppendLine("    measure equal the counting measure for every region (TQMF31) — no alternative works.");
        ...
        sb.AppendLine("  • This upgrades metric origin from PREFERRED (TQM-F0) to DERIVED-in-form (unique), leaving only");
