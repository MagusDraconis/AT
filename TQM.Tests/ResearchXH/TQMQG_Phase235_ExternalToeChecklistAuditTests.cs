...
/// <summary>
/// TQM-QG Phase 235 — External TOE Checklist Audit. Compares TQM against GENERIC (external) Theory-of-
/// Everything requirements — the checklist a referee would apply to ANY claimed TOE, not TQM's own
/// criteria. Audit only — no new physics.
/// </summary>
public class TQMQG_Phase235_ExternalToeChecklistAuditTests : ResearchTestBase
{
    public TQMQG_Phase235_ExternalToeChecklistAuditTests(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void TQMQG2350_ReadinessMatrix()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG2350: the generic TOE readiness matrix");

        sb.AppendLine("ASSUMPTIONS:");
        sb.AppendLine("  - Uses GENERIC TOE requirements (external checklist), not TQM's own criteria.");
        sb.AppendLine();
    ...
    [Fact]
    public void TQMQG2351_MissingItems()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG2351: the exact missing items on the generic TOE checklist");
    ...
    [Fact]
    public void TQMQG2352_Verdict()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG2352: verdict — the external TOE readiness");
        ...
        sb.AppendLine("  - On a GENERIC TOE checklist TQM is 71% derived (80% weighted) with ONE genuinely open");
        sb.AppendLine("    criterion: INFLATION (TQM derives structure formation from Poisson seeds without it).");
        ...
        sb.AppendLine("  - TQM meets every standard TOE requirement except the cosmological-inflation epoch.");
