...
/// <summary>
/// TQM-QG Phase 7 — derive critical branching. The chain is Q-events → critical branching (μ=1) → α=0 → ρ →
...
/// POSTULATED.
///
/// Tests: TQMQG70 (extinction vs runaway — μ=1 marginal), TQMQG71 (three criteria coincide at μ=1),
///        TQMQG72 (classification).
/// </summary>
public class TQMQG_Phase7_CriticalBranchingTests : ResearchTestBase
{
    public TQMQG_Phase7_CriticalBranchingTests(ITestOutputHelper o) : base(o) { }

    // ── TQMQG70: extinction vs runaway — μ=1 is the unique marginal point ─────────────

    [Fact]
    public void TQMQG70_ExtinctionVsRunaway()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG70: μ=1 is the unique marginal point between extinction and runaway");
    ...
    // ── TQMQG71: three criteria coincide at μ=1 ──────────────────────────────────────

    [Fact]
    public void TQMQG71_ThreeCriteriaCoincide()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG71: scale-freeness, marginal stability, and max-entropy coincide at μ=1");
    ...
    [Fact]
    public void TQMQG72_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("TQMQG72: is criticality DERIVED, PREFERRED, or POSTULATED?");
        ...
        sb.AppendLine("    — only μ=1 is non-vanishing and non-exploding (TQMQG70).");
        sb.AppendLine("  • μ=1 is the UNIQUE scale-free point (L=∞, renormalization-invariant), and the maximum-entropy");
        sb.AppendLine("    point (α=0) — three independent criteria coincide (TQMQG71).");
        sb.AppendLine("  • Therefore criticality is DERIVED: it is uniquely selected by stability (non-extinction, non-");
        sb.AppendLine("    runaway) + scale-freeness (renormalization invariance, TQM-F1) + maximum entropy (G4-RHO1).");
        sb.AppendLine("  • The single conditioning input is scale-freeness (renormalization invariance), which TQM-F1");
