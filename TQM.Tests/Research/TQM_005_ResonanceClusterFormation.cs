...
using System.Text;
using TQM.Core.Resonance;
using TQM.Core.Temporal;
using TQM.Core.TemporalField;
...
/// <summary>
/// TQM-005: Resonance Cluster Formation Experiment
///
...
/// </summary>
public class TQM_005_ResonanceClusterFormation : ResearchTestBase
{
    ...
    public TQM_005_ResonanceClusterFormation(ITestOutputHelper output)
        : base(output)
    {
    }

    [Fact]
    public void TQM_005_RunResonanceClusterExperiment()
    {
    ...
    private void ExecuteExperiment()
    {
        ...
        // ── Header ──────────────────────────────────────────────
        PrintHeader("TQM-005 Resonance Cluster Formation");
        report.AppendLine("TQM-005: Self-Organizing Resonance Clusters in a Temporal Field");
        report.AppendLine();

        // ── 1. Objective ────────────────────────────────────────
        AppendSection(report, "1. Objective");
        report.AppendLine("  TQM-004 showed that two asymmetric oscillators do not synchronize via");
        ...
        if (results.StableClusterCount > 0)
        {
            ...
            report.AppendLine("    candidate for proto-particle structures in the TQM framework.");
        }
        else
        {
        ...
        report.AppendLine("    • TQM-006: Parameter sweep (α, γ, N) to find cluster formation thresholds.");
        report.AppendLine("    • TQM-007: Cluster-cluster interactions (proto-particle scattering).");
        report.AppendLine("    • TQM-008: Nonlinear frequency coupling (density saturation effects).");

        report.AppendLine();
        report.AppendLine(new string('=', 100));
        report.AppendLine("  Experiment TQM-005 completed successfully.");
        report.AppendLine(new string('=', 100));
