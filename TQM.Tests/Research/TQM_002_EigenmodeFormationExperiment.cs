...
/// <summary>
/// TQM-002: Eigenmode Formation Experiment
///
...
/// </summary>
public class TQM_002_EigenmodeFormationExperiment : ResearchTestBase
{
    private const int MatrixSize = 100;
    private const int TopModesToCompute = 20;
    private const int RandomSeed = 271;

    public TQM_002_EigenmodeFormationExperiment(ITestOutputHelper output)
        : base(output)
    {
    }

    [Fact]
    public void TQM_002_RunEigenmodeExperiment()
    {
    ...
    private void ExecuteExperiment()
    {
        ...
        // ── Header ──────────────────────────────────────────────
        PrintHeader("TQM-002 Eigenmode Formation Experiment");
        report.AppendLine("TQM-002: Eigenmode Formation in Temporal Coupling Matrices");
        report.AppendLine();
        ...
        report.AppendLine("      providing the second pillar of the TQM framework alongside synchronization.");
        report.AppendLine();
        report.AppendLine("  Next steps:");
        report.AppendLine("    • TQM-003: Evolve eigenmodes under Kuramoto dynamics");
        report.AppendLine("    • TQM-004: Structured coupling matrices (banded, small-world, scale-free)");
        report.AppendLine("    • TQM-005: Eigenmode-synchronization coupling (mode → phase coherence)");

        report.AppendLine();
        report.AppendLine(new string('=', 100));
        report.AppendLine("  Experiment TQM-002 completed successfully.");
        report.AppendLine(new string('=', 100));
