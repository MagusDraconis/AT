...
public class TQM_121_ChargeQuantizationMechanism : ResearchTestBase
{
    public TQM_121_ChargeQuantizationMechanism(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void TQM_121_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("TQM-121 Charge Quantization Mechanism");

        // ══════════════════════════════════════════════════════════════
        // ASSUMPTIONS
        // ══════════════════════════════════════════════════════════════
        Sec(sb, "0. Assumptions");
        sb.AppendLine("  1. Q = β₀({R>0.5}) exists, is conserved, created, indivisible (TQM-113..120).");
        sb.AppendLine("  2. The governing PDE is ∂R/∂t = c₀·M·R·(1−R²) + D_R·∇²R (TQM-108).");
        ...
        sb.AppendLine("  TQM-113: Q = #{R>0.5 connected domains} (definition).");
        sb.AppendLine("  TQM-115: Q robust across T∈[0.10, 0.85] (plateau).");
        sb.AppendLine("  TQM-116: dQ/dt = 0 under PDE (conservation).");
        sb.AppendLine("  TQM-117: Q derived from PDE, not arbitrarily defined.");
        sb.AppendLine("  TQM-118: Q created through nucleation (kink-antikink pairs).");
        sb.AppendLine("  TQM-119: Q follows parameter-dependent statistics.");
        sb.AppendLine("  TQM-120: Q=+1 is the minimal indivisible charge quantum.");
        sb.AppendLine();
        ...
        sb.AppendLine("  THEOREM: Q ∈ ℕ and dQ/dt = 0 under the TQM PDE.");
        sb.AppendLine();
        ...
        // ══════════════════════════════════════════════════════════════
        // SECTION 7: RELATIONSHIP TO TQM-120
        // ══════════════════════════════════════════════════════════════
        Sec(sb, "7. Relationship to TQM-120 (Minimal Charge Quantum)");

        sb.AppendLine("  TQM-120 PROVED: Q=+1 is indivisible — no sub-Q structure exists.");
        sb.AppendLine("  TQM-121 PROVES: WHY Q=+1 is indivisible.");
        sb.AppendLine();
        sb.AppendLine("  The connection:");
        sb.AppendLine("    TQM-120: Empirical. Q survived 5 fragmentation attempts.");
        sb.AppendLine("    TQM-121: Mathematical. Q ∈ ℕ because it's a Betti number.");
        sb.AppendLine();
        sb.AppendLine("  TQM-120's empirical result is EXPLAINED by TQM-121's mathematical");
        ...
        sb.AppendLine("  boundary conditions on wavefunctions, TQM charge quantization");
        ...
        sb.AppendLine("  count discrete topological features. TQM's Q belongs to this");
        sb.AppendLine("  family of CLASSICAL TOPOLOGICAL CHARGES.");
        sb.AppendLine();
        sb.AppendLine("  The uniqueness of TQM is the COMBINED MECHANISM:");
        ...
        sb.AppendLine("  Experiment TQM-121 completed successfully.");
