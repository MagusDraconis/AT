...
public class TQM_105_MeanCouplingFirstPrinciplesDerivation : ResearchTestBase
{
    private const int BaseSeed = 105_000_001;

    public TQM_105_MeanCouplingFirstPrinciplesDerivation(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void TQM_105_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("TQM-105 Mean Coupling First-Principles Derivation");

        sb.AppendLine("TQM-105: Deriving dM/dt from position-dynamic Kuramoto.");
        ...
        sb.AppendLine("    This is the same ensemble average that gave R·(1-R²) in TQM-104.");
        sb.AppendLine();
        ...
        if (report.Classification.StartsWith("D"))
            ...
            sb.AppendLine("    PARTIALLY — dR/dt is derived (TQM-104), dM/dt partially derived.");
        sb.AppendLine();

        sb.AppendLine("  Q6: Does the resulting system outperform TQM-101?");
        sb.AppendLine($"    TQM-101 survival: 4/8 (empirical fit)");
        sb.AppendLine($"    TQM-105 survival: {report.AttacksPassed}/8 (derived form)");
        ...
        sb.AppendLine($"  Classification: {report.Classification}");
        sb.AppendLine($"  dR/dt derived: YES (TQM-104, c₀·M·R·(1-R²))");
        sb.AppendLine($"  dM/dt derived: {(bestR2 > 0.15 ? "YES (this experiment)" : "PARTIALLY")}");
        sb.AppendLine();

        // ── Section 7: Conclusion ────────────────────────────────────
        Sec(sb, "7. Conclusion");
        sb.AppendLine($"  C1.  dR/dt = c₀·M·R·(1-R²)          [TQM-104, derived]");
        ...
        sb.AppendLine("  Experiment TQM-105 completed successfully.");
        sb.AppendLine(new string('=', 100));
