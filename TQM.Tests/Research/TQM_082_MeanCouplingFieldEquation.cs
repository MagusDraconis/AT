...
public class TQM_082_MeanCouplingFieldEquation : ResearchTestBase
{
    ...
    public TQM_082_MeanCouplingFieldEquation(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void TQM_082_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("TQM-082 Mean Coupling Field Equation");

        sb.AppendLine("TQM-082: Does MeanCoupling Obey a Simple Dynamical Field Equation?");
        sb.AppendLine();

        // ── Section 1: Objective ─────────────────────────────────────
        Sec(sb, "1. Objective");
        sb.AppendLine("  TQM-081: MeanCoupling compresses 97.7% of topology info.");
        ...
        sb.AppendLine("    TQM-081 showed M captures 97.7% of topology info for dR/dt.");
        if (best.AdjustedR2 >= 0.3)
            sb.AppendLine("    TQM-082 shows M itself follows a predictable law → CLOSED THEORY.");
        else if (best.AdjustedR2 >= 0.1)
            sb.AppendLine("    TQM-082 shows M has weak self-dynamics → partially closed.");
        else
            sb.AppendLine("    TQM-082 shows M is static → topology replacement is complete but M needs no equation.");
        sb.AppendLine();
        ...
        sb.AppendLine("  TQM-081: dR/dt = f(R, M)  →  R² = 0.758");
        sb.AppendLine($"  TQM-082: dM/dt = f(M, R)  →  Adj R² = {best.AdjustedR2:F4}");
        ...
        if (best.AdjustedR2 >= 0.3)
            ...
            sb.AppendLine("  → ONE-WAY: R depends on M (TQM-081), but M is static → M is a parameter, not a variable.");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine(new string('=', 100));
        sb.AppendLine("  Experiment TQM-082 completed successfully.");
        sb.AppendLine(new string('=', 100));
