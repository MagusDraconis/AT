...
public class TQM_021_EffectiveConnectivityUniversality : ResearchTestBase
{
    ...
    public TQM_021_EffectiveConnectivityUniversality(ITestOutputHelper output) : base(output) { }

    [Fact]
    public void TQM_021_RunUniversalityTest()
    {
    ...
    private void ExecuteExperiment()
    {
        ...
        PrintHeader("TQM-021 Effective Connectivity Universality");
        report.AppendLine("TQM-021: Is Nc_eff ≈ 42 a Fundamental TQM Constant?");
        report.AppendLine();

        AppendSection(report, "1. Objective");
        report.AppendLine("  TQM-020 identified Nc_eff ≈ 42 as the best universal predictor.");
        ...
        report.AppendLine($"  C4. TQM-020's Nc_eff ≈ 42 {(reject42 ? $"has been refined to {mean:F1}" : "is confirmed as a fundamental TQM constant")}.");

        report.AppendLine();
        report.AppendLine(new string('=', 100));
        report.AppendLine("  Experiment TQM-021 completed successfully.");
        report.AppendLine(new string('=', 100));
