...
public class TQM_X060_NeutrinoOrdering : ResearchTestBase
{
    public TQM_X060_NeutrinoOrdering(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void TQM_X060_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("TQM-X060 Neutrino Mass Ordering and Oscillation Structure");
        ...
        sb.AppendLine();
        sb.AppendLine("  TQM's defect potential comes from the PDE reaction term:");
        sb.AppendLine("    (1-R²)·R → V(R) ∝ (R²-1)² → λ > 0 (ATTRACTIVE).");
        sb.AppendLine("  Attractive self-interaction is BUILT INTO the TQM PDE.");
        sb.AppendLine("  → NORMAL ORDERING IS A PREDICTION OF TQM.");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("  TQM: δ_CP set by the geometric phase from the defect's");
        ...
        sb.AppendLine($"  TQM-X060 COMPLETE.");
        sb.AppendLine($"  Classification: {classification}");
        sb.AppendLine($"  NORMAL ORDERING (m_1 < m_2 < m_3) from attractive self-interaction.");
        sb.AppendLine($"  Inverted ordering requires unstable defect → ruled out by TQM PDE.");
