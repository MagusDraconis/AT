...
public class TQM_X063_CorrelationDarkMatterAudit : ResearchTestBase
{
    public TQM_X063_CorrelationDarkMatterAudit(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void TQM_X063_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("TQM-X063 Correlation-Induced Dark Matter Audit");

        var tests = DarkMatterAuditAnalyzer.RunTests();
        var fits = DarkMatterAuditAnalyzer.FitRotationCurves();

        int explained = tests.Count(t => t.TQMExplains);
        int notExplained = tests.Count(t => !t.TQMExplains);

        // 1. The acceleration scale
        Sec(sb, "The TQM Acceleration Scale — Natural a₀");
        sb.AppendLine(DarkMatterAuditAnalyzer.TheAccelerationScale());
        sb.AppendLine();

        // 2. Test results
        Sec(sb, "Dark Matter Tests — Correlation vs Particle");
        sb.AppendLine("  Observation                     TQM Explains?  Confidence  Notes");
        sb.AppendLine("  " + new string('─', 75));
        foreach (var t in tests)
        {
            string explains = t.TQMExplains ? "✓ YES" : "✗ NO";
        ...
        sb.AppendLine("  TQM correlation gravity EXPLAINS:");
        ...
        sb.AppendLine("  THIS IS A GENUINE TQM PREDICTION.");
        sb.AppendLine();
        ...
        sb.AppendLine("  Galaxy        v_flat(obs)  v_flat(TQM)  Agreement");
        sb.AppendLine("  " + new string('─', 55));
        foreach (var f in fits)
        {
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
                "  {0,-12}  {1,10:F0}    {2,10:F0}     {3,7:F3}",
                f.Galaxy, f.VFlatObs, f.VFlatTQM, f.Agreement));
        }
        sb.AppendLine();
        ...
        // 7. The Hybrid Model
        Sec(sb, "The Most Likely TQM Solution: Hybrid Model");
        sb.AppendLine("  TQM predicts: CORRELATION GRAVITY + PARTICLE DM.");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("  The particle DM could itself be a TQM defect — a stable,");
        sb.AppendLine("  neutral, weakly-interacting topological relic. This is");
        sb.AppendLine("  the natural TQM dark matter candidate.");
        sb.AppendLine();
        ...
        sb.AppendLine($"  TQM-X063 COMPLETE.");
