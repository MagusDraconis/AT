...
public static class QuantumNovelPredictionAnalyzer
{
    public static QMNovelResult RunFullAnalysis()
    {
        var ec = new[]{new EquivCheck("Hilbert space","L^2(C)","Reconstructed from Q-events (QM-002).","YES","IDENTICAL — same mathematical structure."),
            ...
            new EquivCheck("Schrodinger eq","iℏ ∂_t|ψ⟩=H|ψ⟩","Q-event field evolution (AT-149).","YES","IDENTICAL — same dynamics."),
        ...
        var fp = new[]{new FalsifyPath("Measure ℓ (Q-event spacing)","If ℓ > 10^-20 m → testable.","No — no experiment can probe ℓ.","Unknown","LOW — requires new physics experiment."),
            new FalsifyPath("Find Born Rule violation","AT predicts EXACT |ψ|^2.","Current: no violation at 10^-6.","Now","MEDIUM — would falsify both QM and AT."),
            new FalsifyPath("Find Tsirelson violation","AT predicts EXACT 2√2.","Current: 2√2 confirmed.","Now","MEDIUM — same as QM."),
    ...
    static string BuildA(EquivCheck[] e){
        ...
        sb.AppendLine("AT–QM EXACT EQUIVALENCE");sb.AppendLine();
    ...
    static string BuildB(ActResidue[] a){
        ...
        sb.AppendLine("  AT is experimentally INDISTINGUISHABLE from standard QM.");
        return sb.ToString();
    }
    ...
    static string BuildD(QmExpConstraint[] x){
        ...
        sb.AppendLine("  Experiment                  Precision   Constrains AT?   Status");
        sb.AppendLine("  --------------------------  ----------  ----------------  ---");
        ...
        sb.AppendLine();sb.AppendLine("  NO existing experiment constrains AT-specific effects.");
        sb.AppendLine("  AT is consistent with ALL tested quantum phenomena.");
        return sb.ToString();
    }
    ...
    static string BuildI(NovelPred[] n){
        ...
        sb.AppendLine();
        sb.AppendLine("  AT IS NOT A DISTINCT PHYSICAL THEORY — YET.");
        sb.AppendLine("  It is an ONTOLOGICAL REFORMULATION with superior compression:");
        sb.AppendLine("    5 QM axioms → 2 AT primitives (+ 1 parameter M^2).");
        sb.AppendLine();
        sb.AppendLine("  WHAT WOULD MAKE AT A DISTINCT THEORY:");
        sb.AppendLine("    1. Compute ℓ (Q-event spacing) from AT structure.");
        sb.AppendLine("    2. Predict N_Q (total Q-event count).");
        sb.AppendLine("    3. Derive observable deviations at accessible scales.");
        sb.AppendLine();
        sb.AppendLine("  Until ℓ is computed, AT remains:");
