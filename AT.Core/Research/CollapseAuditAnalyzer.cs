using System.Text;
using static AT.Core.Research.CollapseAuditMetrics;
...
public static class CollapseAuditAnalyzer
{
    public static CollapseAuditReport Audit()
    {
        ...
        string summary = verdict == AuditVerdict.FullySurvives
            ? "X038 FULLY SURVIVES. All 7 Many-Worlds defenses FAIL. "
              ...
              + "All three escape routes destroy the AT framework. "
              + "Within AT, branching is MATHEMATICALLY FORBIDDEN. "
    ...
    public static string DefenseReport(List<MwDefense> defenses)
    {
        ...
        foreach (var d in defenses)
        {
            ...
            foreach (var line in d.ManyWorldsArgument.Split('\n'))
                sb.AppendLine($"    {line}");
            sb.AppendLine();
            sb.AppendLine($"  AT RESPONSE:");
            foreach (var line in d.AtResponse.Split('\n'))
                sb.AppendLine($"    {line}");
            sb.AppendLine();
    ...
    public static string MwEscapeRoutes()
    {
        ...
        sb.AppendLine("    Status: MW survives, AT dies. NOT A DEFENSE WITHIN AT.");
        sb.AppendLine();
        sb.AppendLine("  Route 2: Deny Q conservation in quantum regime");
        sb.AppendLine("    Cost: Loses AT-116 theorem. Q becomes arbitrary.");
        sb.AppendLine("    Status: MW survives, AT loses its charge-conservation structure.");
        sb.AppendLine();
        ...
        sb.AppendLine("    Status: MW survives, the entire AT derivation chain collapses.");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("  ALL ESCAPE ROUTES DESTROY EITHER AT OR EMPIRICAL ADEQUACY.");
        sb.AppendLine("  Many-Worlds and AT are LOGICALLY INCOMPATIBLE.");
        return sb.ToString();
    }
}
