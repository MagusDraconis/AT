...
/// <summary>
/// Analyzes which AT results survive nonlinear operators
...
1. THE QUESTION:
   ...
   All AT (117-154) uses L_Q (linear).
...
";
    ...
    public static NonlinearPhaseMetrics.NonlinearReport Analyze(int? seed = null)
    {
        ...
        string verdict = newPhysics
            ? $"NONLINEARITY CREATES NEW PHYSICS. {regimes} regimes identified. "
              ...
              + $"Nonlinear AT is a DIFFERENT THEORY from linear AT."
    ...
    public static string HostileReview(NonlinearPhaseMetrics.NonlinearReport report)
    {
        ...
        sb.AppendLine("HOSTILE REVIEW: How much of AT is just linear algebra?");
        sb.AppendLine();
        sb.AppendLine("  AT results that DEPEND on linearity:");
        ...
        sb.AppendLine();
        sb.AppendLine("  AT results that do NOT depend on linearity:");
        ...
        sb.AppendLine();
        sb.AppendLine("  VERDICT: Most of AT's 'quantum' results are linear algebra.");
