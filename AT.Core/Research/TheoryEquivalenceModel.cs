...
/// <summary>
/// Builds the equivalence matrix between main AT and ResearchX.
    ...
    public static List<GapAnalysisMetrics.EquivalenceEntry> BuildMatrix()
    {
        return new List<GapAnalysisMetrics.EquivalenceEntry>
        {
            ...
            new("Reversibility", "POSTULATE 2 (152)", "PRINCIPLE (R axis, X011)", true,
                "Same concept: d/dt||ψ||²=0. Main AT calls it postulate, ResearchX calls it principle."),

            new("Self-Consistency F(x)=x", "IMPLICIT (eigenmodes)", "PRINCIPLE (S axis, X010)", true,
                "Main AT: eigenmodes implicitly satisfy L·v=λv = F(x)=x. ResearchX makes it explicit."),
            ...
            new("L_Q (graph Laplacian)", "DERIVED (142)", "NOT DERIVED (assumes dynamics)", false,
                "GAP: ResearchX doesn't derive the specific operator L_Q. Main AT provides the mathematical form."),

            new("Schrödinger equation", "DERIVED (149-151)", "DERIVED (at R=1,S=1, X012)", true,
                "Both converge to i∂ψ/∂t = Hψ. Main AT: via J. ResearchX: via Rev∩SC."),
            ...
            new("Information Carriers", "IMPLICIT (species)", "DERIVED (X007-X008)", true,
                "Main AT: species = eigenmodes. ResearchX: carriers = persistent info structures. Equivalent."),
            ...
            new("Complexity Staircase", "NOT FORMALIZED", "DERIVED (X018)", false,
                "GAP: Main AT never formalized the L0-L6 staircase. ResearchX contribution."),

            new("Finite/Infinite Boundary", "NOT EXPLORED", "DERIVED (X027)", false,
                "GAP: Main AT never explored L6 limits. ResearchX proves finite → saturation."),

            new("Quantum Necessity", "IMPLICIT", "DERIVED (X031)", false,
                "GAP: Main AT shows QM emerges. ResearchX proves QM is NECESSARY for max complexity."),
        };
    }
}
