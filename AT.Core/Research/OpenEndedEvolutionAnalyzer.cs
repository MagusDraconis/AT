...
/// <summary>
/// Determines whether Open-Ended Evolution (L6) is achievable in AT
...
1. THE QUESTION:
   ...
   AT-X018: 6-level staircase. L6 NOT observed.
...
3. L6 REQUIREMENTS (8):
   ...
   All 8 requirements FAIL in current AT:
...
4. THE MISSING INGREDIENT:
   ...
   AT can produce more SPECIES within existing classes.
   AT CANNOT produce new CLASSES of carriers.
   This is the bottleneck preventing L6.

5. NULL HYPOTHESIS: L6 is achievable in AT.
...
";
    ...
    public static InnovationMetrics.OpenEndedEvoReport Analyze()
    {
        ...
        int satisfied = reqs.Count(r => r.SatisfiedInAT);
        int bottlenecks = reqs.Count(r => r.IsBottleneck);

        bool achievable = satisfied >= 4;
        string missing = "CARRIER CLASS GENERATION MECHANISM. AT has no process "
        ...
        string verdict = bottlenecks >= 5
            ? $"L6 IS CONDITIONAL — NOT ACHIEVED IN CURRENT AT. "
              ...
              + $"Without at least one of these, L6 is IMPOSSIBLE in AT."
            : "L6 is impossible.";
    ...
    public static string HostileReview(InnovationMetrics.OpenEndedEvoReport report)
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"  {report.Requirements.Count(r => r.SatisfiedInAT)}/{report.Requirements.Count} requirements satisfied.");
        sb.AppendLine();
        ...
        sb.AppendLine("  - AT-138 showed saturation at ~19 species");
