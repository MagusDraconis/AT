...
public static class QEventSpacingAnalyzer
{
    ...
    public static LResult RunFullAnalysis()
    {
        ...
        var lo = new[]{new LOrigin("Causal set density","In 3+1D, causal set → continuum requires specific element density.","Causal structure + dimension.","MODERATE — constrains l but doesn't fix it.","PARTIAL — external mathematics (Sorkin+)."),
            new LOrigin("Entropy bound saturation","Bekenstein bound S ≤ A/4 must saturate for black holes. l = l_P is the only scale.","Black hole thermodynamics.","STRONG — if AT requires BH entropy = A/4 exactly.","CANDIDATE — but l_P imported from GR+QFT."),
            ...
            new LOrigin("M^2 coupling","Nonlinearity M^2 sets the scale of Q-event clustering → effective l.","M^2 + Q-event dynamics.","AT-SPECIFIC — could be the answer.","SPECULATIVE — M^2 unknown."),
        };
        ...
        var ld = new[]{new LDependency("Gravity constant G","G = l^2 * c^3 / hbar (QG-007).","G is UNCONSTRAINED.","CRITICAL — G requires l."),
            ...
            new LDependency("Planck scale","l_P = l (by definition in AT).","All quantum gravity scales unknown.","CRITICAL — entire QG program needs l."),
        ...
        var lc = new[]{new LConstraint("BH entropy = A/4","S = A/(4*l^2) matches GR if l = l_Planck.","ONLY if l = 1.616e-35 m.","MATCHES — l_Planck reproduces GR entropy."),
            new LConstraint("G measured value","G = 6.67430e-11 → l = 1.616e-35 m.","G measurement fixes l.","CIRCULAR — AT must predict G, not measure it."),
    ...
    static string BuildA(LMeaning[] l){
        ...
        sb.AppendLine("  It is the GRAIN SIZE of reality in AT.");
        return sb.ToString();
    }
    ...
    static string BuildE(ParamElim[] p){
        ...
        sb.AppendLine();
        sb.AppendLine("  AT IRREDUCIBLE PARAMETERS:");
    ...
    static string BuildI(ParamElim[] p){
        ...
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        sb.AppendLine("  ℓ IS THE FINAL UNEXPLAINED CONSTANT OF AT.");
        sb.AppendLine();
        sb.AppendLine("  AT'S IRREDUCIBLE CORE:");
        ...
        sb.AppendLine("    AT: 2 primitives + 1-3 parameters.");
        ...
        sb.AppendLine("    This is the HOLY GRAIL of the AT research program.");
