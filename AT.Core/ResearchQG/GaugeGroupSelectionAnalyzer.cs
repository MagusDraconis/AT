...
public static class GaugeGroupSelectionAnalyzer
{
    ...
    static GaugeDeriv[] BuildDerivations()
    {
        return new GaugeDeriv[]
        {
            ...
            new GaugeDeriv("SU(2) weak",2,
                ...
                "B: PARTIAL. Binary structure (n=±1) is real in AT. But SU(2)'s full continuous rotations need an independent justification. WEAK CORRESPONDENCE."),
    ...
    static GaugeCandidate[] BuildCandidates()
    {
        return new GaugeCandidate[]
        {
            ...
            new GaugeCandidate("SU(3)×SU(2)×U(1)","Tri-winding + spinor + phase. The SM.","U(1) C; SU(2) B; SU(3) B.","HIGH (observed)","FULL: Atoms, chemistry, nuclei, stable matter, life.","WORKS: The observed universe. But NOT derived from AT — largely empirical."),
            new GaugeCandidate("SU(5) GUT","Unified. 5-plet (quarks+leptons).","NOT derived from AT. External GUT hypothesis.","MEDIUM: proton decay unobserved","Unified at 10^16 GeV. Proton decay rate problem (not observed, tau>10^34 yr).","DISFAVORED: Predicts proton decay at observable rate. NOT observed. Minimal SU(5) ruled out."),
            new GaugeCandidate("SO(10) GUT","Spinor 16 = one generation.","NOT derived from AT.","MEDIUM","Right-handed neutrino naturally. But proton decay still.","UNPROVEN: Elegant (16-plet) but no experimental support. Proton decay still required."),
            new GaugeCandidate("E6","Exceptional group. 27-plet.","NOT derived from AT.","LOW: too many states","78 generators, 27-plet. Many exotic particles.","DISFAVORED: Excessive particle content. No observational motivation."),
            new GaugeCandidate("E8","Largest exceptional group. 248-dim.","NOT derived from AT. LQG/string proposals external.","LOW: enormous","248-dim adjoint. String-theory motivated (heterotic). No particle physics evidence.","SPECULATIVE: No empirical support. AT gives no reason to prefer E8."),
        };
    }
    ...
    static string BuildA()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  AT'S CURRENT STATE:");
        sb.AppendLine("    AT explains: topology, phase, oscillation, architecture.");
        sb.AppendLine("    AT does NOT yet explain: the gauge group selection.");
        ...
        sb.AppendLine("    U(1) has a NATURAL AT derivation (S¹ topology).");
    ...
    static string BuildC()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  AT CANDIDATE MECHANISMS:");
        sb.AppendLine();
        ...
        sb.AppendLine("     AT: phase θ → θ+2πn. For spin-½, θ → θ+2π gives -1.");
        ...
        sb.AppendLine("     AT: 3+1 dimensions → SO(3) rotations → SU(2) cover.");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("  VERDICT: SU(2) has SUGGESTIVE AT correspondences but");
    ...
    static string BuildD()
    {
        ...
        sb.AppendLine("     AT: net winding number conserved → hadrons have");
    ...
    static string BuildG()
    {
        ...
        sb.AppendLine("     HINTS, not derivations. AT has NOT derived SU(2) or SU(3).");
        ...
        sb.AppendLine("     CORRECT. AT should not lean on this.");
        sb.AppendLine();
    ...
    static string BuildH()
    {
        ...
        sb.AppendLine("     These specific values are NOT derived from AT.");
        sb.AppendLine();
        sb.AppendLine("  5. COUPLING CONSTANTS NOT DERIVED:");
        sb.AppendLine("     g_s, g, g' (or equivalently α_s, α_EM, sin²θ_W) are");
        sb.AppendLine("     measured, not derived. No AT prediction yet.");
        sb.AppendLine();
        ...
        sb.AppendLine("    in the AT program's particle physics coverage.");
    ...
    static string BuildI()
    {
        ...
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        sb.AppendLine("  Q1: Gauge symmetry in AT = symmetry of the phase structure.");
        ...
        sb.AppendLine("    AT has NOT yet derived SU(3)×SU(2)×U(1).");
