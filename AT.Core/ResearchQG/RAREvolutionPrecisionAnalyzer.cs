...
public static class RAREvolutionPrecisionAnalyzer
{
    ...
    static string BuildA()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  AT (DATA-004): g† = c·H₀/(2π) — the RAR acceleration scale.");
    ...
    static string BuildC(RARPoint[] pts)
    {
        ...
        sb.AppendLine("    + 0.685). ANY flat/constant g†(z) KILLS AT (favors MOND).");
        return sb.ToString();
    }
    ...
    static string BuildD()
    {
        ...
        sb.AppendLine("  AT: g†(z) = c·H(z)/2π (grows with z).");
        sb.AppendLine("    - g†(z)/g†(0) = sqrt(0.315(1+z)³ + 0.685). Rising curve.");
        sb.AppendLine();
        sb.AppendLine("  THE SEPARATION (Q3):");
        sb.AppendLine("    - At z=1: AT g† = 1.79× local; MOND = 1.00×. (79% difference).");
        sb.AppendLine("    - At z=2: AT g† = 3.03× local; MOND = 1.00×. (203% difference).");
        sb.AppendLine("    - At z=3: AT g† = 4.57× local; MOND = 1.00×. (357% difference).");
        ...
        sb.AppendLine();
        sb.AppendLine("  VERDICT: The MOND/AT separation is O(1) at z≥1 — easily");
    ...
    static string BuildE()
    {
        ...
        sb.AppendLine("    - AT: g† = c·H(z)/2π (a fundamental scale, EVOLVING).");
        sb.AppendLine("    - ΛCDM: no fundamental g† (RAR is emergent halo physics).");
        sb.AppendLine();
        sb.AppendLine("  WHY THE EVOLUTION TEST IS DECISIVE:");
        sb.AppendLine("    - If g† evolves as c·H(z)/2π → AT wins (ΛCDM has no");
        sb.AppendLine("      mechanism, MOND is constant).");
        sb.AppendLine("    - If g† is constant → MOND wins (AT's evolution fails).");
        ...
        sb.AppendLine("  It is the sharpest single test in the AT program.");
        return sb.ToString();
    }
    ...
    static string BuildG(RARPoint[] pts, double maxSep)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  WHAT WOULD FALSIFY AT (the smallest rejection):");
        sb.AppendLine("    Measure g† at z≈1 with ~25% precision.");
        sb.AppendLine("    - AT predicts g†(1) = 1.79× g†(0).");
        sb.AppendLine("    - If g†(1) = 1.00× g†(0) ± 0.25 (no evolution at 3σ),");
        sb.AppendLine("      AT's evolving RAR is FALSIFIED (MOND constant wins).");
        sb.AppendLine();
        ...
        sb.AppendLine("    - If R(z) grows as predicted → AT confirmed.");
        sb.AppendLine("    - If R(z) = 1 (flat) → AT falsified, MOND favored.");
        sb.AppendLine();
    ...
    static string BuildH()
    {
        ...
        sb.AppendLine("    AT (rising) from MOND (flat) from ΛCDM (no scale).");
        sb.AppendLine();
    ...
    static string BuildI()
    {
        ...
        sb.AppendLine("  Q6: AT (rising) vs MOND (flat) vs ΛCDM (no scale). Three-way.");
        ...
        sb.AppendLine("  Q10: g†(z)=constant at z≈1 (flat, no evolution) FALSIFIES AT.");
        sb.AppendLine();
        ...
        sb.AppendLine("    - Three-way discriminator (AT vs MOND vs ΛCDM).");
        sb.AppendLine();
        sb.AppendLine("    THE KILL-SHOT: measure R(z)=g†(z)/g†(0) at z≈1-2 to ~25%.");
        sb.AppendLine("    - Rising → AT confirmed.");
        sb.AppendLine("    - Flat → AT falsified, MOND favored.");
        sb.AppendLine("    This is the single most decisive experiment in the AT");
