...
/// to detect the AT g†(z) evolution over MOND. Deterministic and reproducible.
/// </summary>
public static class GasMassSystematicsAnalyzer
{
        ...
        string massCatalogCsv, string largeSampleCsv, string outDir)
    {
        ...
        WriteDiscriminationCsv(Path.Combine(outDir, "AT_vs_MOND_Discrimination.csv"), discrimination);
        ...
        DerivedData.Persist(fitsDir, outDir,
            "GasMassErrorBudget.csv", "GdaggerSensitivity.csv", "MonteCarloRecovery.csv", "AT_vs_MOND_Discrimination.csv");
    ...
    {
        var sb = new StringBuilder();
        sb.AppendLine("Detection thresholds — AT vs MOND separation (signal-to-noise).");
        sb.AppendLine();
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
    ...
    private static string BuildE(RecoveryPoint[] rec)
    {
        ...
        sb.AppendLine();
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "  {0,10} {1,12} {2,14} {3,12} {4,10}", "σ(Mgas)", "recover AT", "false AT", "Δχ²(AT)", "SNR²"));
        foreach (var r in rec)
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
                "  {0,10:F2} {1,12:P0} {2,14:P1} {3,12:F1} {4,10:F1}",
                r.SigmaGasDex, r.RecoveryRateAt, r.FalsePositiveRateMond, r.MeanDeltaChi2At, r.Snr2));
        return sb.ToString();
    }
    ...
    {
        var sb = new StringBuilder();
        sb.AppendLine("AT vs MOND discrimination on the OBSERVED g† (weighted by budget).");
        sb.AppendLine();
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
    ...
    private static string BuildG(RecoveryPoint[] rec, double empiricalSnr2, double observedScatter)
    {
        ...
        double recover = at03?.RecoveryRateAt ?? 0;
        double falsePos = at03?.FalsePositiveRateMond ?? 0;
        sb.AppendLine($"  If AT is TRUE at σ(Mgas)=0.3 dex (budget only): recovery = {recover:P0}.");
        sb.AppendLine($"     -> gas uncertainty misses AT {1 - recover:P0} of the time under the modeled");
        sb.AppendLine("        budget alone; it does NOT systematically hide AT.");
        sb.AppendLine($"  If MOND is TRUE at σ(Mgas)=0.3 dex: false-AT rate = {falsePos:P1}.");
        sb.AppendLine("     -> gas uncertainty does NOT fake AT (AT has no free amplitude to absorb noise).");
        sb.AppendLine();
        ...
        double observedScatter, double unmodeledScatter, double empSnr2Half, double empSnr2Fifth)
    {
        ...
        else cls = "C = AT signal recoverable with improved gas masses";
        ...
        sb.AppendLine("  cleaner rotation curves) before AT's ~0.35 dex evolution is detectable.");
        return sb.ToString();
    }
        ...
        Dictionary<string, (double logGdagger, bool constrained)> fits)
    {
        ...
        foreach (var kv in fits)
        {
            ...
            var full = AT.Core.FitsAnalysis.HighZRarAnalyzer.AnalyzeFull(path, obj, k.z, k.line, LineRest(k.line));
    ...
    {
        var sb = new StringBuilder();
        sb.AppendLine("SigmaGasDex,RecoveryRateAt,FalsePositiveRateMond,MeanDeltaChi2At,Snr2");
        foreach (var r in rec)
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
    ...
    private static void PlotRecovery(string path, RecoveryPoint[] rec)
    {
        RARPlotter.PlotLinear(path, new[]
        {
            new RARPlotter.Series(rec.Select(r => r.SigmaGasDex).ToArray(),
                rec.Select(r => r.RecoveryRateAt).ToArray(), Green, true, 0),
        }, 0.0, 0.55, 0.0, 1.05);
    }
