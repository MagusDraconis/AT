...
public static class CosmicExpansionAnalyzer
{
    public static CEExpResult RunFullAnalysis()
    {
        ...
        var cc = new[]{new CosmoComp("FLRW","Assume a(t), H(t) as primitives.","Assume ρ_Λ = constant.","AT derives a(t), H(t) from Q-events."),
            new CosmoComp("ΛCDM","FLRW + Λ + CDM. 6 parameters.","Λ = cosmological constant (fundamental).","AT: Λ(t) emerges, not constant. CDM = defect-DM."),
            new CosmoComp("Causal Set Cosmo","Discrete causal set → effective FLRW.","Not addressed directly.","CLOSEST — AT IS causal set + Q-event growth."),
            new CosmoComp("Emergent Gravity","Entropy → gravity → expansion.","Emergent from entropy.","SIMILAR — but AT starts from Q-events, not entropy."),
            new CosmoComp("AT","Q-event growth → a(t) → H(t) → Λ(t).","Λ(t) = α/√V(t) — time-varying.","THIS FRAMEWORK — expansion = Q-event accumulation."),
        };
    ...
    static string BuildI(){
        ...
        sb.AppendLine("  Q8-Q9: AT predicts w(z) = -1 + η(1+z)^(3/2). ΛCDM is late-time limit.");
        ...
        sb.AppendLine();
        sb.AppendLine("  AT's contribution:");
        ...
        sb.AppendLine();
        sb.AppendLine("  AT does NOT derive the functional form of N(t).");
        sb.AppendLine("  N(t) is the Q-event growth history — it's what nature GIVES us.");
        sb.AppendLine("  AT interprets it, but does not predict it from first principles.");
        sb.AppendLine();
