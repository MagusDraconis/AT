...
public static class GravityConstantAnalyzer
{
    ...
    public static GResult RunFullAnalysis()
    {
        ...
        var dm = new[]{new Dimensional("hbar, c, l → G","G = l^2 * c^3 / hbar","l from Q-event structure.","l GIVES G. G is the SI translation of l."),
            new Dimensional("G, c, hbar → l_P","l_P = sqrt(hbar*G/c^3)","Standard derivation.","STANDARD — l_P defined FROM G. AT reverses this."),
    ...
    static string BuildA(GRole[] g){
        ...
        sb.AppendLine("THE ROLE OF G IN AT");sb.AppendLine();
        sb.AppendLine("  Aspect               Standard View              AT View");
        sb.AppendLine("  -------------------  -------------------------  -------------------------");
        foreach(var x in g) sb.AppendLine(string.Format(CultureInfo.InvariantCulture,"  {0,-20} {1,-26} {2}",x.Aspect,x.StandardView,x.AtView));
    ...
    static string BuildC(Dimensional[] d){
        ...
        sb.AppendLine();sb.AppendLine("  AT REVERSES the Planck relation: l → G, not G → l.");
        sb.AppendLine("  Standard: measure G → compute l_P. AT: compute l → predict G.");
        return sb.ToString();
    }
    ...
    static string BuildI(DerivationPath[] d){
        ...
        sb.AppendLine("    4. AT has ONE FEWER free parameter than the Standard Model.");
        ...
        sb.AppendLine("    If l is EVER computed from AT first principles:");
        ...
        sb.AppendLine("    If AT computes l ≈ l_Planck → G is explained.");
        sb.AppendLine("    If AT computes l ≠ l_Planck → G prediction conflicts with data.");
        sb.AppendLine();
