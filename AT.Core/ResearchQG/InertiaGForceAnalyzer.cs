...
public static class InertiaGForceAnalyzer
{
    ...
    static InertiaSource[] BuildInertiaSources()
    {
        return new InertiaSource[]
        {
            ...
            new InertiaSource("Causal resistance (Machian)","Changing velocity changes the phase relationship with ALL other Q-events in the causal universe. The entire causal network must 'update.' Resistance is proportional to total causal coupling.",
                0.02,"SPECULATIVE: Consistent with Mach's principle. But AT's local phase-field explanation is sufficient — this is an unnecessary addition."),
        };
    }
    ...
    static string BuildA()
    {
        ...
        sb.AppendLine("WHAT IS INERTIA IN AT?");
        sb.AppendLine();
    ...
    static string BuildG()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  THE EQUIVALENCE PRINCIPLE IN AT:");
    ...
    static string BuildH()
    {
        ...
        sb.AppendLine("     - AT: inertia from attractor stability + phase reconfig.");
        sb.AppendLine("       (Applies to ALL structures — unified explanation).");
        sb.AppendLine("     AT's explanation is MORE GENERAL than the Higgs mechanism");
    ...
    static string BuildI(InertiaSource[] sources)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("    Inertia and G-forces EMERGE from existing AT structures");
        ...
        sb.AppendLine("    Einstein's 'happiest thought' becomes a THEOREM in AT.");
        sb.AppendLine();
