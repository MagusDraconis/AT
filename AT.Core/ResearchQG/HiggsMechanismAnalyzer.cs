...
public static class HiggsMechanismAnalyzer
{
    ...
    static MassOrigin[] BuildMassOrigins()
    {
        ...
        {
            new MassOrigin("Frequency architecture (AT)","m = E/c^2 = hbar*omega/c^2. Mass = energy of the stable oscillation pattern. Frequency IS mass.",
    ...
    static HiggsRole[] BuildHiggsRoles()
    {
        ...
        {
            new HiggsRole("AT reinterpretation","Phase field amplitude A(x). VEV <A> = v ~ 246 GeV. Sets baseline amplitude of reality's oscillation field.",
                ...
                "Yukawa coupling y_f = coupling strength between frequency architecture f and amplitude mode. Sets HOW MUCH each architecture 'feels' the VEV. m_f = y_f * v/sqrt(2). In AT: y_f determines the EFFECTIVE frequency shift of the architecture in the VEV background.",
                "FULLY COMPATIBLE: AT provides the field (phase field with amplitude DOF). SM provides the specific dynamics (Mexican hat potential, SSB). Higgs boson = AT amplitude mode. No contradiction."),
            new HiggsRole("Standard Model (unchanged)","Complex scalar doublet Phi. Potential V = -mu^2|Phi|^2 + lambda|Phi|^4. Spontaneous symmetry breaking SU(2)xU(1)->U(1)_EM.",
                ...
                "STANDARD: The established mechanism. AT does NOT contradict it — AT provides the ontological substrate (what the Higgs field IS)."),
            new HiggsRole("Amplitude mode (condensed matter)","In superconductor: order parameter Psi = |Psi|*e^{i*theta}. Amplitude mode (Higgs mode) = oscillation of |Psi| around equilibrium.",
                ...
                "Coupling of electrons to amplitude mode determines superconducting gap Delta. In AT: coupling of fermion architectures to phase field amplitude determines mass m_f.",
                "DEEP ANALOGY: Superconductor : Higgs mode :: AT phase field : Higgs boson. The Higgs IS an amplitude mode. AT explains WHY such a mode must exist."),
        };
    }
    ...
    static ParticleMass[] BuildParticleMasses()
    {
        return new ParticleMass[]
        {
            ...
            new ParticleMass("W boson",80.377,0.65,"n=0 propagating wave. Mass from 'eating' Higgs Goldstone modes. Gauge boson — not topological defect.","Gauge boson mass from Higgs kinetic term: m_W = g*v/2. Coupling g sets strength. Higgs VEV v sets scale. AT: amplitude mode VEV gives gauge bosons their inertia."),
            ...
            new ParticleMass("Photon (gamma)",0.0,0.0,"n=0 propagating wave. No core. No winding.","Yukawa = 0. Photon does NOT couple to amplitude mode (VEV). Remains massless. AT: n=0 architectures don't feel the amplitude VEV. Consistent."),
            new ParticleMass("Neutrino (nu_e)",0.0,1e-11,"n=1 vortex with nearly-decoupled core. Architecture almost does not interact with amplitude mode.","Extremely small y_nu (~0). Why? Architecture nearly invisible to amplitude VEV. Seesaw mechanism (SM) or architectural decoupling (AT). Open question."),
        };
    }
    ...
    static string BuildA(MassOrigin[] origins)
    {
        ...
        sb.AppendLine("WHAT IS MASS IN AT?");
        sb.AppendLine();
        sb.AppendLine("  AT CHAIN OF MASS EMERGENCE:");
        ...
        sb.AppendLine("    AT explains WHY mass EXISTS (architecture = energy).");
    ...
    static string BuildC(MassOrigin[] origins)
    {
        ...
        sb.AppendLine("    AT: mass = hbar*omega/c^2 (from frequency architecture).");
        ...
        sb.AppendLine();
        sb.AppendLine("    AT frequency: omega_f = intrinsic oscillation frequency");
        ...
        sb.AppendLine("    AT: different frequency bands -> different omega_f.");
    ...
    static string BuildD()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  AT PHASE FIELD:");
        sb.AppendLine("    The fundamental field in AT has two degrees of freedom:");
        ...
        sb.AppendLine("    The 'order parameter' is the AT phase field amplitude.");
        sb.AppendLine();
        sb.AppendLine("  WHAT THIS MEANS:");
        sb.AppendLine("    1. The Higgs field IS the amplitude of the AT phase field.");
        ...
        sb.AppendLine();
        sb.AppendLine("  AT PROVIDES THE ONTOLOGICAL SUBSTRATE:");
        sb.AppendLine("    SM asks: 'What field gives particles mass?' (Higgs field)");
        sb.AppendLine("    AT asks: 'What IS that field, ontologically?'");
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.AppendLine("PARTICLE SPECTRUM: AT + HIGGS");
        sb.AppendLine();
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "  {0,-20} {1,10} {2,12} {3}","Particle","Mass(GeV)","Yukawa y_f","AT + Higgs role"));
        ...
        sb.AppendLine("  AT architecture + Higgs coupling. The two frameworks");
    ...
    static string BuildF()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  AT + Higgs reinterpretation makes ALL the SAME predictions");
        sb.AppendLine("  as the Standard Model for collider physics.");
        sb.AppendLine();
        sb.AppendLine("  HIGGS BOSON PROPERTIES (LHC, measured):");
        sb.AppendLine("    m_H = 125.10 ± 0.14 GeV  — AT: amplitude mode mass. Compatible.");
        sb.AppendLine("    Spin = 0                 — AT: scalar excitation. Compatible.");
        sb.AppendLine("    Parity = +1              — AT: amplitude mode is CP-even. Compatible.");
        sb.AppendLine("    Width = 4.1 MeV          — AT: amplitude decays to coupled architectures. Compatible.");
        sb.AppendLine();
        ...
        sb.AppendLine("    All couplings proportional to mass — consistent with AT + SM.");
        sb.AppendLine();
        sb.AppendLine("  AT MAKES NO NEW COLLIDER PREDICTIONS:");
        sb.AppendLine("    The amplitude-mode reinterpretation is ONTOLOGICAL,");
        sb.AppendLine("    not phenomenological. AT changes WHAT we think the");
        sb.AppendLine("    Higgs IS, not WHAT we should see at the LHC.");
        sb.AppendLine("    This means AT is currently INDISTINGUISHABLE from SM");
    ...
    static string BuildG()
    {
        ...
        sb.AppendLine("     Accusation: AT calls the Higgs field 'phase field amplitude'");
        sb.AppendLine("     and claims to have reinterpred it.");
        sb.AppendLine("     Response: AT provides three things the SM does not:");
        ...
        sb.AppendLine("     AT does NOT derive V(A) = -mu^2 A^2 + lambda A^4.");
        sb.AppendLine("     The shape of the potential is PHENOMENOLOGICAL at this stage.");
        sb.AppendLine("     AT explains WHY there IS an amplitude degree of freedom");
        ...
        sb.AppendLine();
        sb.AppendLine("  3. 'DOES AT PREDICT THE HIGGS MASS?':");
        ...
        sb.AppendLine("     AT's failure to predict m_H is NO WORSE than the SM's.");
        sb.AppendLine();
        sb.AppendLine("  4. 'COULD FUTURE PRECISION MEASUREMENTS DISTINGUISH AT + SM?':");
        sb.AppendLine("     Possibly. If Higgs couplings deviate from SM predictions");
        sb.AppendLine("     in ways that align with AT architectural patterns");
        sb.AppendLine("     (e.g., frequency-band-dependent coupling deviations),");
        sb.AppendLine("     that would favor AT. Currently: no deviation observed.");
        sb.AppendLine();
        sb.AppendLine("  5. 'THE REAL CONTRIBUTION IS HUMILITY':");
        sb.AppendLine("     AT does NOT replace the Higgs mechanism.");
    ...
    static string BuildH()
    {
        ...
        sb.AppendLine("     mu and lambda are free parameters. AT does not yet");
        ...
        sb.AppendLine("     AT and SM. AT identifies them as architectural couplings");
        ...
        sb.AppendLine("     AT does not yet explain WHY this specific gauge group");
        ...
        sb.AppendLine("     AT currently has no answer. Hierarchy problem persists.");
        sb.AppendLine();
        sb.AppendLine("  5. NEUTRINO MASSES:");
        sb.AppendLine("     Why are neutrinos ~10^6 times lighter than electrons?");
        sb.AppendLine("     AT: 'nearly decoupled architecture.' But WHY decoupled?");
        sb.AppendLine("     Seesaw mechanism (SM extension) not yet integrated into AT.");
        sb.AppendLine();
        sb.AppendLine("  HONEST ASSESSMENT:");
        sb.AppendLine("    AT + Higgs is a COMPLEMENTARY picture that works at the");
    ...
    static string BuildI()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine("  HIGGS = AMPLITUDE MODE OF THE AT PHASE FIELD");
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        sb.AppendLine("  Q1-Q3: Mass in AT = E/c^2 = hbar*omega/c^2 (architectural energy).");
        ...
        sb.AppendLine();
        sb.AppendLine("  Q4-Q6: Higgs role = amplitude mode of the AT oscillation field.");
        ...
        sb.AppendLine("         AT + Higgs = COMPLEMENTARY. Both needed.");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("    The Higgs mechanism is NOT replaced by AT.");
        sb.AppendLine("    It is GIVEN AN ONTOLOGICAL FOUNDATION.");
        sb.AppendLine();
        sb.AppendLine("    AT explains WHAT the Higgs field IS:");
        ...
        sb.AppendLine("    AT = Ontology. SM = Phenomenology. Higgs = Bridge.");
        sb.AppendLine();
