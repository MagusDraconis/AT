...
public static class QuantumOfActionAnalyzer
{
    public static HResult RunFullAnalysis()
    {
        ...
        var ia = new[]{new InfoAction("Action per bit","hbar*ln2 per bit (Landauer-like limit).","No — this is speculative.","SPECULATIVE — not derived in AT."),
    ...
    static string BuildI(){
        ...
        sb.AppendLine();
        sb.AppendLine("  AT's IRREDUCIBLE TRIPLE:");
        ...
        sb.AppendLine("    AT: l, tau, hbar (3 fundamental constants).");
        sb.AppendLine("    Difference: AT explains WHAT they mean (spatial grain,");
        sb.AppendLine("    temporal grain, action grain). Values remain empirical.");
        sb.AppendLine();
        sb.AppendLine("  CLASSIFICATION: A — COMPLETELY EMPIRICAL");
        sb.AppendLine("  hbar is the final irreducible constant of AT.");
