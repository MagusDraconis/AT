...
/// gold (cleanest) sample to compare MOND (constant) vs AT (rising). Deterministic.
/// </summary>
public static class GoldSamplePurityAnalyzer
{
        ...
        string massCatalogCsv, string largeSampleCsv, string outDir)
    {
        ...
        // Refit g†(z) on gold: MOND vs AT vs NULL.
        var discrimination = CompareModels(gold);
        double atSignalGold = Std(gold.Select(g => RARPhysics.LogGdaggerAt(g.Z)).ToArray());
        ...
        PlotModelBars(Path.Combine(outDir, "MOND_vs_AT_GoldSample.png"), discrimination);
        ...
        return new GoldSampleReport(
            ...
            BuildC(intrinsic, atSignalGold),
            BuildD(gold),
            BuildE(discrimination, gold.Length),
            BuildF(intrinsic, atSignalGold, discrimination),
    ...
    {
        int n = gold.Length;
        double chi2At = 0, chi2Null = 0, sumW = 0, sw = 0;
        ...
        return new[]
        {
            new DiscriminationRow("AT  (g† ∝ H(z))", chi2At, chi2At, chi2At, Math.Exp(-0.5 * (chi2At - chi2Mond))),
    ...
    private static void PlotGdaggerVsZ(string path, GoldGalaxy[] gold)
    {
        ...
        // AT curve over the gold z range.
        double zmin = zs.Min(), zmax = zs.Max();
        var atZ = new double[50]; var atY = new double[50];
        for (int i = 0; i < 50; i++)
        {
            atZ[i] = zmin + (zmax - zmin) * i / 49.0;
            atY[i] = RARPhysics.LogGdaggerAt(atZ[i]);
        }
        RARPlotter.PlotLinear(path, new[]
        {
            new RARPlotter.Series(zs, lg, Blue, false, 3),
            new RARPlotter.Series(atZ, atY, Red, true, 0),
        }, zmin - 0.05, zmax + 0.05, Math.Min(lg.Min(), atY.Min()) - 0.3, Math.Max(lg.Max(), atY.Max()) + 0.3);
    }
    ...
    private static string BuildB(List<SampleScatter> samples, double intrinsic)
    {
        ...
        sb.AppendLine("  subset exists with scatter below the AT signal.");
        return sb.ToString();
    }

    private static string BuildC(double intrinsic, double atSignal)
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine($"  AT evolution signal in gold sample = {atSignal:F2} dex");
        sb.AppendLine();
        ...
        sb.AppendLine("  high-purity scatter EXCEEDS the AT signal. Kinematic purity is not the lever.");
        return sb.ToString();
    }
    ...
    {
        var sb = new StringBuilder();
        sb.AppendLine($"MOND vs AT on gold sample ({n} galaxies, weighted by σ).");
        sb.AppendLine();
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
        ...
        var at = disc.First(d => d.Model.StartsWith("AT"));
        var mond = disc.First(d => d.Model.StartsWith("MOND"));
        double dchi2 = mond.Chi2 - at.Chi2;
        sb.AppendLine();
        sb.AppendLine($"  Δχ²(MOND − AT) = {dchi2:F1}  (positive favors AT).");
        return sb.ToString();
    }

    private static string BuildF(double intrinsic, double atSignal, DiscriminationRow[] disc)
    {
        var sb = new StringBuilder();
        ...
        string cls;
        if (intrinsic < 0.35 && dchi2 > 9) cls = "Level 4 = gold sample discriminates MOND vs AT";
        else if (intrinsic < 0.35) cls = "Level 3 = intrinsic scatter < 0.35 dex (signal resolvable)";
        else if (intrinsic < atSignal) cls = "Level 2 = intrinsic scatter estimated below signal";
        ...
        sb.AppendLine($"  Extrapolated high-purity scatter = {intrinsic:F2} dex vs AT signal {atSignal:F2} dex.");
        sb.AppendLine($"  Gold-sample Δχ²(MOND−AT) = {dchi2:F1} (MOND still preferred).");
        sb.AppendLine();
        sb.AppendLine("  Central question ANSWERED: using only the most kinematically coherent galaxies");
        sb.AppendLine("  does NOT reduce the g† scatter below the AT signal. The purity-selection");
        ...
        sb.AppendLine("  route: no subset of this sample can discriminate MOND from AT at current");
