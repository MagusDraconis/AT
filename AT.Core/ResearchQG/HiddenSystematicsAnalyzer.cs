...
public static class HiddenSystematicsAnalyzer
{
    ...
    private static string BuildA(GalaxyResidual[] gals, double bestConst, double residualStd)
    {
        ...
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "  residual vs AT mean offset = {0:F2} dex (AT = {1:F2} at median z)",
            gals.Average(g => g.LogGdagger - RARPhysics.LogGdaggerAt(g.Z)),
            RARPhysics.LogGdaggerAt(gals.Select(g => g.Z).OrderBy(z => z).ElementAt(gals.Length / 2))));
        sb.AppendLine();
    ...
    {
        var sb = new StringBuilder();
        sb.AppendLine("Remaining scatter vs AT signal.");
        sb.AppendLine();
        double atSignal = 0.35;
        ...
        sb.AppendLine($"  AT evolution signal across sample      : ~{atSignal:F2} dex");
        sb.AppendLine();
        sb.AppendLine(remaining < atSignal
            ? "  Removing the correlated systematics WOULD bring scatter below the signal."
            : "  Even after removing correlated systematics, scatter EXCEEDS the AT signal.");
        return sb.ToString();
    }
    ...
    private static string BuildF(double residualStd, double r2Total, double remaining, ResidualCorrelation[] corr)
    {
        ...
        string cls;
        if (r2Total >= 0.5 && remaining < 0.35) cls = "Level 4 = remaining variance compatible with AT recovery";
        ...
        sb.AppendLine($"  Remaining after decomposition = {remaining:F2} dex vs AT signal ~0.35 dex.");
        sb.AppendLine();
        sb.AppendLine($"  Dominant hidden systematics: {topNames} (kinematic-coherence proxies).");
        sb.AppendLine("  Removing them does NOT (yet) drop scatter below the AT signal — the");
