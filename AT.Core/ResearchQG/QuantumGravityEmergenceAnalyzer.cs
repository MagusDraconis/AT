...
public static class QuantumGravityEmergenceAnalyzer
{
    public static QGResult RunFullAnalysis()
    {
        ...
        var es = new[]{new EmergenceStep(1,"Q-events","Causal order","Q defines succession (before/after).","Q (individuation).","DERIVED — Q-events are temporal."),
            ...
            new EmergenceStep(6,"Curvature","Einstein equations","Causal set action → G = 8πG T.","M^2 (nonlinearity).","DERIVED — from AT action."),
        };
        ...
        var gs = new[]{new GeoStep(1,"Connection Γ^λ_μν","Metric derivatives → Christoffel symbols.","Metric.","CONNECTION — from metric."),
            ...
            new GeoStep(5,"Einstein equations","G_μν = 8πG_eff T_μν.","Action principle + M^2.","GRAVITY — from AT dynamics."),
        };
        ...
        var qf = new[]{new QGFramework("String Theory","Quantize GR. Strings in background spacetime.","FUNDAMENTAL (background).","Fundamental force (spin-2).","OPPOSITE — AT: gravity is emergent, not quantized."),
            new QGFramework("Loop Quantum Gravity","Quantize geometry directly.","EMERGENT (spin networks).","Fundamental (kinematical).","SIMILAR — but AT starts from Q-events, not geometry."),
            new QGFramework("Causal Set Theory","Spacetime = discrete causal set.","EMERGENT from causal order.","Emergent from action.","CLOSEST — AT IS causal set with Q-event foundation."),
            new QGFramework("Asymptotic Safety","GR is non-perturbatively renormalizable.","FUNDAMENTAL (continuum).","Fundamental interaction.","DIFFERENT — AT denies continuum fundamentality."),
            new QGFramework("AT","Q-events → causal set → metric → GR.","COMPLETELY EMERGENT.","Last structure to emerge.","THIS FRAMEWORK — gravity = geometry from Q-events."),
        };
    ...
    static string BuildI(){
        ...
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
        sb.AppendLine();
        sb.AppendLine("  GRAVITY IS COMPLETELY EMERGENT IN AT.");
        sb.AppendLine();
        ...
        sb.AppendLine("    AT view: 'Gravity IS quantum — it emerges from Q-events.'");
        sb.AppendLine("    The problem is not quantization. It's emergence.");
        sb.AppendLine();
        sb.AppendLine("  AT's position: CLOSEST to Causal Set Theory, but adds:");
