...
/// acceleration scale g† per galaxy, bins by redshift, and compares AT (rising
...
public static class HighZRARExtractionAnalyzer
{
    ...
    // Local acceleration scale (AT prediction at z=0): c*H0/(2pi).
    ...
    private static EvolutionBin[] BuildEvolutionBins(RARFit[] fits)
    {
        ...
        foreach (var (zmin, zmax) in bins)
        {
            ...
            result.Add(new EvolutionBin(zmean, zmin, zmax, mean, median, err, g.Length, at));
        }
    ...
    private static TheoryComparison[] CompareModels(RARFit[] fits)
    {
        ...
        // AT (fixed normalization): g†(z) = g†_local * E(z), 0 free params.
        double chi2AtFixed = 0;
        foreach (var f in d)
        {
            double pred = Math.Log10(GdaggerLocal * Math.Sqrt(OmM * Math.Pow(1 + f.Redshift, 3) + OmL));
            chi2AtFixed += Sq((f.LogGdagger - pred) / Math.Max(f.LogGdagger_err, 0.05));
        }
        // AT (free amplitude): g†(z) = A * E(z), 1 free param.
        double chi2AtFree = FitAmplitude(d, rising: true);
        ...
        return new[]
        {
            new TheoryComparison("AT  (g† ∝ H(z), fixed norm)", chi2AtFixed, chi2AtFixed + 0, chi2AtFixed + 0 * Math.Log(n), 0, n, -0.5 * chi2AtFixed),
            new TheoryComparison("AT  (g† ∝ H(z), free amp)", chi2AtFree, chi2AtFree + 2, chi2AtFree + 1 * Math.Log(Math.Max(n,1)), 1, n, -0.5 * chi2AtFree),
    ...
    {
        var atFixed = cmp.First(c => c.Model.StartsWith("AT  (g† ∝ H(z), fixed"));
        ...
        results.Add(new FalsificationResult(
            ...
            "BTFR prior is circular (assumes constant g†). Cannot falsify or confirm AT."));
    ...
    private static string BuildE(EvolutionBin[] bins)
    {
        ...
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            ...
            "z_mean", "N", "mean", "median", "err", "AT pred"));
    ...
    private static string BuildF(TheoryComparison[] cmp)
    {
        ...
        sb.AppendLine();
        var at = cmp.First(c => c.Model.StartsWith("AT  (g† ∝ H(z), fixed"));
        var mond = cmp.First(c => c.Model.StartsWith("MOND"));
        double dchi2 = mond.Chi2 - at.Chi2;
        double bf = Math.Exp(0.5 * dchi2);   // likelihood ratio AT/MOND
    ...
    private static string BuildG(TheoryComparison[] cmp, int n)
    {
        ...
        sb.AppendLine($"Δχ² (MOND − AT) = {dchi2:F1} → nominal significance ≈ {sig:F1} σ.");
        sb.AppendLine();
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.AppendLine("Hostile falsification attempts (assume AT wrong; try to kill it).");
        sb.AppendLine();
    ...
    private static string BuildI(string verdict, EvolutionBin[] bins, TheoryComparison[] cmp)
    {
        ...
        sb.AppendLine("     AT forms agree), and the baryonic Tully-Fisher relation");
        ...
        sb.AppendLine("     scale. It cannot discriminate AT from MOND.");
        sb.AppendLine();
