...
/// <summary>Shared RAR/AT constants and closed-form predictions used by the
...
public static class RARPhysics
{
    ...
    /// <summary>AT prediction: g†(z) = c·H(z)/2π = g†(0)·sqrt(Ωm(1+z)³+ΩΛ).</summary>
    public static double GdaggerAt(double z) =>
        GdaggerLocal() * Math.Sqrt(OmM * Math.Pow(1 + z, 3) + OmL);

    public static double LogGdaggerAt(double z) => Math.Log10(GdaggerAt(z));
}
...
public sealed record RecoveryPoint(
    double SigmaGasDex,
    double RecoveryRateAt,
    double FalsePositiveRateMond,
    double MeanDeltaChi2At,
    double Snr2);
