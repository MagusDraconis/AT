...
public static class HighZRarExecutionAnalyzer
{
    public static ExecResult RunFullAnalysis()
    {
        ...
        var fp = new[]{new FalsifyPath("g†(0) with MUSE","g†=cH0/(2pi) to <5%","Calibrates baseline.","2025","CRITICAL"),
            new FalsifyPath("g† at z=0.8 (KMOS3D)","g†>g†(0) by >2sigma","AT consistent.","2026","FASTEST"),
            new FalsifyPath("g† at z=1.5 (JWST)","g†>g†(0) by >3sigma","AT validated. MOND falsified.","2028","DEFINITIVE"),
            new FalsifyPath("g†(z) slope measurement","d(g†)/dz > 0 at >3sigma","AT confirmed. MOND dead.","2031","FINAL"),
            new FalsifyPath("Null: g†(z)=CONSTANT","No evolution at 3sigma","AT FALSIFIED.","2031","HONEST"),
        };
        ...
        var rp = new[]{new RoadmapPhase(1,"2025-2026","Calibrate g†(0). Compile KMOS3D+KROSS.","g†(0) to <5%. First g†(z) at z~0.8.","ARCHIVAL — publish by 2026."),
            ...
            new RoadmapPhase(3,"2028-2031","Combine JWST+Euclid+Roman. Full systematics.","g†(z) to 3σ. AT-vs-MOND decided.","COMBINED — publish by 2031."),
    ...
    static string BuildI(RoadmapPhase[] r,FalsifyPath[] f){
        ...
        sb.AppendLine("    5. 2031: Combined 3σ result. AT vs MOND decided.");
        sb.AppendLine();
        sb.AppendLine("  ═══════════════════════════════════════════════════════════════");
