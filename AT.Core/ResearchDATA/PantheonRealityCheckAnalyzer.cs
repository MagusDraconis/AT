...
/// <summary>
/// Parses Pantheon+SH0ES data and compares ΛCDM vs AT w(z) fits.
...
public static class PantheonRealityCheckAnalyzer
{
    ...
    private static double EofZ(double z, double omegaM, bool useAt)
    {
    ...
    private static double LuminosityDistanceIntegral(double z, double omegaM, bool useAt)
    {
        int steps = 300;
        double dz = z / steps;
        double sum = 0;
        ...
        {
            double zp = i * dz;
            double ez = EofZ(zp, omegaM, useAt);
    ...
    private static double DistanceModulus(double z, double omegaM, bool useAt)
    {
        double integral = LuminosityDistanceIntegral(z, omegaM, useAt);
    ...
    public static CosmologyFit FitModel(List<PantheonRecord> data, double omegaM, bool useAt)
    {
        int n = data.Count;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            muModel[i] = DistanceModulus(z, omegaM, useAt);
        ...
        string modelName = useAt ? "AT w(z)" : "LambdaCDM";
    ...
    public static (CosmologyFit lcdm, CosmologyFit at, string summary) CompareModels(
...
Redshift: z in [{data.Min(d => d.Zcmb):F4}, {data.Max(d => d.Zcmb):F2}]
                         ...
                         LCDM               AT
                         ----               ---
  Omega_m                {lcdm.OmegaM:F4}             {at.OmegaM:F4}
  M (nuisance)           {lcdm.BestM:F4}            {at.BestM:F4}
  chi^2                  {lcdm.ChiSq:F1}           {at.ChiSq:F1}
  dof                    {lcdm.Dof}               {lcdm.Dof}
  chi^2/dof              {lcdm.ReducedChiSq:F4}         {at.ReducedChiSq:F4}
  AIC                    {lcdm.Aic:F1}           {at.Aic:F1}
  BIC                    {lcdm.Bic:F1}           {at.Bic:F1}

  Delta_chi2 (AT-LCDM): {dChi:F2}
  Delta_AIC:             {dAic:F2}
  Delta_BIC:             {dBic:F2}
  Significance:          {sig:F2}sigma

  VERDICT: The data {(Math.Abs(dChi) < 1.0 ? "CANNOT distinguish" :
     dChi < 0 ? "MARGINALLY PREFER AT" : "MARGINALLY PREFER LCDM")}
  ({prefers} by |Delta_chi2| = {Math.Abs(dChi):F2}, {sig:F1}sigma).

  AT deviation |w+1| ~ 0.015 is TOO SMALL for Pantheon+SH0ES
...
";
        ...
        var atF = at with { Verdict = $"Om={at.OmegaM:F4} chi2/dof={at.ReducedChiSq:F3} Dchi2={dChi:F2}" };

        return (lcdmF, atF, summary);
    }
...
QUESTION: Does Pantheon+SH0ES data prefer AT over LCDM?

ANSWER: NO — the data CANNOT distinguish them at statistically
        significant level.

WHY:
  AT predicts w(z) = -1 + 0.015·(1+z)^(3/2) (~1.5% deviation at z=0).
...
THIS IS EXPECTED:
  ...
  Pantheon+SH0ES is consistent with BOTH models. AT survives its first
