...
public static class HighZRarFeasibilityAnalyzer
{
    ...
    public static FeasResult RunFullAnalysis()
    {
        ...
        var tl = new[]{new TLEntry(1,2028,"JWST+Euclid","First g†(z) measurement. 1σ hint possible."),
            ...
            new TLEntry(3,2031,"All combined","3σ detection. AT vs MOND distinguishable."),
    ...
    static string BuildA(ZPoint[] z,double g0){
        ...
        sb.AppendLine();sb.AppendLine("  AT: g† DOUBLES by z≈2. MOND: constant.");
        return sb.ToString();
    }

    static string BuildB(MondDiff[] m){
        var sb=new System.Text.StringBuilder();
        sb.AppendLine("MOND vs AT DIFFERENCE");sb.AppendLine();
        sb.AppendLine("  z      AT g†     MOND a0    Δ          σ/galaxy   Detectable per galaxy?");
        sb.AppendLine("  -----  ---------  ---------  ---------  ---------  ------------------------");
        foreach(var x in m) sb.AppendLine(string.Format(CultureInfo.InvariantCulture,"  {0,-6:F1} {1,-10:F3} {2,-10:F3} {3,-10:F3} {4,-10:F2} {5}",x.Z,x.At,x.Mond,x.Delta,x.Sigma,x.Detectable));
