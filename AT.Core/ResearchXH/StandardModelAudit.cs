...
/// This phase identifies all major measured SM quantities NOT yet tested against AT-QG and classifies
...
/// observables), assigns each a classification from the AT-QG record, and ranks the remaining
/// (untested/partially-tested) tests by importance and gap size. Fully deterministic — the table is a
/// static catalog of the AT-QG derivation record through QG169.
/// </summary>
public static class StandardModelAudit
{
    ...
    /// <summary>The major measured SM quantities against the AT-QG record.</summary>
    public static SmQuantity[] Catalog()
    {
        return new[]
        {
            ...
            new SmQuantity("muon g-2 (a_μ)", Coverage.Untested, "—",
                "—", "11659205.9e-10", 0.0, "no AT-QG derivation"),
            new SmQuantity("electron g-2 (a_e)", Coverage.Untested, "—",
                "—", "1159652180.7e-12", 0.0, "no AT-QG derivation"),
    ...
    /// measured-SM information space that AT-QG has reached.
    /// </summary>
    ...
    public static (string Name, Coverage Status, int Rank, string Why)[] RemainingTests()
    {
        ...
        // Ranked by physics importance and size of the gap to AT-QG reach.
        (string, Coverage, int, string)[] ranked =
        {
            ("muon g-2 (a_μ)", Coverage.Untested, 1,
                "the largest measured-vs-SM deviation; no AT-QG origin — highest-priority gap"),
