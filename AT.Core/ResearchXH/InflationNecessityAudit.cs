...
/// <summary>
/// AT-QG Phase 236 — Inflation Necessity Audit. Determines whether inflation is actually required, by
/// checking the five problems inflation was invented to solve against the AT derivations (QG227-231).
...
///         causal horizon.
///       • solved by AT: the initial state is the UNIFORM CRITICAL STATE ρ_k = 1/K (QG227) — a single
...
///       • solved by AT: the single-scale R universe has Ω_Λ + Ω_m = 1 EXACTLY as a structural identity
...
///       • solved by AT: the Poisson counting variance of Q-events (QG228 information; QG231 seeds
...
///       • solved by AT: the uniform critical initial state (QG227) is isotropic by construction; QG77
...
///       • solved by AT: the pressureless deficit dust grows the Poisson seeds linearly (QG231:
///         δ(a) = δ_i·a/a_i).
///
/// RESULT: all five problems are SOLVED BY AT without an inflationary epoch.
///
/// CAVEAT — the CMB ANISOTROPY SPECTRUM (the acoustic-peak structure and the near-scale-invariant tilt
/// n_s ≈ 0.96 that inflation predicts) is NOT numerically derived by AT: the Poisson seed is white/scale-
...
/// AT: uniform critical initial state QG227, exact flatness identity QG230/234, Poisson seeds QG228/231,
...
{
    public enum Resolution { ByInflation, ByAt, Unresolved }
    ...
    {
        new Check(1, "Horizon problem", Resolution.ByAt,
        ...
        new Check(2, "Flatness problem", Resolution.ByAt,
        ...
        new Check(3, "Initial perturbations", Resolution.ByAt,
        ...
        new Check(4, "CMB isotropy", Resolution.ByAt,
        ...
        new Check(5, "Structure formation", Resolution.ByAt,
    ...
    /// <summary>Number of problems solved by AT.</summary>
    public static int AtSolvedCount()
        => Checks().Count(c => c.Resolved == Resolution.ByAt);

    /// <summary>Number of problems that would be solved by inflation (0 in the AT view).</summary>
    ...
    /// <summary>All five motive problems are solved by AT.</summary>
    public static bool AllFiveSolvedByAt()
        => AtSolvedCount() == 5;
    ...
    ///   INFLATION REQUIRED — the motive problems are not solved by AT (unresolved or inflation-only);
    ///   INFLATION REPLACED — all motive problems are solved by AT AND the observable spectrum is matched;
    ///   PARTIAL INFLATION  — all motive problems are solved by AT (the epoch is replaced) but the
    ///                        observable spectrum content (tilt, acoustic peaks) is not fully reproduced.
    /// </summary>
    public static string Classify()
    {
        if (!AllFiveSolvedByAt()) return "INFLATION REQUIRED";
    ...
    public static string Summary()
        => $"{Classify()} — {AtSolvedCount()}/5 motive problems solved by AT "
