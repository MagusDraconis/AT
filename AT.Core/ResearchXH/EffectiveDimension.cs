...
/// <summary>
/// AT-QG Phase 4 — fundamental vs observed dimension. Tests whether AT can be fundamentally D-dimensional
...
public static class EffectiveDimension
{
    ...
    /// <summary>
    /// Conformal efficiency = fraction of the observable metric's degrees of freedom NOT frozen by conformal
    /// flatness = 1/(1 + d(d−3)/2). Maximized at d=3 (1.0, nothing frozen); decreases monotonically for d≥4.
    /// Independent of the fundamental D.
    /// </summary>
    public static double ConformalEfficiency(int d)
        => 1.0 / (1.0 + DimensionAnalysis.GravitonPolarizations(d));

    /// <summary>Curvature per active degree of freedom = Einstein components generated by the single scalar ρ =
    /// d(d+1)/2 (monotonic increasing in d).</summary>
    public static double CurvaturePerDof(int d) => d * (d + 1.0) / 2.0;
}
