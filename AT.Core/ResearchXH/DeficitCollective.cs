...
{
    /// <summary>3D radial AT acceleration a = −(1/d) d(ln ρ)/dr = −(1/d) ρ′/ρ (central difference).</summary>
    public static double AtAcceleration3D(Func<double, double> rho, double r, int d = 3, double h = 1e-6)
    ...
    /// The deficit m = m₀ r₀/(r₀+r) has a 1/r tail, so a_AT ∝ −1/r² (Newtonian point-mass form).
    /// </summary>
    ...
    /// <summary>Rotation-curve proxy v²(r) = r·|a(r)| (circular-velocity squared for the AT field).</summary>
    public static double RotationCurveProxy(Func<double, double> rho, double r, int d = 3, double h = 1e-6)
        => r * Math.Abs(AtAcceleration3D(rho, r, d, h));
