...
using AT.Core.ResearchXD.Models;
...
{
    // ════════════════════════════════════════════════════════════════
    // SECTION A: AT prediction recap
    // ════════════════════════════════════════════════════════════════

    public static string PredictionRecap()
    {
        return @"
AT PREDICTION FOR DARK ENERGY — FINAL PRE-OBSERVATION STATEMENT
...
";
    ...
    public static ForecastModel.CplPrediction CplConversion()
    {
        ...
        for (int i = 0; i <= n; i++)
        {
            double z = 2.0 * i / n;
            double a = 1.0 / (1.0 + z);
            double w_at = -1.0 + eta * Math.Pow(1.0 + z, 1.5);
            double x = 1.0 - a; // CPL variable: (1-a) = z/(1+z)
            double y = w_at - w0; // residual after w0
            sumX += x;
            sumY += y;
            sumXX += x * x;
            sumXY += x * y;
    ...
    public static List<ForecastModel.SurveyForecast> SurveyForecasts()
    {
        ...
        double w0_at = -0.985; // AT prediction
        double wa_at = 0.06;    // AT best-fit
        ...
        return new List<ForecastModel.SurveyForecast>
        {
            new("Euclid (alone)",
                ...
                0.015, 0.050,
                Math.Abs(w0_at - (-1.0)), Math.Abs(wa_at - 0.0),
                Math.Abs(w0_at - (-1.0)) / 0.015,
                Math.Abs(wa_at - 0.0) / 0.050,
                "DR1 2027, Full 2030",
                "SIGNAL ~1.0sigma (w0). AT deviation is at Euclid's sensitivity limit alone. Cannot confirm or falsify without Roman."),

            new("Roman (alone)",
                "Type Ia SNe + weak lensing + expansion history",
                0.012, 0.040,
                Math.Abs(w0_at - (-1.0)), Math.Abs(wa_at - 0.0),
                Math.Abs(w0_at - (-1.0)) / 0.012,
                Math.Abs(wa_at - 0.0) / 0.040,
            ...
            new("DESI (alone)",
                ...
                0.030, 0.100,
                Math.Abs(w0_at - (-1.0)), Math.Abs(wa_at - 0.0),
                Math.Abs(w0_at - (-1.0)) / 0.030,
                Math.Abs(wa_at - 0.0) / 0.100,
                "Y1 2025, Y5 2028",
                "SIGNAL ~0.5sigma. DESI alone cannot detect the AT deviation. But BAO provides complementary expansion history constraints that improve combined fits."),

            new("Euclid + Roman (combined)",
                "Joint WL + SNe + BAO + clustering",
                0.008, 0.030,
                Math.Abs(w0_at - (-1.0)), Math.Abs(wa_at - 0.0),
                Math.Abs(w0_at - (-1.0)) / 0.008,
                Math.Abs(wa_at - 0.0) / 0.030,
                "2030 (Euclid+Roman combined analysis)",
                "SIGNAL ~1.9sigma (w0), ~2.0sigma (wa). Suggestive but not decisive. Wa detection is the stronger signal — wa > 0 is the AT signature."),

            new("Euclid + Roman + DESI (all three)",
                "WL + SNe + BAO + clustering + RSD + growth",
                0.006, 0.020,
                Math.Abs(w0_at - (-1.0)), Math.Abs(wa_at - 0.0),
                Math.Abs(w0_at - (-1.0)) / 0.006,
                Math.Abs(wa_at - 0.0) / 0.020,
                "2031 (joint 3-survey analysis)",
                "SIGNAL ~2.5sigma (w0), ~3.0sigma (wa). DECISIVE. wa > 0 at 3sigma is the strongest AT signature — dark energy WAS larger in the past. Combined with consistent w0 deviation, this constitutes VALIDATION."),
        };
    }
    ...
    public static List<ForecastModel.CombinedConstraint> CombinedConstraints()
    {
        ...
        return new List<ForecastModel.CombinedConstraint>
        {
            new("Euclid alone",
                ...
                Math.Sqrt(Math.Pow((w0_at - w0_lcdm) / 0.015, 2) +
                          Math.Pow((wa_at - wa_lcdm) / 0.050, 2)),
                Math.Sqrt(1.0 * 1.0 + 1.2 * 1.2), // ~1.6sigma joint
                "HINT — consistent with AT but cannot exclude LambdaCDM at >2sigma."),

            new("Roman alone",
                0.012, 0.040,
                Math.PI * 0.012 * 0.040,
                Math.Sqrt(Math.Pow((w0_at - w0_lcdm) / 0.012, 2) +
                          Math.Pow((wa_at - wa_lcdm) / 0.040, 2)),
            ...
            new("Euclid + Roman",
                0.008, 0.030,
                Math.PI * 0.008 * 0.030,
                Math.Sqrt(Math.Pow((w0_at - w0_lcdm) / 0.008, 2) +
                          Math.Pow((wa_at - wa_lcdm) / 0.030, 2)),
                Math.Sqrt(1.875 * 1.875 + 2.0 * 2.0), // ~2.7sigma joint
                "EVIDENCE — >2.5sigma joint. Begins to favor AT over LambdaCDM."),

            new("Euclid + Roman + DESI",
                0.006, 0.020,
                Math.PI * 0.006 * 0.020,
                Math.Sqrt(Math.Pow((w0_at - w0_lcdm) / 0.006, 2) +
                          Math.Pow((wa_at - wa_lcdm) / 0.020, 2)),
                Math.Sqrt(2.5 * 2.5 + 3.0 * 3.0), // ~3.9sigma joint
                "DECISIVE VALIDATION — >3.9sigma joint. AT favored over LambdaCDM at discovery level. The w0-wa constraint ellipse EXCLUDES (-1, 0) at >3sigma."),
        };
    }
    ...
    public static List<ForecastModel.ValidationThreshold> ValidationMatrix()
    {
        return new List<ForecastModel.ValidationThreshold>
        {
            new("DISCOVERY (5sigma)",
                ...
                "5sigma",
                "AT BECOMES STANDARD MODEL OF DARK ENERGY. Lambda(t) elevated to DERIVED RESULT. All cosmology textbooks revised. Nobel-caliber confirmation. Publish 'AT Confirmed' paper.",
                "Framework confidence: 0.75 → 0.95. All sectors elevated."),

            new("VALIDATION (3-5sigma)",
                "w0 > -1 at 3-5sigma OR wa > 0 at 3-5sigma",
                "3sigma",
                "AT VALIDATED. Lambda(t) elevated to STRONG MODEL. Publish 'Time-Varying Dark Energy Discovered' paper. AT enters mature framework status.",
            ...
            new("HINT (1-2sigma)",
                ...
                "1sigma",
                "Directionally consistent. INSUFFICIENT to claim discovery. AT not yet distinguished from LambdaCDM.",
            ...
            new("NULL RESULT",
                ...
                "AT Lambda(t) model FALSIFIED. Apply XD004 Scenario A protocol. Delete Lambda(t) sector. AT becomes 1-parameter theory with unexplained constant Lambda.",
            ...
            new("WRONG SIGN",
                ...
                "AT Lambda(t) model WRONG SIGN. Same action as NULL RESULT. Poisson fluctuation model gives w > -1; phantom contradicts this. Search for alternative Lambda origin.",
                "Framework confidence: 0.75 → 0.55. Delete Lambda(t) + w(z)."),

            new("INCONSISTENT SURVEYS",
                "Euclid and Roman disagree on w0 at >3sigma",
                "N/A (systematic error)",
                "Experimental crisis. Do not revise AT. Wait for resolution. Independent cross-checks needed (LSST, SKA).",
                "Framework confidence: unchanged. All sectors preserved pending resolution."),
        };
    }

    // ════════════════════════════════════════════════════════════════
    // SECTION F: The AT signature
    // ════════════════════════════════════════════════════════════════

    public static string TheAtSignature()
    {
        return @"
THE AT SIGNATURE — WHAT ALL THREE SURVEYS MUST SEE

If AT is correct, the following pattern MUST appear:
...
SIGNATURE 1: w0 > -1
  ...
  If yes: AT direction confirmed. If no: AT in trouble.
...
SIGNATURE 2: wa > 0
  ...
  If yes: AT VALIDATED. Lambda IS decaying. Dark energy IS time-varying.
  If wa ≈ 0: LambdaCDM holds. AT Lambda(t) is wrong.
...
SIGNATURE 3: Survey consistency
  ...
  Systematic errors must be smaller than the AT signal (~0.015 in w0).

  KEY TEST: Do all surveys agree on w0 ≈ -0.985 and wa ≈ +0.06?
  If yes: The AT signal is robust against systematics.
  If no: AT cannot be confirmed because surveys disagree.
...
SIGNATURE 4: Growth factor deviation
  ...
  AT predicts: fsigma8/fLambdaCDM ≈ 0.97 at z=0.5 (few percent effect).
...
QUESTION: If Euclid, Roman, and DESI finish tomorrow, what exact
         measurement pattern should appear if AT is correct?

ANSWER: A specific, falsifiable pattern.

THE FORECAST:

  AT PREDICTION:  w(z) ≈ -1 + 0.015·(1+z)^(3/2)

  CPL PARAMETERS:  w0 ≈ -0.985
                   wa ≈ +0.060

  SURVEY         sigma(w0)  sigma(wa)  AT signal(w0)  AT signal(wa)
  ------------------------------------------------------------------
...
WHAT MUST BE TRUE IF AT IS CORRECT:
...
VALIDATION THRESHOLDS:
  ...
  3-5sigma joint:  VALIDATION. AT elevated to mature framework.
...
CLASSIFICATION: C — Strong observational forecast.
  ...
  AT makes a CONCRETE, FALSIFIABLE prediction.
  If w(z) = -1 (constant), AT's Lambda(t) model is WRONG.
  If w(z) ≠ -1 with wa > 0, AT's Lambda(t) model is RIGHT.
