...
public static class EuclidDecisionTreeAnalyzer
{
    ...
    {
        return @"
AT PREDICTION FOR DARK ENERGY EQUATION OF STATE

PRECISE FORM:
  w(z) ≈ −1 + η · (1+z)^(3/2)

  η ≈ 0.015 (AT-estimated coefficient)
...
IF FAILURE OCCURS — WHERE?
  ...
  The chain fails at the LINK where the observation disagrees:
    ...
    • Inconsistent surveys → NOT a AT problem. Experimental issue.
    ...
    public static List<EuclidDecisionModel.ObservationalScenario> ScenarioMatrix()
    {
        return new List<EuclidDecisionModel.ObservationalScenario>
        {
            ...
            new("B", "Weak deviation (Euclid alone, ~2sigma)",
                ...
                "HINT of deviation — consistent with AT, but wait for Roman.",
                "PRESERVE — MONITOR",
                new string[] { },
            ...
            new("C", "Moderate deviation, correct sign (>3sigma)",
                ...
                "TIME-VARYING Λ CONFIRMED. AT prediction SURVIVES critical test.",
                "MODEL REFINEMENT",
                new string[] { },
            ...
            new("D", "Strong deviation, correct sign (>5sigma)",
                ...
                "STRONGER THAN PREDICTED — correct sign but larger magnitude. AT survives but Λ(t) model needs recalibration.",
                "MODEL REVISION",
            ...
            new("E", "Deviation, WRONG sign (w < -1, phantom)",
                ...
                "AT PREDICTION WRONG. Poisson fluctuation model gives w > -1. Phantom (w < -1) contradicts Lambda(t) = alpha/sqrt(V(t)).",
                "SECTOR REPLACEMENT",
            ...
            new("F", "Euclid-Roman tension (inconsistent surveys)",
                ...
                "EXPERIMENTAL CRISIS — not a AT-specific problem.",
                "NO ACTION — WAIT",
                new string[] { },
            ...
            new("G", "DESI confirms w(z) evolution, independent of Euclid",
                ...
                "INDEPENDENT CONFIRMATION of time-varying dark energy. AT STRENGTHENED by two-survey agreement.",
                "ELEVATE CONFIDENCE",
                new string[] { },
    ...
    public static List<EuclidDecisionModel.RevisionAction> RevisionActions()
    {
        return new List<EuclidDecisionModel.RevisionAction>
        {
            ...
            new("A (w=−1)", "Λ(t) model (X046)", "DELETE",
                "Poisson fluctuation model for Λ is WRONG. Λ is either a fundamental constant or generated by a different mechanism. Remove from AT. Publish retraction.",
                "2030: Retraction. 2032: Replacement cosmology (if any)."),
            new("A (w=−1)", "w(z) prediction (P1)", "DELETE",
                "Prediction falsified. Remove from AT prediction catalog.",
            ...
            new("A (w=−1)", "Overall framework", "REVISE",
                "Framework becomes 1-parameter (M²) with unexplained constant Λ. AT explains quantum + particles + gravity but not dark energy. Open problem: why Λ ≈ H₀²?",
                "2030-2035: Revised AT with constant Λ."),
            ...
            new("C (moderate dev)", "w(z) prediction", "PRESERVE – CELEBRATE",
                "AT's #1 falsifiable prediction survives the most critical test. AT is now DISTINGUISHED from ΛCDM empirically.",
                "2030: Major publication. AT validated."),
            ...
            new("C (moderate dev)", "Overall framework", "PRESERVE – STRENGTHEN",
                "AT survives Euclid. Framework confidence elevated from 0.75 to 0.85. Causal set cosmology is empirically supported.",
                "2030: AT enters 'mature framework' status."),
            ...
            new("E (phantom)", "Overall framework", "REVISE",
                "Similar to Scenario A. Time-varying Λ exists but with wrong sign → different mechanism. AT gravity + particle sectors survive.",
...
QUESTION: If Euclid publishes tomorrow, can AT respond immediately,
...
DECISION TREE SUMMARY:
  ...
  IF Euclid and Roman disagree (>3σ tension):
    → EXPERIMENTAL CRISIS. Not a AT problem.
    → All sectors preserved pending resolution.
    → Action: No AT changes. Wait for experimental resolution.

  IF DESI independently confirms w(z) evolution:
    → TWO-SURVEY CONFIRMATION. AT strengthened.
