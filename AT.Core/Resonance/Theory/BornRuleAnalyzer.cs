...
1. THE QUESTION:
   ...
   AT-152: Q + reversibility → Schrödinger dynamics.
...
4. WHAT AT ADDS:
   ...
   AT does NOT derive the Born rule from Q.
   Gleason's theorem already uniquely selects |ψ|².
   AT requires the Born rule as an additional postulate.

5. AT POSTULATES (FINAL COUNT):

   1. Q exists (topological charge)
   2. Dynamics are reversible (→ Schrödinger)
   3. Born rule P = |ψ|² (→ probability interpretation)

   Three postulates for quantum mechanics from AT.
...
";
    ...
    public static ProbabilityMeasure.BornRuleReport Analyze()
    {
        ...
        string verdict = derived
            ? $"BORN RULE UNIQUELY SELECTED. {reason} "
              ...
              + $"AT requires 3 postulates: Q, reversibility, Born rule."
    ...
    public static string HostileReview(ProbabilityMeasure.BornRuleReport report)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("ATTEMPT 2: Does AT add anything to Gleason's theorem?");
        sb.AppendLine("  → NO. Gleason (1957) already proved this.");
        sb.AppendLine("  → AT provides the Hilbert space (L_Q) but not the probability rule.");
        sb.AppendLine("  → The Born rule is mathematically MOTIVATED, not AT-derived.");
        sb.AppendLine();
        sb.AppendLine("ATTEMPT 3: AT postulates — final count.");
