...
1. FROM SINGLE CHARGE TO MANY CHARGES:
   ...
   AT-117..122 established that Q=+1 is the fundamental charge quantum.
...
2. INTERACTION MECHANISM:
   ...
   Effective range: d_eff ≈ 5λ (coupling range, AT-110).
...
4. CONTINUUM CHARGE DESCRIPTION:
   ...
   — S = source term (nucleation rate — AT-118)
...
5. PHASE DIAGRAM PREDICTIONS:
   ...
   At low K: Vacuum only (no nucleation — AT-118).
   ...
   — Nucleation probability (AT-118, AT-119)
   — Coupling range λ
   — System size N
...
";
        ...
        int seedsPerPoint = 4, int maxIterations = 3000)
    {
        ...
        string continuumEq =
            ...
            "where ν = nucleation rate (AT-118), γ = merger rate ∝ K, μ = decay rate ≈ 0";
...
2. GOVERNING EQUATION (phenomenological):
   ...
   — ν = nucleation rate (charge creation, AT-118)
...
";
        ...
        CollectiveStateProfile.ProtoMatterCollectiveReport report)
    {
        ...
        sb.AppendLine(report.PhaseDiagram.CriticalDensityIndex >= 0
            ? $"  YES — percolation threshold at ρ_Q ≈ {report.PhaseDiagram.DensityAxis[report.PhaseDiagram.CriticalDensityIndex]:F3}. " +
              "This is the analog of AT-006's ρc for the charge system."
    ...
    public static Dictionary<string, string> ValidateAgainstPriorExperiments()
    {
        ...
        {
            ["AT-005"] = "Resonance clusters at ρ>0 = collective charge behavior. " +
                          "The clusters observed in AT-005 are the Cluster phase. " +
                          "The critical density is the percolation threshold of the charge network.",

            ["AT-006"] = "ρc≈0.09 = charge percolation threshold. " +
                          "Below ρc: Q=0 (vacuum). Above ρc: Q≥1 nucleates. " +
                          "Far above ρc: multiple charges → collective phases.",

            ["AT-010"] = "Proto-matter condensates = Q=+1 charges. " +
            ...
            ["AT-012"] = "Two-condensate interaction = Q=2→Q=1 merger. " +
                          ...
                          "Merger rate γ in ∂ρ_Q/∂t is measured by AT-012.",

            ["AT-118"] = "Charge creation = nucleation source term ν in ∂ρ_Q/∂t. " +
                          "The creation rate determines the steady-state charge density.",

            ["AT-119"] = "Charge statistics = counting statistics of the charge gas. " +
