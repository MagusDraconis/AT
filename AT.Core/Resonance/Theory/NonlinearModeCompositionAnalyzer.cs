...
/// <summary>
/// Determines whether AT-139 information species are nonlinear compositions
...
1. THE HYPOTHESIS:
   ...
   AT-140: 10 eigenmodes, but AT-138/139: 13-19 species.
...
";
    ...
    public static CompositeMode.NonlinearCompositionReport Analyze(int? seed = null)
    {
        ...
        // Get AT-139 species.
        var (at139Species, _, _, _) = AttractorGraph.MapLandscape(500);

        // Map AT-139 species to composites.
        var mappings = SpeciesCompositionMap.MapToComposites(at139Species, clustered);
        ...
        double coverage = at139Species.Count > 0 ? (double)mappedSpecies / at139Species.Count : 0;
        ...
        string verdict = compositesExplainExcess
            ? $"NONLINEAR COMPOSITION CONFIRMED. {totalUniqueComposites} unique composites "
              ...
              + $"{mappedSpecies}/{at139Species.Count} AT-139 species mapped "
    ...
    public static string HostileReview(CompositeMode.NonlinearCompositionReport report)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("ATTEMPT 2: Do composites actually match AT-139 species?");
