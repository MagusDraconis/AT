...
public sealed class GravityService : ICalculationService
{
    ...
    public GravityService()
    {
        ...
        [
            ...
            new("temporal-core-test-audit",
                 "Temporal Core Test Audit \u2014 can the time sector be tested without the shape of space?",
                 "BOUNDARY: the temporal observable z = 1/sqrt(-g00) - 1 is PURE - it needs g00 alone, with no gamma, no lensing and no Shapiro delay - and AT and GR agree at O(x) and first differ at O(x^2). The signal is large at compact objects (dz = 0.125628 at J0740+6620, 45 % of z_AT) but the arena is neutron stars only, and the compactness that READS the signal is itself obtained from light bending - so the observable is pure while the number required to read it is not",
                 [
                     new("THE OBSERVABLE IS PURE", "AT: z = e^x - 1 from g00 = -e^(-2x); GR: z = 1/sqrt(1-2x) - 1", "the series agree at O(x) and first differ at O(x^2): dz = x^2 + (7/3)x^3 + (13/3)x^4. The observables take NO spatial argument (G_035's arity test). G_020's published figures are reproduced EXACTLY: dz = 0.125628 (G_020: 0.1256) at x = 0.247002; at x = 0.187982, z_AT = 0.206812 and z_GR = 0.265888"),
                     new("THE ARENA IS NEUTRON STARS ONLY", "reaching |dz| > sigma_z needs x >= 0.030494 (sigma_z = 1e-3), 0.089439 (0.01), 0.115094 (0.017766) and 0.133350 (0.025125)", "the weak field (tower 2.45e-15, optical clock 1.1e-18, Sun 2.12e-6, binary pulsar 1e-6, white dwarf 1e-4) would need sigma_z between 1e-30 and 1e-12 - nine to twelve orders out of reach. This is the arithmetic behind G_019's 'only neutron stars are a live arena'"),
                     new("THE WEAK-FIELD SPLIT IS UNREPRESENTABLE", "each law carries ~1 ulp = 2.220446049250313e-16 of absolute error while the true split at x = 2.45e-15 is 6.0025e-30", "MEASURED: the direct subtraction returns -7.5093e-18 - wrong magnitude AND wrong SIGN - and at x = 1.1e-18 z_GR is annihilated outright (exactly 0). The split can be STATED but not COMPUTED BY SUBTRACTION, so the audit uses the series below x = 1e-3"),
                     new("THE BOTTLENECK IS THE COMPACTNESS", "sigma_x/x = 13.73 % achieved against 3.661 % needed for 5 sigma - 3.75x - with 1.334 sigma attainable today", "the signal is not the problem (44.84 % of z_AT), AND for a neutron star the compactness is itself obtained from LIGHT BENDING (pulse-profile modelling), which is the spatial sector G_032 showed to be an assumed primitive. CENTRAL FINDING: the observable is pure, but the number required to READ it is not"),
                     new("A PURE ROUTE EXISTS AND IS ~3x SHORT", "R_inf = R/sqrt(1-2x) from a thermal flux plus a parallax distance is a g00 effect - photon energy and arrival rate, not bending", "so {z, R_inf} solves {M, R} with NO light bending at all. Distance and atmosphere systematics keep sigma_x/x above the ~3.7 % needed"),
                     new("THE FIRST EXPERIMENT", "Pound & Rebka (1960): the 22.5 m tower redshift, x = 2.45e-15 - pure g00, no light bending anywhere in the measurement, and the first probe of the core at all", "BUT it reaches only the first-order term, the one AT SHARES WITH GR (x^2 = 6.0e-30). No experiment has yet probed the core's distinctive content; the frontier is a compact-object {z, R_inf} measurement at sigma_x/x <= 3.7 %"),
                 ],
                 "BOUNDARY. The temporal core is testable IN PRINCIPLE: the observable z = 1/sqrt(-g00) - 1 needs g00 alone, and AT and GR first differ at O(x^2) - which makes the arena neutron stars only, since the weak field would need sigma_z between 1e-30 and 1e-12. In fact the weak-field split is not merely unmeasurable but UNREPRESENTABLE: each law carries ~1 ulp of 1.0 of absolute error while the true split at the Pound-Rebka compactness is 6.0025e-30, so the direct subtraction returns -7.5093e-18 with the WRONG SIGN, and at x = 1.1e-18 z_GR is annihilated to exactly 0. Where the signal IS large - J0740+6620, dz = 0.125628, 44.84 % of z_AT - the obstacle is not the signal but the compactness: G_020's sigma_x/x = 13.73 % against 3.661 % needed (3.75x, 1.334 sigma attainable today), and for a neutron star that compactness is itself obtained from LIGHT BENDING via pulse-profile modelling. THE OBSERVABLE IS PURE, BUT THE NUMBER REQUIRED TO READ IT IS NOT. A pure route exists - R_inf = R/sqrt(1-2x) from a thermal flux plus a parallax distance is a g00 effect, not a bending effect, so {z, R_inf} solves {M, R} with no light bending at all - and it is about a factor of three short. The first experiment to probe the core at all, Pound & Rebka (1960) at x = 2.45e-15, is pure g00 but reaches only the first-order term AT shares with GR. OUTPUT: BOUNDARY (verdict computed, not typed - G_027); the missing ingredient is COMPACTNESS PRECISION, not signal. No reclassification; D_040 untouched; 6/6 PASSED. DERIVED / BOUNDARY (ResearchY-G_036)."),

            new("temporal-independence-audit",
                 "Temporal Independence Audit \u2014 which G-results survive if the conformal metric is removed?",
                 "SURVIVES: of 36 group-G suites, 25 need g00 only (the entire density era G_001-G_020), 8 need g_ij and 3 require conformal flatness. The boundary is exactly G_020 -> G_021 (G_021 is the first audit to ask for light bending) and the minimal time sector is 1 metric function + 1 scalar + 1 exponent, with no eta",
                 [
                     new("THE CLASSIFICATION", "SURVIVES (g00 only) 25 of 36; BOUNDARY (g_ij, conformality not required) 8; REFUTED (requires A = B) 3", "the axis maps 1:1 onto the output labels, so the verdict is not a judgement call: a result requiring only g00 cannot be affected by removing conformality, because it never refers to the spatial block at all"),
                     new("THE SHARP BOUNDARY", "EVERY audit from G_001 to G_020 is g00-ONLY - the source law, the clock law, control and realizability, SI magnitudes, calibration, the suppression mechanism, the actuation chain, mass-independence and watch-ontology, the second-order signature, the neutron-star redshift", "the spatial requirement begins EXACTLY at G_021 (Light-Propagation), the first audit to ask for light bending; G_020 -> G_021 IS the conformal boundary (index 23 of 36, temporal era 22 entries), computed never chosen"),
                     new("THE MINIMAL TIME SECTOR", "1 metric function (g00 = -rho^(2/d)) + 1 scalar (rho) + 1 exponent (1/d, d = 3 by rotation self-duality, M_013)", "NO spatial metric, NO conformal factor, NO reference eta. The time half is ONE EQUATION ON ONE SCALAR"),
                     new("THE SOURCE LAW IS THE CLOCK LAW", "A = 0.5 ln(-g00) = sigma = (1/d) ln rho (the clock potential, = Phi/c^2); dtau/dt = sqrt(-g00) = rho^(1/d); a = -(1/d) grad ln rho = -grad A", "the source law is the NEGATIVE gradient of the time exponent, exactly as an attractive force requires (G_028). Verified on a PEAKED profile: acceleration points INWARD with the peak as an equilibrium"),
                     new("THE ARITY PROOF", "the temporal observables are functions whose SIGNATURE CONTAINS NO B: G00(rho), ClockOf(rho), ClockPotentialOf(rho), RedshiftOf(rho), SourceAccelerationOf(rhoField), SecondOrderRatioOf(x)", "the spatial observables DO take B: GammaOf(a, b), AdmittedBand(x), SurvivorB(x). A RESULT THAT CANNOT BE CALLED WITH B CANNOT REQUIRE B"),
                     new("THE CHECK IS HONEST ABOUT ITS LIMITS", "the live scan strips COMMENTS AND STRING LITERALS so a narrative mention cannot mask a dependency", "TWO PATTERNS WERE REMOVED AFTER MEASUREMENT: 'double b' matched a LEAST-SQUARES SLOPE (G_006) and a TRIDIAGONAL PARAMETER (G_007, G_016b), and 'K(' matched any method. Bare 'psi' is EXCLUDED ON PURPOSE - it names the WAVEFUNCTION in the density era (|psi|^2 = rho) and the metric's traceless face elsewhere. RULE: a token qualifies only if it cannot plausibly mean anything else"),
                     new("TWO EXPLICIT TRIAGE ENTRIES", "G_025 and G_033 declare ScanDetectsIt = false WITH WRITTEN REASONS", "G_025's dependence is an EXPONENT in rho*exp(-d*psi/(d-1)) and psi is unusable as a marker; G_033 contains NO metric vocabulary at all (0/0/0) because its argument is group-theoretic (T2g). An auditable triage rather than a silent exception"),
                     new("HARD AND SOFT DIRECTIONS", "HARD (asserted): a ClockOnly result must have ZERO executable spatial references - holds for all 25", "SOFT: a spatial/conformal result should show at least one, unless explicitly triaged. The HARD direction is the one that defends the audit's claim"),
                 ],
                 "SURVIVES. Removing the conformal metric leaves 25 of 36 group-G results untouched, because they need g00 = -rho^(2/d) and nothing else. THE CLASSIFICATION maps 1:1 onto the verdict labels: ClockOnly -> SURVIVES (25), Spatial -> BOUNDARY (8), ConformalFlatness -> REFUTED (3) - so a result requiring only g00 cannot be affected by removing conformality, as it never refers to the spatial block. THE SHARP BOUNDARY: EVERY audit from G_001 to G_020 is g00-only (the source law, the clock law, control and realizability, SI magnitudes, calibration, the relaxation mechanism, the actuation chain, mass-independence, watch-ontology, the second-order signature and the neutron-star redshift), and the spatial requirement begins EXACTLY at G_021 (Light-Propagation), the first audit to ask for light bending. G_020 -> G_021 IS the conformal boundary, computed from the registry (index 23 of 36; temporal era 22 entries). THE MINIMAL TIME SECTOR is 1 metric function (g00 = -rho^(2/d)) + 1 scalar (rho) + 1 exponent (1/d, d = 3 by rotation self-duality, M_013), with NO spatial metric, NO conformal factor and NO reference eta: the theory's time half is ONE EQUATION ON ONE SCALAR. The source law and the clock law are ONE STATEMENT - A = 0.5 ln(-g00) = sigma = (1/d) ln rho is the clock potential (Phi/c^2), dtau/dt = sqrt(-g00) = rho^(1/d) is the clock, and a = -(1/d) grad ln rho = -grad A is the source law, the NEGATIVE gradient of the time exponent exactly as an attractive force requires (G_028); on a PEAKED profile the acceleration is verified to point INWARD with the peak as an equilibrium. THE ARITY PROOF is structural rather than numerical: the temporal observables are functions whose signature contains no B (G00, ClockOf, ClockPotentialOf, RedshiftOf, SourceAccelerationOf, SecondOrderRatioOf) while the spatial ones take B explicitly (GammaOf, AdmittedBand, SurvivorB) - and a result that cannot be CALLED with B cannot REQUIRE B. THE MECHANICAL CHECK IS HONEST ABOUT ITS LIMITS: it strips comments AND string literals (so a narrative mention cannot mask a dependency), it removed TWO patterns after measurement because 'double b' matched a least-squares slope (G_006) and a tridiagonal parameter (G_007, G_016b) while 'K(' matched any method, and it excludes bare 'psi' on purpose because that token names the WAVEFUNCTION in the density era (|psi|^2 = rho, G_014) and the metric's traceless face elsewhere - the rule adopted being that a token qualifies as a marker only if it cannot plausibly mean anything else. TWO claims declare ScanDetectsIt = false with WRITTEN REASONS (G_025: an exponent in rho*exp(-d*psi/(d-1)); G_033: no metric vocabulary at all, a group-theoretic T2g argument), and the check runs in two directions of different strength - HARD (asserted): a ClockOnly result must show ZERO executable spatial references, which holds for all 25; SOFT: a spatial/conformal result should show at least one unless explicitly triaged. WHAT THIS BUYS: G_031 showed conformal flatness IS the counting measure and G_032 showed it is an ASSUMED PRIMITIVE eta whose imposition is REFUTED (gamma = -1, Cassini 8.6957e4 sigma), so G_035 establishes that THE CONFORMAL PROBLEM IS CONFINED TO THE SPATIAL SECTOR AND THAT AT'S TIME HALF IS NOT HOSTAGE TO ITS SPACE HALF. OUTPUT: SURVIVES (verdict computed, not typed - G_027). DERIVED / BOUNDARY / REFUTED (ResearchY-G_035)."),

            new("a0-robustness-audit",
                 "A0 Robustness Audit \u2014 which gravity/time conclusions depend on A0?",
                 "REFUTED for six pinned values, UNCHANGED for every conclusion: the exact-double keying is REPLACED by tolerance clustering (the rule D_047 documents), the corrected D96^3 spectrum matches D_047 on 5/5 independent figures, and of 15 G-series claims consuming an A0-derived quantity 7 are UNCHANGED, 2 BOUNDARY and 6 REFUTED",
                 [
                     new("THE REPLACEMENT, APPLIED", "SpectralCaseCatalog.D96CubedBreakdown keyed its 49^3 triple sums on EXACT binary64 equality; it now CLUSTERS at a tolerance", "the tolerance is not invented - it is the rule D_047 already documents and applies: 'private const double TolCube = 1e-8; // 3-factor sums carry ~1e-14 noise; tol above it'"),
                     new("FIVE INDEPENDENT FIGURES MATCH", "A0 = 16 080 (D_047: 16 080); A0(m>1) = 16 079 (16 079); max multiplicity = 738 (738); lock release = 4.165691 (4.16569); free room SUM(m-1) = 868 656 (868 656)", "5/5 MATCH, plus M_012's independently reported 16 080. The G-series is now consistent with the D-series instead of contradicting it"),
                     new("ROOT CAUSE - A MIXED EQUALITY DISCIPLINE", "D96's own spectrum arrives via a real TOLERANT eigensolver (A0 = 45, lock 0.802314, free room 51); D96^3's construction used exact-double dictionary keying", "the SAME quantity under TWO equality rules inside one programme - which is why D96's invariants never moved while the cube's did"),
                     new("WHAT MOVED", "A0 20 812 -> 16 080 (-22.7%); A0(m>1) 20 811 -> 16 079; free room 863 924 -> 868 656 (+0.55%); latent fraction L 0.976477 -> 0.981825 (+0.55%); lock release 3.948614 -> 4.165691 (+5.50%); max multiplicity 562 -> 738 (+31.3%)", "THE ASYMMETRY MATTERS: the count moves 23% while the fractions move 0.55% - which is why conclusions survive while counts do not"),
                     new("REFUTED - SIX PINNED VALUES", "G_002's A0 = 20 812; G_002's max multiplicity 562; G_003's D96^3-vs-D96 density delta 0.276596 -> 0.263158; G_005's cube lock 3.948614 -> 4.165691; G_005's cube L 0.97648 -> 0.981825; G_006's restatement", "measured IN THEIR OWN TOLERANCES: G_003 moved 13 438x, G_005's lock 21 708x, G_005's L 535x"),
                     new("UNCHANGED - WHY NOTHING COLLAPSES", "the cube's free fraction > 0.97 (holds at 0.981825 AND at 0.97648); free room = N - A0 = the D_048 latent fraction L (an algebraic identity); the cube's free room exceeds D96's; D96's A0 = 45, lock 0.802314, contraction 33.78", "the ordering arrangement > degeneracy > lattice-average > survivor compression is intact and the regime is still CONTROLLABLE. THE REFUTED ITEMS ARE NUMBERS, NOT CONCLUSIONS"),
                     new("THE BLAST RADIUS, MEASURED", "G_002/G_003/G_005/G_006 - four assertions updated; G_033 - live scanner now excludes TWO meta-audits (G_033, G_034); T_012 - headline UNCHANGED (D96^3 -> S-infinity = 16; sub-claim 21 -> 14); T_013 - assertions pass, A = 16 079, C = 1004.9, max mult 738, '~150x' -> '~114x'", "T_014, T_008-T_011 and D_047 ALL PASS UNCHANGED - D_047 needed nothing, because it was already correct"),
                 ],
                 "REFUTED for six pinned values; UNCHANGED for every conclusion. The previous audit (G_033) found that A0 = 20 812 eigenspaces for D96^3 was not an invariant but an artifact of binary64 dictionary keying, and registered the replacement as OP1 without applying it, because the keying lived in shared infrastructure. THIS AUDIT APPLIES THE REPLACEMENT AND MEASURES THE FALLOUT. SpectralCaseCatalog.D96CubedBreakdown keyed its 49^3 triple sums on EXACT double equality; it now CLUSTERS at a tolerance - and the tolerance is not invented, it is the rule D_047 already documents and applies ('private const double TolCube = 1e-8; // 3-factor sums carry ~1e-14 noise; tol above it'). THE CORRECTED SPECTRUM REPRODUCES D_047 ON FIVE INDEPENDENT FIGURES - A0 = 16 080, A0(m>1) = 16 079, max multiplicity = 738, lock release = 4.165691, free room SUM(m-1) = 868 656 - and M_012's independently reported 16 080. ROOT CAUSE: a MIXED EQUALITY DISCIPLINE - D96's own spectrum arrives through a real tolerant eigensolver and therefore already carries the tolerant answers (A0 = 45, lock 0.802314, free room 51), while D96^3's construction used exact-double keying; the same quantity, two equality rules, inside one programme. WHAT MOVED: A0 20 812 -> 16 080 (-22.7%), A0(m>1) 20 811 -> 16 079, free room 863 924 -> 868 656 (+0.55%), latent fraction L 0.976477 -> 0.981825 (+0.55%), lock release 3.948614 -> 4.165691 (+5.50%), max multiplicity 562 -> 738 (+31.3%) - and the ASYMMETRY is the point: the count moves 23% while the fractions move 0.55%, which is why conclusions survive while counts do not. FIFTEEN G-SERIES CLAIMS CONSUME AN A0-DERIVED QUANTITY, CLASSIFIED BY COMPUTATION: REFUTED (6) - the two G_002 counts, G_003's density delta 0.276596 -> 0.263158, G_005's cube lock 3.948614 -> 4.165691, G_005's cube L 0.97648 -> 0.981825, and G_006's restatement; measured in their own tolerances G_003 moved 13 438x, G_005's lock 21 708x and G_005's L 535x. UNCHANGED (7) - the cube's free fraction > 0.97 (true at 0.981825 AND at 0.97648), G_006's L > 0.97, free room = N - A0 = the D_048 latent fraction L (an algebraic identity, true for either A0), the cube's free room exceeds D96's, D96's A0 = 45, D96's lock 0.802314, D96's contraction 33.78. BOUNDARY (2) - the quoted '97.6% energy-free' -> 98.2% and the cube's contraction at m = 200, 7.33 -> 7.10. TOTAL 7 / 2 / 6. WHY NOTHING COLLAPSES: ConclusionsSurvive() = true, CHECKED ON BOTH KEYINGS rather than asserted - the cube is still ~98% energy-free, free room is still exactly N - A0 and still equals the latent fraction, D96's own invariants never moved, the ordering arrangement > degeneracy > lattice-average > survivor compression is intact, and the regime is still classified CONTROLLABLE. THE REFUTED ITEMS ARE NUMBERS, NOT CONCLUSIONS. THE BLAST RADIUS, MEASURED NOT ASSUMED (the change is in SHARED infrastructure): G_002/G_003/G_005/G_006 - four assertions updated to the corrected values; G_033 - its live scanner now excludes TWO meta-audits (G_033 and G_034) that compute the spectrum in order to audit it; T_012 - headline UNCHANGED (D96^3 still gives S-infinity = 16, distance 3 from 19, and random 17 is still nearest), only the sub-claim 'the cubic 3-axis sector gives 21 (distance 2)' moved to 14 (distance 5); T_013 - assertions all pass, quoted figures updated (A = 16 079 was 20 811, C = 1004.9 was 1300.7, max multiplicity 738 was 562, '~150x' -> '~114x'); T_014, T_008-T_011 and D_047 ALL PASS UNCHANGED - D_047 needed nothing, because it was already correct. OUTPUT: REFUTED - six pinned G-series values were artifacts and fail at their own stated precision (up to 21 708x their tolerance); SCOPE: no G-series conclusion breaks. The verdict is COMPUTED from the claim registry (G_027). DERIVED / BOUNDARY / REFUTED (ResearchY-G_034)."),

            new("cubic-substrate-audit",
                 "Cubic Substrate Audit \u2014 does gravity/time need D96^3 too?",
                 "PARTIAL: gravity structurally REQUIRES the cubic substrate (the spatial metric's traceless part needs the dim-3 irrep T2g, which a single D96 ring cannot host), but the metric era is substrate-free because the requirement was absorbed into the primitive eta (G_032). Also finds A0 = 20812 is a binary64-keying artifact; the robust value is 16080",
                 [
                     new("LIVE INVENTORY (34 suites)", "the core re-reads the group-G sources at TEST TIME (G_027 pattern), separating code from comments; Y_G_033 excludes itself as self-referential", "20 reference D96 in code (the density era); 1 comment-only (G_001); 13 with NO reference at all \u2014 exactly the metric / closure era: G_015, G_017, G_019-G_022, G_025, G_027-G_032"),
                     new("THE REQUIREMENT IS INHERITED", "spatial metric g_ij = 6 components = 1 (trace, l=0) + 5 (traceless, l=2)", "subduction onto the octahedral rotational group O (order 24, dims 1/1/2/3/3, sum d^2 = 24) reproduces M_012 EXACTLY: norms^2 = 1,1,2,3,4; l=1 -> T1(3) (vector); l=2 -> E(2) + T2(3). So gravity needs a dim-3 irrep \u2014 T2g \u2014 that a single D96 ring (cycle graph C_96, symmetry D_96, irreps of dim 1-2 only) CANNOT supply"),
                     new("WHY THE METRIC ERA IS SUBSTRATE-FREE", "it IMPORTS 3D space through the primitive eta (G_032; QG289: 'a TRUE THEORY INPUT ... part of the geometry, not a choice')", "G_032 and G_033 are ONE FACT SEEN TWICE: G_032 says the shape of space is an input; G_033 says that input is precisely what the cubic lattice would otherwise have supplied. The requirement was DISCHARGED BY A PRIMITIVE, not removed"),
                     new("DIMENSION 3 IS DERIVED", "rotation self-duality d(d-1)/2 = d", "the UNIQUE solution is d = 3 (1->0, 2->1, 3->3, 4->6), the same reason M_013 gives"),
                     new("DEFECT: A0 = 20 812 IS NOT AN INVARIANT", "the cited D96^3 eigenspace count is the count of distinct BINARY64 sums from a Dictionary<double,int> keying", "exact-double keying gives 20 812 under .NET (Math.Cos) and 20 440 under Python (math.cos) for the SAME IEEE-754 algorithm; a one-ulp (1e-16) perturbation of the 1D spectrum swings it over 20 488 ... 21 369 (range 881)"),
                     new("THE ROBUST VALUE IS 16 080", "tolerance-clustered count: plateau at 7-10 decimal places, spread EXACTLY 0 under 1e-16 AND 1e-13 noise", "16 080 is M_012's OWN reported figure AND the value the repository uses elsewhere: NewChat_Start records 'D96^3 136 005 of 868 656', and 868 656 = 884 736 - 16 080. THE TWO LINEAGES NEVER MET"),
                     new("WHAT SURVIVES", "A0 = 20 812 -> free room 863 924 -> L = 0.976477   versus   A0 = 16 080 -> free room 868 656 -> L = 0.981825", "SURVIVES: the qualitative claim (D96^3 is ~98% energy-free; landscape ~150x larger; compression dominated by A). DOES NOT SURVIVE: 20 812, 863 924, L = 0.97648, and the lock-release 3.948614 derived from them. NOT FIXED HERE DELIBERATELY: the keying is shared infrastructure consumed by other research groups, so correcting it renumbers A0 programme-wide"),
                 ],
                 "PARTIAL. The programme established that D96 (x) D96 (x) D96 is required to explain physics: M_012 proved a genuine 3D sector exists on the cubic lattice but is ABSENT from a single D96 ring (max irrep dim 2). Does the gravitation/time search (group G) show the same behaviour? A LIVE SCAN re-reading the group-G sources at test time (the G_027 pattern, with code separated from comments, and Y_G_033 excluding itself as self-referential) finds a clean split among 34 suites: 20 reference D96 in code (the density era) and 13 with NO reference at all \u2014 exactly the metric / closure era (G_015, G_017, G_019-G_022, G_025, G_027-G_032), which are pure continuum PPN calculations. STRUCTURALLY, however, the requirement is INHERITED, not eliminated: gravity's central object is the spatial metric, a symmetric rank-2 tensor in d = 3 with 6 components = 1 (trace, l = 0) + 5 (traceless, l = 2); subducting the O(3) irreps onto the octahedral rotational group O (order 24, dims 1/1/2/3/3, sum d^2 = 24) reproduces M_012 exactly (norms^2 = 1,1,2,3,4; l = 2 -> E(2) + T2(3)), so the metric's traceless part needs the dimension-3 irrep T2g \u2014 exactly what a single D96 ring (a cycle graph C_96 whose symmetry D_96 has irreps of dimension 1-2 only) cannot supply, while the cube can. Gravity therefore needs the cubic substrate as much as the vector sector does, and d = 3 is derived for the same reason M_013 gives (rotation self-duality d(d-1)/2 = d has the unique solution 3). WHERE DID IT GO? Into the primitive eta: the metric era imports 3D space through the conformal reference metric that G_032 proved is an assumed input (QG289: 'a TRUE THEORY INPUT ... part of the geometry, not a choice'). G_032 and G_033 are ONE FACT SEEN TWICE \u2014 the shape of space is an input, and that input is precisely what the cube would otherwise have supplied. The requirement was DISCHARGED BY A PRIMITIVE, not removed. A DEFECT WAS ALSO FOUND: the figure A0 = 20 812 eigenspaces that group G cites for D96^3 (G_002, G_005, G_006 and downstream) is NOT an invariant \u2014 it is the count of distinct binary64 sums from the implementation's exact-double dictionary keying, so it depends on the last bits of Math.Cos. The same IEEE-754 algorithm gives 20 812 under .NET and 20 440 under Python, and a one-ulp perturbation swings it over 20 488 ... 21 369 (range 881). The TOLERANCE-CLUSTERED count is stable \u2014 plateau at 7-10 decimal places, spread exactly 0 under 1e-16 and 1e-13 noise \u2014 and equals 16 080, which is M_012's own independently reported figure AND the value the repository uses elsewhere (NewChat_Start: 'D96^3 136 005 of 868 656'; 868 656 = 884 736 - 16 080). The two lineages never met. The qualitative conclusion survives (D96^3 is about 98% energy-free); the specific figures (20 812, 863 924, L = 0.976477 versus 16 080, 868 656, L = 0.981825) and everything computed FROM A0, including the lock release 3.948614, do not. NOT FIXED HERE DELIBERATELY: the keying lives in shared infrastructure (SpectralCaseCatalog, DensityField) consumed by other research groups, so correcting it would renumber A0 programme-wide \u2014 a programme-level decision, registered as an open item. Output: PARTIAL (same in structure, different in locus; verdict computed, not typed \u2014 G_027). DERIVED / BOUNDARY / REFUTED (ResearchY-G_033)."),

            new("conformal-assumption-audit",
                 "Conformal Assumption Audit \u2014 is g = Omega^2 eta derived, or only assumed?",
                 "ASSUMED (BOUNDARY): the ansatz first becomes necessary at the rho -> metric step (the TENSOR face) and the imported object is eta, AT's second primitive \u2014 the forcing set is EMPTY, and imposing it forces gamma = -1, Cassini-excluded at 86957.4348 sigma",
                 [
                     new("WHERE IT ENTERS", "step 1 Difference -> Counting: bookkeeping only, NO metric content; step 2 Counting -> rho (the SCALAR face): no reference tensor needed", "step 3 rho -> metric (the TENSOR face) is the FIRST step that requires it \u2014 QG285/QG288: 'the tensor face REQUIRES eta [conformal reference]'; the chain is METRIC-FREE for its first two steps"),
                     new("THE IMPORTED OBJECT", "eta \u2014 the conformal reference metric", "QG289/AnchorInventoryAudit (QG77) classifies it as a TRUE THEORY INPUT: 'the conformal reference metric (g = rho^(2/d) eta) \u2014 the framework's flat reference, not a measured value; part of the geometry, not a choice'. G_031 said the ansatz is assumed; G_032 names its owner"),
                     new("THE CRITERION, PROVED", "g = Omega^2 eta <=> A = B", "(<=) immediate; (=>) in area gauge e^(B_area) = 1/(1 + rB') and R A_R = rA'/(1 + rB'), so it holds iff B' = A'. Executed sweep: 0 mismatches"),
                     new("INDEPENDENT WEYL CONFIRMATION", "W = C_abcd C^abcd", "flat 0.0; A = B = -c/r -> 1.0e-30; A = B = -c/r^2 -> 1.6e-29; A = -c/r with B = A/2 -> 0.6937; A = 0 with B = -c/r -> 15.39; the G_029 SURVIVOR -> 0.2537"),
                     new("THE SURVIVOR IS NOT CONFORMALLY FLAT", "G_029's g_rr = 2 - rho^(2/d)", "area-gauge deficit = -1.5x^2 + (13/6)x^3 - (29/8)x^4 + ... \u2014 conformal to FIRST order, violated at SECOND; verified against the closed form to 2.4e-12 relative. NOTE: the direct difference suffers ~12 digits of cancellation; the audit uses the conjugate form with a series numerator"),
                     new("NO PRIMITIVE FORCES IT", "the forcing set is EMPTY", "setting eta aside \u2014 it IS the conformal statement, so citing it RESTATES the assumption \u2014 every remaining primitive is rank 0 (rho, psi, d, pi). A scalar can only be read into a conformal factor, never into a SHAPE. Component accounting: 10 -> 1, DISCARDING 9; AT supplies 0 of the 5 traceless components"),
                     new("COUNTING IS A DETERMINANT, NOT A SHAPE", "sqrt(det g_ij) = rho fixes only the PRODUCT of the eigenvalues", "diag(rho^2,1,1) and diag(rho^(2/3),rho^(2/3),rho^(2/3)) share IDENTICAL sqrt(det) = rho but differ in conformal status (the anisotropic one is conformal only at rho = 1). With A = sigma fixed: conformality <=> counting measure <=> B = sigma, 0 mismatches over 4001 densities"),
                     new("IMPOSING IT IS REFUTED", "A = B + A = sigma => B = sigma => gamma = -1 EXACTLY at every body", "Cassini separation 86957.4348 sigma with ZERO deflection and ZERO Shapiro. The admitted band, by EXACT inversion B(x,gamma) = 0.5 ln(1 + gamma(1 - e^(-2x))) with no sampling, is ~1.84e-10 wide at the solar point and sits at ~ +x = -sigma while conformality demands +sigma: OPPOSITE SIDES OF ZERO. Required violation: a FACTOR OF 2.000013. gamma = -1 is NEVER inside the band"),
                 ],
                 "BOUNDARY. The conformal ansatz g = Omega^2 eta is ASSUMED, not derived. Tracing Difference -> Counting -> rho -> metric, the first two steps carry no metric content at all: step 1 is bookkeeping, and step 2 produces the SCALAR face of the one Difference, which needs no reference tensor (QG288: 'the scalar face does not [require eta]'). The ansatz first becomes NECESSARY at step 3, the rho -> metric step, where a rank-2 object must be built from scalar primitives \u2014 and that is exactly where eta, AT's second primitive, is imported (QG285/QG288: 'the tensor face requires eta'). QG289's anchor inventory already classifies eta as a TRUE THEORY INPUT. The criterion is proved exactly: in the G-group's gauge g = Omega^2 eta <=> A = B, by two lines of chain rule in area gauge, and confirmed independently by the Weyl invariant (1e-30 for A = B versus 0.25-15.4 for A != B). The G_029 survivor consequently is NOT conformally flat: its deficit is -1.5x^2 + (13/6)x^3, conformal to first order and violated at second. NO PRIMITIVE FORCES IT: setting eta aside as self-referential, the forcing set is empty, because every remaining primitive is rank 0 and a scalar cannot determine a shape; the 10 metric components reduce to 1 conformal function, discarding 9, and AT supplies 0 of the 5 traceless components. Counting is a determinant condition, not a shape condition (identical sqrt(det) = rho, different conformal status), and with A = sigma fixed the three names coincide. DECISIVE: imposing the ansatz forces B = sigma, hence gamma = -1 exactly, excluded by Cassini at 86957.4348 sigma with zero deflection and Shapiro; the band sits on the opposite side of zero and conformality must be violated by a factor of 2.000013. AT's own metric form g = rho^(2/d) eta IS that pinned case. So if the ansatz were entailed, AT would be refuted \u2014 being unentailed is what keeps it viable as a choice, which is precisely what makes it an assumption. This restates G_030's dilemma as CONFORMAL FLATNESS VERSUS OPTICS and locates G_031's theorem's premise in eta. Output: BOUNDARY (verdict computed, not typed \u2014 G_027). DERIVED / BOUNDARY / REFUTED (ResearchY-G_032)."),

            new("spatial-origin-audit",
                 "Spatial-Origin Audit \u2014 where does B = sigma enter the theory?",
                 "NOWHERE: B = sigma is ENTAILED, not assumed \u2014 with g_uv = rho^(2n) eta_uv the statements n = 1/d, the clock law sqrt(-g00) = rho^(1/d) and the counting measure sqrt(det g_ij) = rho are THE SAME EQUATION (0 mismatches over a 401x401 sweep)",
                 [
                     new("THE PROVENANCE THEOREM, EXECUTED", "g_uv = rho^(2n) eta_uv gives A = B = n ln rho, sqrt(-g00) = rho^n and sqrt(det g_ij) = rho^(n d)", "conformal flatness (A = B) is BUILT INTO the ansatz, so only n is free \u2014 and in d = 3 the clock law and the counting measure are two readings of ONE exponent choice from two slots already made equal. Sweep 401x401 in (n, rho): ZERO MISMATCHES"),
                     new("THE EXCLUDED POINT IS FLAT SPACE", "rho = 1 gives A = B = 0 for EVERY exponent, so all exponents agree trivially", "the point carries no information about the equivalence; it is excluded explicitly and SEPARATELY ASSERTED to be degenerate \u2014 not silently dropped"),
                     new("PROVENANCE OF EVERY STEP", "Difference -> Density: DERIVED (trace/traceless, QG285/286/292); Density -> Metric: ASSUMED (the metric ansatz QG207 \u2014 the source of A = B); the exponent n = 1/d: ASSUMED (this IS the clock law)", "Metric -> Spatial measure: DERIVED (algebraic identity rho^(n d) = rho at n = 1/d); rho read as a geometric volume: CORRESPONDENCE (G_018's provenance asymmetry). So the measure is DERIVED ALGEBRAICALLY, FROM AN ASSUMED EXPONENT, UNDER A CORRESPONDENCE READING"),
                     new("AN ALTERNATIVE EXISTS \u2014 BUT NOT WITHIN THE ANSATZ", "an alternative needs B != sigma, which under conformal flatness forces A != B \u2014 the traceless face psi", "psi is not a new primitive TYPE (G_024/QG285) but IS an unconstrained degree of freedom, so the metric becomes underdetermined \u2014 exactly G_030's result. The two audits AGREE"),
                     new("FIVE LOAD-BEARING USES, TWO NEUTRAL", "LOAD-BEARING: the metric ansatz; N = int rho dV (QG194/QG222); deficit accounting Sum rho = 1 (QG181/QG182); horizon area -> entropy S = A/4 (QG185/QG259); cosmological densities Omega, n_s", "NEUTRAL: |psi|^2 = rho (QG216, a probability rather than a volume reading) and the RAR scale g_dagger = c H0/(2 pi) (QG080, no volume element)"),
                     new("WHAT BREAKS, QUANTIFIED", "geometric-to-count volume ratio sqrt(det g_ij)/rho for the surviving alternative", "1.000012735 (Sun), 1.000600120 (x = 1e-4), 1.733052388 (x = 0.1), 3.437584871 (J0740+6620), 51.142808724 (x = 1): FOUR breakages at magnitude 2.44 \u2014 count conservation, deficit accounting, the area/entropy chain, the cosmological densities"),
                     new("RENARRATION OF G_023 \u2014 ONE PIN, NOT TWO", "AT does NOT pin k = 0 twice: conformal flatness supplies A = B; ONE exponent choice supplies both values", "the counting measure was never an independent postulate. So G_030's dilemma SHARPENS: not 'counting measure vs optics' but CONFORMAL FLATNESS + CLOCK LAW vs OPTICS \u2014 reducing the theory's difficulty to the two things that ARE assumed"),
                 ],
                 "REQUIRED. B = sigma does not enter the theory as an assumption anywhere. Write the conformal ansatz g_uv = rho^(2n) eta_uv: it gives A = B = n ln rho, sqrt(-g00) = rho^n and sqrt(det g_ij) = rho^(n d); conformal flatness (A = B) is built into the ansatz, so only n is free, and in d = 3 the three statements n = 1/d, the clock law and the counting measure are THE SAME EQUATION \u2014 two readings of one exponent choice from two slots conformal flatness already made equal. Verified by an exhaustive 401x401 sweep of (n, rho): ZERO MISMATCHES, with the flat-space point rho = 1 excluded explicitly and separately asserted degenerate. There is no separate 'spatial measure' step to find or remove. Provenance: Density DERIVED (QG285/286/292), the metric ansatz ASSUMED (QG207), the exponent ASSUMED (it IS the clock law), the measure DERIVED (algebra at n = 1/d), rho-as-volume CORRESPONDENCE (G_018). An alternative measure exists but requires B != sigma, hence A != B, hence the traceless face psi \u2014 not a new primitive type but an unconstrained degree of freedom, exactly G_030's underdetermination. The measure is load-bearing in FIVE places (the ansatz; N = int rho dV; deficit accounting; horizon area -> entropy; cosmological densities) and neutral in two; removing it breaks those four chains by up to 3.437584871x at J0740+6620. This RENARRATES G_023: there are not two pins \u2014 one exponent choice, enabled by conformal flatness. The measure is required for a reason the programme had not understood: it is what the clock law already says. Output: REQUIRED \u2014 the verdict is COMPUTED, not typed (G_027). DERIVED / BOUNDARY / REFUTED (ResearchY-G_031)."),

            new("no-go-audit",
                 "No-Go Audit \u2014 must every local B(r) = F(sigma) violate a requirement?",
                 "NO-GO PROVED: six of the eight constraints are INERT;  gamma = -1 <=> B = sigma (the counting measure, AT's only derivation, excluded at 8.6957e4 sigma) and gamma = +1 <=> g_rr = 2 - rho^(2/d) (underivable) \u2014 there is NO THIRD",
                 [
                     new("SIX OF EIGHT ARE INERT", "Newton, Earth clock, GPS, neutron-star audit and the clock sector depend only on A (untouched by B); light deflection is (1+gamma)/2", "so light deflection is the SAME TEST as Cassini, and the live pair is {Cassini/deflection} vs {no-new-primitive}"),
                     new("TWO EXACT EQUIVALENCES", "B(gamma) = 0.5 ln(1 - gamma(e^(2A) - 1)) gives B(-1) = sigma and B(+1) = 0.5 ln(2 - e^(2A))", "(i) gamma = -1 <=> B = sigma (the counting measure sqrt(det g_ij) = rho); (ii) gamma = +1 <=> e^(2B) = 2 - e^(2A), i.e. g_rr = 2 - rho^(2/d) = 2 - |g00|. Both verified by SOLVING, not assertion"),
                     new("THE PROOF", "F = Id (the counting measure, AT's ONLY determination of B from rho) -> B = sigma -> gamma = -1 -> 8.6957e4 sigma outside the Cassini band [0.9999520, 1.0000900], deflection IDENTICALLY 0", "F != Id places no AT-derived value in B, so F's specification is an input beyond rho = a NEW PRIMITIVE (G_023's accounting). Every other ingredient fixes A, fixes numbers (occ, Sum-m, #g) or supplies FREE content (the traceless face psi); none determines B. QED"),
                     new("THE ADMITTED BAND IS A SLIVER, ON THE WRONG SIDE OF ZERO", "at solar compactness it is ~5.5e-10 wide and sits at B ~ +x = -sigma, while the derived value sits at B = sigma = -x", "THE TWO ARE ON OPPOSITE SIDES OF ZERO \u2014 the derived B has the wrong SIGN, not merely the wrong size. Established RIGOROUSLY by exact inversion (no sampling; true at every body) and confirmed by a band-resolving sweep (20001 points over [-2x, 2x] per body, ZERO escapes)"),
                     new("THE DEPARTURE IS NOT A SMALL CORRECTION", "survivor sqrt(det g_ij)/rho", "1.000000004 (Earth, 4.2e-9), 1.000012735 (Sun, 1.3e-5), 1.733052388 (x = 0.1), 3.437584871 (J0740+6620, departure 2.44), 51.142808724 (x = 1): growth beyond 1e5, reaching O(1) EXACTLY where the theory is most discriminating"),
                 ],
                 "NO-GO. With A = sigma fixed by G_028, six of the eight constraints are inert (Newton, Earth clock, GPS, neutron-star audit, clock sector all depend only on A; light deflection is (1+gamma)/2, the same test as Cassini), leaving {Cassini/deflection} against {no-new-primitive} \u2014 and the two cannot both be satisfied. Two EXACT equivalences: gamma = -1 <=> B = sigma (the counting measure, AT's only determination of B from rho) and gamma = +1 <=> e^(2B) = 2 - e^(2A) (g_rr = 2 - rho^(2/d)). F = Id gives gamma = -1, which lies 8.6957e4 sigma outside the Cassini band with zero deflection; F != Id places no AT-derived value in B, making F's specification a new primitive. So every local B(sigma) violates a requirement. The admitted band is ~5.5e-10 wide and sits at -sigma while the derived B = sigma \u2014 OPPOSITE SIDES OF ZERO. The counting-measure departure runs from 4.2e-9 at Earth to 2.44 at J0740+6620. GOAL: a surviving spatial sector is REFUTED AS A DERIVATION (G_029's 'chosen, not derived' elevated to a theorem) and AFFIRMED AS A POSTULATE. CONSEQUENCE: a two-horned dilemma \u2014 keep the counting measure (gamma = -1, excluded) or abandon it (correct optics, but the volume-count identification is replaced by an input the theory does not supply); the choice is about WHICH OF AT'S IDENTIFICATIONS IS PRIMITIVE. Output: NO-GO. DERIVED / BOUNDARY / REFUTED (ResearchY-G_030)."),

            new("spatial-sector-closure",
                 "Spatial Sector Closure \u2014 can any space rule B(r) survive?",
                 "EXACTLY ONE SURVIVES: g_rr = 2 - rho^(2/d) = 2 - |g00| \u2014 the unique member with gamma = +1 at ALL orders;  B = sigma (the ONLY derivable member) is REFUTED at Cassini 8.6957e4 sigma, so the sector CLOSES ON A POSTULATE at a volume cost of 3.437585",
                 [
                     new("WHAT THE CONSTRAINTS BIND", "reqs 1-4 depend only on A (fixed by G_028, so EVERY candidate passes); reqs 5-7 depend only on gamma", "so the eight requirements pin ONE NUMBER, not a function \u2014 the entire nonlinear completion of B stays free"),
                     new("CANDIDATE TABLE (gamma at J0740+6620)", "B = sigma -> -1.000000; B = -sigma -> 1.638865; B = 0.5 ln(2 - e^(2 sigma)) -> 1.000000 EXACTLY; B = -sigma - 2 sigma^2 -> 0.350133; rational/exponential -> ~1 + O(x)", "B = sigma: deflection 0, Shapiro 0, vol/rho = 1.000000, REFUTED at Cassini 8.6957e4 sigma. Exact member: defl/GR = Shapiro/GR = 1.000000, vol/rho = 3.437585. At the solar point (x = 4e-6) B = sigma gives gamma - 1 = -2 (8.6957e4 sigma), the linear member 0.565 sigma"),
                     new("THE UNIQUE SURVIVOR", "e^(2B) = 2 - e^(2A)  <=>  g_rr = 2 - rho^(2/d) = 2 - |g00|", "the spatial metric is the REFLECTION of the temporal one about 1 (the conformal gamma = -1 member is g_rr = |g00|); to first order g_rr = 1 + 2x, the GR value"),
                     new("NON-DEGENERACY, CORRECTED HERE", "2 - e^(2A) vanishes only at A = +0.5 ln2, i.e. x = -0.5 ln2 = -0.3465735903", "a NEGATIVE compactness (repulsive object), so the exact member is non-degenerate everywhere PHYSICAL with g_rr in (1, 2) for every x > 0, tending to 2 as x -> infinity: space uniformly STRETCHED. The first derivation in this audit claimed a degeneracy at R = 2.8854 GM/c^2 \u2014 a sign error, caught by the tests and corrected before committing"),
                     new("AT DERIVES NO B", "the counting measure \u2014 AT's ONLY equation for B \u2014 yields B = sigma, the excluded member", "so the survivor is selected by OBSERVATION: counting measure -> gamma = -1 -> REFUTED; GR optics -> g_rr = 2 - rho^(2/d) -> gamma = +1 -> SURVIVES at a volume cost of 3.437585, which REPRODUCES G_023's recorded 3.437585 exactly (computed 3.437584871)"),
                     new("THE COMPLETION IS UNSELECTED", "Cassini bounds the quadratic coefficient of B = -sigma + c2 sigma^2 only by |c2 + 2| <= 22.5 at 3 sigma", "a sweep of c2 from -20 to +18 keeps most of its range inside \u2014 the requirements pin gamma, NOT B"),
                 ],
                 "EXACTLY ONE SPACE RULE SURVIVES, AND IT IS CHOSEN, NOT DERIVED. With A = sigma fixed by G_028, the eight requirements pin ONE NUMBER rather than a function: reqs 1-4 depend only on A (passed by every candidate) and reqs 5-7 only on gamma. Exactly one member gives gamma = +1 at ALL orders: g_rr = 2 - rho^(2/d) = 2 - |g00|, the reflection of the temporal metric about 1, non-degenerate everywhere physical with g_rr in (1, 2). But AT derives no B: its only equation, the counting measure, yields B = sigma, which Cassini excludes at 8.6957e4 sigma (gamma = -1, deflection 0, Shapiro 0). The survivor is therefore selected by OBSERVATION, at an exact price already known from G_023 \u2014 the volume measure becomes 3.437585x the count at J0740+6620 (computed here as 3.437584871). The completion stays unselected (|c2 + 2| <= 22.5 at 3 sigma). CRITICAL: all derivable B fail, so THE SPATIAL SECTOR IS CLOSED AS A DERIVATION; exactly one survives the strengthened requirement, so THE SECTOR CLOSES ON A POSTULATE. Output: SURVIVES / BOUNDARY / REFUTED. DERIVED / BOUNDARY / REFUTED (ResearchY-G_029)."),

            new("clock-sector-closure",
                 "Clock Sector Closure \u2014 is the clock rho^(1/d) or rho^(1/d)*exp(psi)?",
                 "CLOCK CLOSED on dTau/dt = rho^(1/d):  the potential IS the time exponent, so psi CANNOT be nonzero without changing the source law;  gamma = +1 fixes psi = -4 sigma, forcing A = +3x > 0 (a blueshift at every body, 2000 sigma at Earth)",
                 [
                     new("THE ONE FACT", "g00 = -e^(2A), A = sigma + psi, sigma = (1/d) ln rho, Phi/c^2 = A", "so a = -grad Phi = -(1/d) grad ln rho - grad psi: the source law holds IFF grad psi = 0, and a constant psi is only a global time unit"),
                     new("CRITICAL ANSWER", "CAN psi BE NONZERO WITHOUT CHANGING THE SOURCE LAW? NO", "A - sigma = psi IDENTICALLY, so any nonzero psi moves the time exponent \u2014 a proof, not a judgement"),
                     new("THE OPTICS PRICE", "gamma = +1 (A + B = 0) fixes psi = -4 sigma UNIQUELY in the trace-preserving QG207 direction", "then A = -3 sigma = +3x > 0: the potential is POSITIVE, the surface clock runs FAST, and every gravitating body is BLUESHIFTED \u2014 the clock SIGN inverts"),
                     new("z(rho-only)", "Earth +6.957e-10, Sun +2.1225e-6, x=1e-4 +1.0000e-4, J0740+6620 +0.2801817", "attractive everywhere; positivity holds at every body"),
                     new("z(QG207, psi=-4 sigma)", "Earth -2.087e-9, Sun -6.3675e-6, x=1e-4 -3.0000e-4, J0740+6620 -0.5233658", "the sign flips at EVERY body, Earth included; the psi contribution to the acceleration is 4x the rho contribution"),
                     new("FIVE REQUIREMENTS", "rho-only satisfies ALL FIVE", "(1) source law; (2) Earth/GPS at 0.000 sigma; (3) the G_020 neutron-star audit z_AT = 0.2801817; (4) positivity at every body; (5) no new primitive. QG207(psi=-4 sigma) fails 1, 2 (2000.4 sigma), 3 and 4 \u2014 passing only 5"),
                     new("CORRECTION TO G_024 / QG212", "the quoted GPS bound |psi| <= 2e-3 is wrong IN KIND", "it compared the shift e^(4x)-1 = 2.78e-9 against a clock of 1, but the observable is the redshift RELATIVE TO INFINITY, where the conformal law already gives z ~ +x = 6.957e-10. The correct bound is |psi| <= 1.391e-12, i.e. 1.44e9x TIGHTER, and psi = -4 sigma is excluded at 2000 sigma. QG212's 'physical sector' is a REPULSIVE geometry"),
                     new("BOUNDARY \u2014 THE SPATIAL ROUTE", "A held at sigma with B = 0.5 ln(2 - e^(2 sigma)) is the ONLY gamma = +1 that leaves the clock untouched", "satisfies requirements 1-4, fails 5: it leaves the trace-free direction, so the volume measure becomes 3.437585x the count at J0740+6620"),
                     new("THE STRUCTURAL RESULT", "psi and the clock are NOT independent", "every repair of gamma from the trace-free direction repays the debt in the clock, and the repayment is a SIGN INVERSION; the optics fix must be spatial"),
                 ],
                 "CLOCK CLOSED on dTau/dt = rho^(1/d). The Newtonian potential IS the time exponent (g00 = -e^(2A), A = sigma + psi, Phi/c^2 = A), so a = -(1/d) grad ln rho - grad psi and the source law holds IFF grad psi = 0: psi CANNOT be nonzero without changing the source law. Because gamma = +1 fixes psi = -4 sigma uniquely in the trace-preserving direction, it forces A = +3x > 0 \u2014 the potential becomes POSITIVE, the surface clock runs fast, and every gravitating body is blueshifted (J0740+6620 z = -0.5233658 against an observed +0.28; Earth 2000 sigma). The rho-only law satisfies ALL FIVE requirements; the spatial route is the only gamma = +1 that keeps the clock, at a 3.437585 volume cost. This CORRECTS G_024 and the restored QG212: the quoted GPS bound 2e-3 is wrong in kind and the correct bound is 1.391e-12 (1.44e9x tighter), so QG212's 'physical sector' is repulsive and refuted. The closure RELOCATES the failure: the clock is sound and the entire open problem is the SPATIAL sector. Output: CLOCK CLOSED. DERIVED / BOUNDARY / REFUTED (ResearchY-G_028)."),

            new("literal-verdict-audit",
                 "Literal Verdict Audit \u2014 can the rule be broken quietly?",
                 "ENFORCED: no result path may let a classification depend on a literal;  7 paths found, ALL BOUNDARY authored judgements/input taxonomies;  ZERO REFUTED remain (the one found, in the g-dagger coupling audit, is now computed)",
                 [
                     new("THE RULE", "literal boolean (verdict name) -> score / classification / verdict function", "REFUTED unless COMPUTED or adjudicated SAFE / BOUNDARY with a cited reason; FAIL IF literal -> verdict without a computation path"),
                     new("WHY AN ENFORCEMENT", "G_025 and G_026 fixed instances by hand and the class RECURRED FOUR TIMES in the QG ladder alone", "LiteralVerdictAudit RE-READS THE AT.CORE SOURCE AT TEST TIME, so a NEW literal-to-verdict path breaks the build until it is computed or explicitly adjudicated"),
                     new("LOCATION", "parse every record for verdict-vocabulary boolean params BY NAME AND BY POSITION", "(1) by name: `Holds: true`; (2) by position: `..., true, ...`; then require the file to CONSUME the flag from Score/Classify/Verdict/Rank/Assess/Grade/Count \u2014 so a literal nobody scores is inert"),
                     new("INVENTORY \u2014 7 PATHS, ALL BOUNDARY", "each an authored judgement or input taxonomy with its reason cited in place", "Rule.Derivable (meta-level D96 judgement; the audit states neither rule is FORCED); Anchor.IsTrueInput (input taxonomy); Principle.Consistent (Noether row recorded FALSE with reason); MeasurementClass.StructurallyUnique (needs an exhaustive enumeration of reads); DependencyFact.SupportsDerivedFromDifference (each citing its phase); ReorgMember.Correct (vs pre-registered values); FragmentationAttempt.IsValidCharge (reason in place). NONE IS A PHYSICS RESULT"),
                     new("ZERO REFUTED", "the registry STRUCTURALLY FORBIDS a REFUTED entry", "a refuted path is FIXED, not tolerated: a REFUTED disposition is not a state the audit can rest in"),
                     new("THE ONE REFUTED PATH, FOUND AND FIXED", "ResearchQG/GdaggerOriginAnalyzer.cs: evidence flags, ratio AND score were literals at all six sites, and Score feeds OrderByDescending \u2014 THE RANKING", "the Coincidence row was INTERNALLY INCONSISTENT \u2014 RatioToA0 typed 1.0 while its own numerator was NaN. Now DERIVED: HasExactTwoPi = |pred - cH0/(2pi)|/(cH0/(2pi)) < 1e-6; RatioToA0 = pred/a0 (NaN propagates); Matches = |RatioToA0 - 1| <= 0.15 (the criterion the Verdict text states); Score = 3.0 if exact 2pi and matching, else 1.5 if not falsifiable, else 0.5. VERIFIED to reproduce the typed values 3.0/3.0/0.5/0.5/0.5/1.5 \u2014 provenance changed, not physics"),
                     new("REGRESSION RECORD", "FixedByComputation lists the 11 sites fixed by G_025/G_026/G_027", "they no longer SCAN, so a regression \u2014 a literal reappearing \u2014 is caught and CANNOT be excused by the registry; the scanner is tested in both directions (it must fire on the G_025/G_026 fingerprint and be silent when the flag is computed)"),
                     new("SCOPE AND WHAT IT DOES NOT CLAIM", "the scan covers AT.Core; AT.Tests/AT.App/AT.Book are downstream", "the audit does NOT decide whether a verdict is CORRECT \u2014 only whether it is COMPUTED \u2014 and it forces the difference into the open instead of leaving it implied by a method name"),
                 ],
                 "ENFORCED. The rule \u2014 no result path may let a classification depend on a literal, and every verdict must be traceable to computed evidence \u2014 is now checked automatically: LiteralVerdictAudit re-reads the AT.Core source at test time and fails the suite on any un-triaged path, so a new literal-to-verdict path breaks the build until it is computed or explicitly adjudicated. This matters because G_025 and G_026 each fixed instances by hand and the class still recurred four times in the QG ladder alone: fixing instances does not prevent a class recurring. The inventory is 7 paths, ALL BOUNDARY \u2014 every one an authored judgement or input taxonomy with its reason cited in place, none a physics result. ZERO REFUTED remain, and the registry structurally forbids a refuted entry because a refuted path is fixed rather than tolerated. The single refuted path found \u2014 DaggerOriginAnalyzer, whose evidence flags, ratio and score were all literals at every construction site with Score feeding the ranking, and whose Coincidence row typed RatioToA0 = 1.0 against a NaN numerator \u2014 is now derived from the predicted g-dagger and reproduces the previously typed values exactly, so only the provenance changed. Output: SAFE / BOUNDARY / REFUTED. DERIVED / BOUNDARY / REFUTED (ResearchY-G_027)."),
