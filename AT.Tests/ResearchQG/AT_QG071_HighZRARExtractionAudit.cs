...
public class AT_QG071_HighZRARExtractionAudit : ResearchTestBase
{
    public AT_QG071_HighZRARExtractionAudit(ITestOutputHelper o) : base(o) { }
    ...
    public void QG071_HighZRARExtraction()
    {
        ...
        sb.AppendLine($"  Local g† (AT form)     = {sparc.GdaggerLocalAt_m_s2:E2} m/s²");
        ...
        S(sb, "Section F — AT vs MOND comparison"); sb.AppendLine(r.SF);
    ...
    private static string[] GeneratePlots(string plotDir, SPARCReport sparc, HighZRARExtractionReport r)
    {
...

        {
            ...
            var tq = gb.Select(g => g * Math.Sqrt(1 + sparc.GdaggerLocalAt_m_s2 / g)).ToArray();
            var series = new[]
            {
...

        {
            ...
            var atCurve = zGrid.Select(z => g0 * Math.Sqrt(0.315 * Math.Pow(1 + z, 3) + 0.685)).ToArray();
            var mondCurve = zGrid.Select(_ => g0).ToArray();
            var series = new[]
            {
                new RARPlotter.Series(zArr, gArr, orange, false, 3),
                new RARPlotter.Series(zGrid, atCurve, red, true, 1),
...

        {
            ...
            RARPlotter.PlotBars(Path.Combine(plotDir, "AT_vs_MOND.png"), labels, chi2, blue);
        }
        ...
        return new[]
        {
            ...
            Path.Combine(plotDir, "AT_vs_MOND.png"),
        };
    }
