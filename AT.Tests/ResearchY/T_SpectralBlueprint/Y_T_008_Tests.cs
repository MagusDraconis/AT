...
public class Y_T_008_Tests : ResearchTestBase
{
    ...
    /// <summary>The five T-program spectral landscapes (shared catalog).</summary>
    private static SpectralCase[] Cases() => SpectralCaseCatalog.All();

    /// <summary>S∞ of a catalog case via the replicator–mutator.</summary>
    private static int S(SpectralCase c, double mu, double beta) => SpectralCaseCatalog.SInfinity(c, mu, beta);

    /// <summary>S∞ of the canonical D96 case.</summary>
    private static int SD96(double mu, double beta) => SpectralCaseCatalog.SInfinity(SpectralCaseCatalog.D96(), mu, beta);
    ...
    public void Y_T_008_MutationScaling()
    {
        ...
        int[] s = muValues.Select(mu => SD96(mu, 1.0)).ToArray();
        ...
        int pureSelection = SD96(0.0, 1.0);
        ...
        Assert.True(s[^1] <= SpectralCaseCatalog.D96().A);        // bounded by landscape size
    }
    ...
    public void Y_T_008_CrowdingScaling()
    {
        ...
        int[] s = betaValues.Select(b => SD96(0.01, b)).ToArray();
        ...
        Assert.All(s, x => Assert.InRange(x, 1, SpectralCaseCatalog.D96().A));
    }
    ...
    public void Y_T_008_LandscapeSizeScaling()
    {
        ...
        foreach (int n in ns)
        {
            var (d, m) = AttractorDominanceAnalyzer.Eigenspaces(AttractorDominanceAnalyzer.D96Ring(n));
            int a = SpectralCaseCatalog.NonZeroCount(d);
        ...
        int a48 = SpectralCaseCatalog.NonZeroCount(AttractorDominanceAnalyzer.Eigenspaces(AttractorDominanceAnalyzer.D96Ring(48)).Distinct);
        int a384 = SpectralCaseCatalog.NonZeroCount(AttractorDominanceAnalyzer.Eigenspaces(AttractorDominanceAnalyzer.D96Ring(384)).Distinct);
    ...
    public void Y_T_008_ScalingLaw()
    {
        ...
        // ∂S∞/∂β ≥ 0.
        int b0 = SD96(0.01, 0.0);
        int b10 = SD96(0.01, 10.0);
    ...
    public void Y_T_008_Classification()
    {
        ...
        Assert.NotEqual(
            SD96(0.01, 1.0),
            S(SpectralCaseCatalog.Random(), 0.01, 1.0));
    }
    ...
    private void RunResearchReport()
    {
        ...
        foreach (var c in Cases().OrderByDescending(x => LogFitnessVariance(x.Fitness)))
        {
