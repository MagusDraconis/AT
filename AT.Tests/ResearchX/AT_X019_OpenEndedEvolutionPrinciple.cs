...
public class AT_X019_OpenEndedEvolutionPrinciple : ResearchTestBase
{
    public AT_X019_OpenEndedEvolutionPrinciple(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void AT_X019_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("AT-X019 Open-Ended Evolution Principle");

        Sec(sb, "0. Assumptions");
        sb.AppendLine("  1. AT-X018: 6-level staircase, L6 NOT observed.");
        ...
        foreach (var r in report.Requirements)
            sb.AppendLine($"  {r.Requirement,-37} │ {(r.SatisfiedInAT ? "YES" : "NO"),-10} │ {(r.IsBottleneck ? "YES" : "NO"),-11} │ {r.Why}");
        sb.AppendLine();
        sb.AppendLine($"  Satisfied: {report.Requirements.Count(r=>r.SatisfiedInAT)}/{report.Requirements.Count}");
        ...
        sb.AppendLine("    ✗ No mechanism exists in current AT");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("  NONE of these routes have been tested in AT.");
        sb.AppendLine();
        ...
        sb.AppendLine(new string('=', 100));
        sb.AppendLine($"  AT-X019 complete. Classification: {report.Classification}");
