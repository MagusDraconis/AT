...
/// <summary>
/// Determines whether AT already contains enough ingredients to compute the Einstein tensor
/// G_μν. Implements the STANDARD differential-geometry chain (metric → Christoffel → Riemann →
/// Ricci → Einstein) in 2D and verifies each step, then identifies where AT's own analyzers
...
public class EinsteinTensorTests : ResearchTestBase
{
    ...
    public void RicciProducesEinsteinTensor()
    {
        ...
        sb.AppendLine("NOTE: a NON-trivial G_μν requires dimension ≥ 3 — not computed here or in AT.");
        Output.WriteLine(sb.ToString());
    }
