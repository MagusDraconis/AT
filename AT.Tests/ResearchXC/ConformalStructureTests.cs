...
public class ConformalStructureTests : ResearchTestBase
{
    ...
    public void CausalOrder_DefinesLightConeStructure()
    {
        ...
        sb.AppendLine($"GrBridge \"Causal ordering\": native = {orderStep.IsAtNative}, \"{orderStep.DerivationStatus}\"");
        ...
        Assert.True(orderStep.IsAtNative);
    ...
    public void ConformalClass_ReconstructibleOrImported()
    {
        ...
        sb.AppendLine($"causal order (light-cone primitive):  PRESENT   (native = {orderStep.IsAtNative}, \"{orderStep.DerivationStatus}\")");
        sb.AppendLine($"order → conformal class (Malament):   {(metricStep.IsAtNative ? "NATIVE" : "IMPORTED")}   (\"{metricStep.DerivationStatus}\")");
        ...
        sb.AppendLine("         class (Malament 1977: causal order ⇒ light cones ⇒ conformal metric), but AT");
