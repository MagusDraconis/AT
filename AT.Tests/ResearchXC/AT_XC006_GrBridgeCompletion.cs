...
public class AT_XC006_GrBridgeCompletion : ResearchTestBase
{
    public AT_XC006_GrBridgeCompletion(ITestOutputHelper o) : base(o) { }
    ...
    private void RunTest()
    {
        ...
        foreach (var s in audit.Steps)
        {
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
                ...
                s.IsAtNative ? "YES" : "no", s.Priority));
        }
        sb.AppendLine();
        sb.AppendLine($"  AT-Native: {audit.NativeFraction:P0} ({audit.Steps.Count(s => s.IsAtNative)}/{audit.Steps.Count})");
        ...
        Sec(sb, "Section B — External Dependencies (What AT Imports)");
        sb.AppendLine("  The following theorems are ESTABLISHED in mathematical physics");
        sb.AppendLine("  but NOT re-derived within AT:");
        sb.AppendLine();
        ...
        // ═══ SECTION C: Native AT results ═══
        Sec(sb, "Section C — Native AT Gravity Results");
        sb.AppendLine("  What AT derives INTERNALLY (no external imports):");
        sb.AppendLine();
        foreach (var s in audit.Steps.Where(s => s.IsAtNative))
        {
        ...
        // ═══ SECTION H.5: AT-native derivation vision ═══
        Sec(sb, "Section H.5 — What a AT-Native Derivation Would Look Like");
        sb.AppendLine(GrBridgeAnalyzer.WhatWouldAAtNativeDerivationLookLike());
        ...
        sb.AppendLine("  WORST CASE: ~15% of AT killed. Everything else survives.");
