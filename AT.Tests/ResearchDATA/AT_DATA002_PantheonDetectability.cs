...
public class AT_DATA002_PantheonDetectability : ResearchTestBase
{
    public AT_DATA002_PantheonDetectability(ITestOutputHelper o) : base(o) { }
    ...
    private void RunTest()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  Recovery Statistics (η=0.015 injected, AT model fit):");
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
        ...
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "    Fraction AT pref:   {0:P1}", result.Recovery.FractionAtPreferred));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "    Mean significance:   {0:F2}σ", result.Recovery.MeanSignificance));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "    Ω_m bias (AT):      {0:F4}", result.Recovery.BiasOmegaM_AT));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "    Ω_m RMSE (AT):      {0:F4}", result.Recovery.RMSE_OmegaM_AT));
        ...
        sb.AppendLine();
        sb.AppendLine("  Q4-Q5: Would doubling/tripling AT signal make it detectable?");
        ...
        // ═══ Residual Analysis ═══
        Sec(sb, "Residual Analysis — LCDM vs AT Residuals");
        sb.AppendLine("  Residual distribution comparison (KS test):");
        sb.AppendLine("  ΛCDM data → ΛCDM fit residuals vs AT data → ΛCDM fit residuals.");
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "    LCDM → LCDM:    μ={0:F4}, σ={1:F4}",
            result.Residuals.MeanResidualLCDM_OnLCDMData,
            result.Residuals.StdResidualLCDM_OnLCDMData));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "    AT → LCDM:     μ={0:F4}, σ={1:F4}",
            result.Residuals.MeanResidualLCDM_OnATData,
            result.Residuals.StdResidualLCDM_OnATData));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "    AT → AT:      μ={0:F4}, σ={1:F4}",
            result.Residuals.MeanResidualAT_OnATData,
            result.Residuals.StdResidualAT_OnATData));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "    KS D = {0:F4},  p = {1:F4}",
        ...
        sb.AppendLine();
        sb.AppendLine("  Q8: Can Pantheon exclude stronger AT-like models?");
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "      YES. Models with η ≥ {0:F3} would be excluded at 3σ.",
            result.Thresholds[2].RequiredEta));
        sb.AppendLine("      But AT (η=0.015) is far below this threshold.");
        sb.AppendLine();
        sb.AppendLine("  Q9: What fraction of AT signal is currently hidden by noise?");
        ...
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "      Euclid SNR for AT signal: {0:F1}σ.", result.Euclid.EuclidSNR));
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "      Euclid will detect AT at >3σ WITHOUT additional improvements."));
        ...
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "     AT (η = {0:F3}) is ~{1:F1}x below this threshold.",
        ...
        sb.AppendLine();
        sb.AppendLine("  4. AT is NOT FALSIFIED. AT is NOT VALIDATED.");
        sb.AppendLine("     AT is currently BELOW Panetheon+SH0ES detection threshold.");
        sb.AppendLine();
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
            "  5. Euclid (2027) will improve sensitivity ~{0:F1}x and should",
            result.Euclid.SensitivityRatioW0));
        sb.AppendLine("     detect the AT signal at >3σ if AT is correct.");
        sb.AppendLine();
        sb.AppendLine("  CLASSIFICATION:");
        sb.AppendLine("    Pantheon+SH0ES: AT BELOW DETECTION THRESHOLD");
        sb.AppendLine("    AT status:     CONSISTENT with data. Awaiting stronger tests.");
