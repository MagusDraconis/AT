...
public class AT_XD004_EuclidDecisionTree : ResearchTestBase
{
    public AT_XD004_EuclidDecisionTree(ITestOutputHelper o) : base(o) { }
    ...
    private void RunTest()
    {
        ...
        Sec(sb, "Section A — Current AT Prediction");
        ...
        foreach (var s in tree.Scenarios)
        {
            ...
            sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
                ...
                s.Id, s.AtVerdict, s.ActionClass, s.KilledCount, tree.TotalSectors, pct));
        }
        ...
        var overall = tree.ConfidenceUpdates.First(c => c.Branch == "Overall AT framework");
        sb.AppendLine();
        sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
        ...
        sb.AppendLine("                    │");
        sb.AppendLine("  EUCLID ──────────┼── |w+1| ~ 0.015, w > −1  ──→  CONFIRM Λ(t), AT VALIDATED");
        ...
        sb.AppendLine();
        sb.AppendLine("  WORST CASE (w = −1):  ~70% survives. AT → 1-parameter + constant Λ.");
        sb.AppendLine("  BEST CASE (w ≠ −1, correct sign): 100% survives. AT validated.");
        sb.AppendLine("  ANY CASE: Response is pre-committed, documented, binding.");

        sb.AppendLine();
        sb.AppendLine(new string('=', 100));
        sb.AppendLine("  ResearchXD-004 COMPLETE.");
        sb.AppendLine("  AT is FULLY DECISION-READY for Euclid.");
        sb.AppendLine(new string('=', 100));
