...
public class AT_100_PhysicsCandidateValidation : ResearchTestBase
{
    private const int BaseSeed = 100_000_001;

    public AT_100_PhysicsCandidateValidation(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void AT_100_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("AT-100 Physics Candidate Validation — Hostile Review");

        sb.AppendLine("AT-100: Attempting to FALSIFY the {R, M} minimal theory.");
        ...
        sb.AppendLine("    • Identity dynamics (independent dimension, AT-047)");
        sb.AppendLine("    • Energy dynamics (independent dimension, AT-047)");
        sb.AppendLine("    • Condensate interactions (identity exclusion, AT-050)");
        sb.AppendLine("    • Spatial motion (coupling-driven, AT-062)");
        sb.AppendLine();
        ...
        if (report.FailuresDetected == 0)
        {
            ...
            sb.AppendLine("       the essential physics of the AT system.");
        }
        else
        {
        ...
        sb.AppendLine("  Experiment AT-100 completed successfully.");
        sb.AppendLine(new string('=', 100));
