...
public class AT_109_SolitonInteractionTheory : ResearchTestBase
{
    public AT_109_SolitonInteractionTheory(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void AT_109_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("AT-109 Soliton Interaction Theory");

        // ── Section 1 ───────────────────────────────────────────────
        Sec(sb, "1. Solitons as Proto-Particles");

        sb.AppendLine("  AT-108: Condensates = soliton solutions of the PDE.");
        ...
        sb.AppendLine($"    At d > 5w, interaction is negligible (AT-107 confirmed).");
        sb.AppendLine();
        ...
        sb.AppendLine("  Q5: Can AT-012 mergers be predicted?");
        sb.AppendLine("    AT-012: condensates MERGE when within coupling range.");
        sb.AppendLine($"    PDE prediction: fusion when d < {report.FusionThreshold:F3}.");
        sb.AppendLine("    AT-012 coupling range ≈ 3λ = 0.15 = 1.5w.");
        sb.AppendLine("    → PDE threshold CONSISTENT with AT-012 observations.");
        sb.AppendLine();
        ...
        sb.AppendLine("  │  • AT-012: merges at close range ✓                     │");
        sb.AppendLine("  │  • AT-050: identity exclusion (separate phases) ✓      │");
        sb.AppendLine("  │  • AT-107: survival at d=0.6 > 0.3 ✓                  │");
        sb.AppendLine("  └─────────────────────────────────────────────────────────┘");
        sb.AppendLine();

        sb.AppendLine(new string('=', 100));
        sb.AppendLine("  Experiment AT-109 completed successfully.");
        sb.AppendLine(new string('=', 100));
