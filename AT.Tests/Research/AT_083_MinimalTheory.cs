...
public class AT_083_MinimalTheory : ResearchTestBase
{
    private const int NumConfigs = 180;
    private const int BaseSeed = 830491627;

    public AT_083_MinimalTheory(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void AT_083_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("AT-083 Minimal AT Physics — Autonomous Theory Compression");

        sb.AppendLine("AT-083: What is the smallest predictive theory that explains");
        sb.AppendLine("         all major findings from AT-044 through AT-082?");
        sb.AppendLine();

        // ── Section 1: Objective ─────────────────────────────────────
        Sec(sb, "1. Objective");
        sb.AppendLine("  After 39 experiments (AT-044 through AT-082), the AT project");
        ...
        Sec(sb, "2. Known Causal Chains (AT-044 through AT-082)");
        ...
        foreach (var v in comparison.RetainedVariables)
        {
            ...
            {
                "R" => "essential — captures phase coherence, the fundamental conserved quantity (AT-052)",
                "M" => "essential — captures 97.7% of topology information (AT-081), effective field (AT-082)",
                "A" => "redundant — A≈R² (AT-075), zero additional information",
                "V" => "redundant — highly correlated with M (r>0.99, AT-081)",
                "S" => "redundant — highly correlated with M (r>0.99, AT-081)",
                "G" => "redundant — poorly correlated with dR/dt (AT-080)",
                "β" => "external parameter — does not vary (AT-061), sets curvature (AT-059)",
                _ => ""
            };
        ...
        sb.AppendLine("  Q1: Can all major AT findings be explained with 2-4 variables?");
        ...
        sb.AppendLine("    YES — M captures 97.7% of topology information (AT-081).");
        sb.AppendLine("    Full topology adds only +0.011 in R² over M alone.");
        sb.AppendLine();

        sb.AppendLine("  Q4: Is Alignment more fundamental than Force?");
        sb.AppendLine("    NO — Alignment is DERIVED: A ≈ R² (AT-075).");
        sb.AppendLine("    Force is DERIVED: F_net = A × ⟨f⟩ (AT-074).");
        ...
        sb.AppendLine("    β does not vary during evolution (AT-061).");
        sb.AppendLine("    β sets the curvature of the state space (AT-059)");
        sb.AppendLine("    but curvature does not drive motion (AT-068).");
        sb.AppendLine();
        ...
        // ── Section 10: Minimal AT Physics ──────────────────────────
        Sec(sb, "10. Minimal AT Physics");
        ...
        foreach (var v in best.StateVariables)
        {
            ...
            {
                "R" => "Order parameter — phase coherence, CONSERVED (AT-052)",
                "M" => "Mean coupling — effective field, DYNAMICAL (AT-082)",
                _ => ""
            };
        ...
        sb.AppendLine("  ├─────────────────────────────────────────────────┤");
        sb.AppendLine("  │  A = R²       : Alignment (AT-075)             │");
        sb.AppendLine("  │  F = A × ⟨f⟩  : Net force (AT-074)             │");
        sb.AppendLine("  │  κ ∝ β       : Curvature (AT-059)              │");
        sb.AppendLine("  ├─────────────────────────────────────────────────┤");
        sb.AppendLine("  │  FIXED PARAMETERS                               │");
        sb.AppendLine("  ├─────────────────────────────────────────────────┤");
        sb.AppendLine("  │  β : Memory strength (external, AT-061)        │");
        ...
        sb.AppendLine("  The AT system at scales N=100, K=2, λ=0.05 is described by");
        ...
        sb.AppendLine();
        sb.AppendLine("  R is a CONSERVED quantity (AT-052) that acts as an attractor.");
        ...
        sb.AppendLine();
        sb.AppendLine($"  C11. {comparison.Classification} — the AT system admits a");
        sb.AppendLine("       compressed description with two state variables ({R, M})");
        sb.AppendLine("       capturing all known causal chains from AT-044 to AT-082.");
        sb.AppendLine();

        sb.AppendLine(new string('=', 100));
        sb.AppendLine("  Experiment AT-083 completed successfully.");
        sb.AppendLine(new string('=', 100));
        ...
        TheoryCompressionAnalyzer.PhysicsCandidate best)
    {
        if (comparison.Classification.StartsWith("D"))
        {
            ...
            sb.AppendLine("    The AT system has an effective thermodynamic description");
