...
public class AT_055_ResonanceLandscapeMapping : ResearchTestBase
{
    ...
    public AT_055_ResonanceLandscapeMapping(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void AT_055_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("AT-055 Resonance Landscape Mapping");

        report.AppendLine("AT-055: Mapping the Global Topology of the AT State Space");
        report.AppendLine();

        // ── Section 1: Objective ─────────────────────────────────────
        AppendSection(report, "1. Objective");
        report.AppendLine("  AT-047-054 established: identity ≠ energy, coherence is");
        ...
        if (topo.BasinCount == 1)
        {
            report.AppendLine("  C3. The AT state space is a SINGLE CONNECTED ATTRACTOR BASIN.");
            ...
            report.AppendLine("  C4. Identity stability (AT-047-051) is explained by the");
        ...
        {
            report.AppendLine($"  C3. The AT state space contains {topo.BasinCount} attractor basins.");
        ...
        report.AppendLine("  Experiment AT-055 completed successfully.");
        report.AppendLine(new string('=', 100));
