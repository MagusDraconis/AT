...
public class AT_044_HistoricalPathDependence : ResearchTestBase
{
    ...
    public AT_044_HistoricalPathDependence(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void AT_044_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("AT-044 Historical Path Dependence");
        report.AppendLine("AT-044: Does the Order of Past Experiences Matter?");
        report.AppendLine();

        AppendSection(report, "1. Objective");
        report.AppendLine("  AT-043 showed memory fusion. This experiment tests whether the");
        ...
        report.AppendLine("  C2. Historical path dependence in AT memory represents the first");
        report.AppendLine("      demonstration that temporal ordering creates distinguishable identities.");

        report.AppendLine();
        report.AppendLine(new string('=', 100));
        report.AppendLine("  Experiment AT-044 completed successfully.");
        report.AppendLine(new string('=', 100));
