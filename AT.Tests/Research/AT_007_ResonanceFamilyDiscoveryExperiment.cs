...
/// <summary>
/// AT-007: Resonance Family Discovery Experiment
///
...
/// </summary>
public class AT_007_ResonanceFamilyDiscoveryExperiment : ResearchTestBase
{
    ...
    public AT_007_ResonanceFamilyDiscoveryExperiment(ITestOutputHelper output)
        : base(output)
    {
    }

    [Fact]
    public void AT_007_RunResonanceFamilyDiscovery()
    {
    ...
    private void ExecuteExperiment()
    {
        ...
        // ── Header ──────────────────────────────────────────────
        PrintHeader("AT-007 Resonance Family Discovery");
        report.AppendLine("AT-007: Discovery of Distinct Resonance Families");
        report.AppendLine();

        // ── 1. Objective ────────────────────────────────────────
        AppendSection(report, "1. Objective");
        report.AppendLine("  AT-006 identified a critical density ρc ≈ 0.09 above which stable");
        ...
        report.AppendLine("    • AT-008: Heterogeneous natural frequencies to seed multiple families.");
        report.AppendLine("    • AT-009: Cluster-cluster interactions between different families.");
        report.AppendLine("    • AT-010: Family stability under perturbations (robustness).");

        report.AppendLine();
        report.AppendLine(new string('=', 100));
        report.AppendLine("  Experiment AT-007 completed successfully.");
        report.AppendLine(new string('=', 100));
