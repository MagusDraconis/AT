...
public class AT_024_GeometricResonanceAnalysis : ResearchTestBase
{
    ...
    public AT_024_GeometricResonanceAnalysis(ITestOutputHelper output) : base(output) { }

    [Fact]
    public void AT_024_RunGeometricAnalysis()
    {
    ...
    private void ExecuteExperiment()
    {
        ...
        PrintHeader("AT-024 Geometric Resonance Analysis");
        report.AppendLine("AT-024: Does Geometric Arrangement Control Condensation?");
        report.AppendLine();

        AppendSection(report, "1. Objective");
        report.AppendLine("  AT-023 showed density is the best predictor but remains placement-dependent.");
        ...
        report.AppendLine();
        report.AppendLine("  C3. The AT control parameter hierarchy:");
        foreach (var r in ranked.Take(5))
            report.AppendLine($"      {ranked.IndexOf(r) + 1}. {r.Name} (avg CV={r.AvgCV:F3})");

        report.AppendLine();
        report.AppendLine(new string('=', 100));
        report.AppendLine("  Experiment AT-024 completed successfully.");
        report.AppendLine(new string('=', 100));
