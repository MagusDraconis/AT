...
/// <summary>
/// AT-006: Resonance Phase Transition
///
...
/// </summary>
public class AT_006_ResonancePhaseTransition : ResearchTestBase
{
    ...
    public AT_006_ResonancePhaseTransition(ITestOutputHelper output)
        : base(output)
    {
    }

    [Fact]
    public void AT_006_RunResonancePhaseTransition()
    {
    ...
    private void ExecuteExperiment()
    {
        ...
        // ── Header ──────────────────────────────────────────────
        PrintHeader("AT-006 Resonance Phase Transition");
        report.AppendLine("AT-006: Critical Resonance Density in Kuramoto Networks");
        report.AppendLine();
        ...
        if (hasPhaseTransition)
        {
            ...
            report.AppendLine("      a necessary condition for matter-like self-organization in the AT framework.");
        }
        else
        {
            ...
            report.AppendLine("      where matter-like structures CAN form, guiding future AT experiments");
        ...
        report.AppendLine("    • AT-007: Structured coupling topologies at critical density.");
        report.AppendLine("    • AT-008: Cluster-cluster interactions (proto-particle collisions).");
        report.AppendLine("    • AT-009: Field-mediated cluster formation (resonance + synchronization).");

        report.AppendLine();
        report.AppendLine(new string('=', 100));
        report.AppendLine("  Experiment AT-006 completed successfully.");
        report.AppendLine(new string('=', 100));
