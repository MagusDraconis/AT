...
public class AT_036_ResonanceMemory : ResearchTestBase
{
    ...
    public AT_036_ResonanceMemory(ITestOutputHelper output) : base(output) { }

    [Fact]
    public void AT_036_RunMemoryTest()
    {
    ...
    private void Run()
    {
        ...
        PrintHeader("AT-036 Resonance Memory");
        report.AppendLine("AT-036: Can Historical Memory Break the Universal Attractor?");
        report.AppendLine();

        AppendSection(report, "1. Objective");
        report.AppendLine("  AT-031 revealed a single universal attractor — all states converge.");
        ...
        report.AppendLine("  Experiment AT-036 completed successfully.");
        report.AppendLine(new string('=', 100));
