...
public class AT_101_TheoryRepairProgram : ResearchTestBase
{
    private const int BaseSeed = 101_000_001;

    public AT_101_TheoryRepairProgram(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void AT_101_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("AT-101 Theory Repair Program");

        sb.AppendLine("AT-101: Repairing the rejected {R, M} theory.");
        sb.AppendLine("         Preserve state variables. Repair equations.");
        sb.AppendLine();

        // ── Section 1: The Problem ───────────────────────────────────
        Sec(sb, "1. The Rejected Theory (AT-083 / AT-100)");
        sb.AppendLine("  State = {R, M}");
        sb.AppendLine("  Original equation: dR/dt = α₀ + α₁·R + α₂·M");
        sb.AppendLine("  AT-100 result: REJECTED — 5/8 attacks broke the theory.");
        sb.AppendLine();
        ...
        Sec(sb, "3. Full Validation (AT-100 Attack Vectors)");
        ...
        sb.AppendLine("  Q4: Can all AT-100 failures be eliminated?");
        ...
        sb.AppendLine($"  Original: dR/dt = α₀ + α₁·R + α₂·M  (rejected, AT-100)");
        sb.AppendLine($"  Repaired: {best.Equation}  (AT-101)");
        sb.AppendLine();
        ...
        if (report.Classification.StartsWith("D"))
        {
            ...
            sb.AppendLine("       The repaired equation survives all AT-100 attacks.");
        ...
        else if (report.Classification.StartsWith("C"))
        {
            ...
            sb.AppendLine($"       Most AT-100 attacks now pass. {report.PerCandidateResults[best.Name].Count(r => !r.Passed)} remain.");
            sb.AppendLine($"       {best.Equation}");
        ...
        sb.AppendLine("  Experiment AT-101 completed successfully.");
        sb.AppendLine(new string('=', 100));
