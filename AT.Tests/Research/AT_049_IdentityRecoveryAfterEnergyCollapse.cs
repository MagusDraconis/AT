...
public class AT_049_IdentityRecoveryAfterEnergyCollapse : ResearchTestBase
{
    // ── Experimental parameters ──────────────────────────────────────

    private static readonly string[] Histories = { "A", "B", "AB", "BA", "ABC", "CBA" };
    // Destructive transfers only (beyond ±25% threshold from AT-048).
    ...
    public AT_049_IdentityRecoveryAfterEnergyCollapse(ITestOutputHelper o) : base(o) { }

    [Fact]
    public void AT_049_Run()
    {
    ...
    private void RunTest()
    {
        ...
        PrintHeader("AT-049 Identity Recovery After Energy Collapse");

        report.AppendLine("AT-049: Is Destroyed Identity Truly Erased or Merely Suppressed?");
        report.AppendLine();

        // ── Section 1: Objective ─────────────────────────────────────
        AppendSection(report, "1. Objective");
        report.AppendLine("  AT-048 showed identity is destroyed beyond \u00b125% energy transfer.");
        ...
        report.AppendLine($"    (all beyond AT-048 \u00b125% destruction threshold)");
        ...
        report.AppendLine("  Experiment AT-049 completed successfully.");
        report.AppendLine(new string('=', 100));
