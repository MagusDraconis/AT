...
/// <summary>
/// AT-013: Internal Resonance States
///
...
/// </summary>
public class AT_013_InternalResonanceStates : ResearchTestBase
{
    ...
    public AT_013_InternalResonanceStates(ITestOutputHelper output)
        : base(output)
    {
    }

    [Fact]
    public void AT_013_RunInternalStateExperiment()
    {
    ...
    private void ExecuteExperiment()
    {
        ...
        PrintHeader("AT-013 Internal Resonance States");
        report.AppendLine("AT-013: Persistence of Internal Phase Structure in Condensates");
        report.AppendLine();

        AppendSection(report, "1. Objective");
        report.AppendLine("  AT-012 showed condensates merge via rapid phase-locking.");
        ...
        else
        {
            ...
            report.AppendLine();
            report.AppendLine("  C3. At present, all AT condensates belong to a single resonance state.");
            report.AppendLine("      Internal state diversity requires extensions beyond the Kuramoto model.");
        }

        report.AppendLine();
        report.AppendLine("  Next steps:");
        report.AppendLine("    • AT-014: Non-symmetric coupling matrices for internal state stability.");
        report.AppendLine("    • AT-015: External driving fields to sustain phase patterns.");

        report.AppendLine();
        report.AppendLine(new string('=', 100));
        report.AppendLine("  Experiment AT-013 completed successfully.");
        report.AppendLine(new string('=', 100));
