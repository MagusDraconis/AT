...
///
/// Tests: ATQG230 (anisotropic scalar → still conformally flat), ATQG231 (multi-field needs a new primitive),
///        ATQG232 (classification).
/// </summary>
public class ATQG_Phase23_OriginOfPsiTests : ResearchTestBase
{
    public ATQG_Phase23_OriginOfPsiTests(ITestOutputHelper o) : base(o) { }

    // ── ATQG230: anisotropic branching → still a scalar, still conformally flat ─────

    [Fact]
    public void ATQG230_AnisotropicStillScalar()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG230: anisotropic branching → still 1 scalar, still conformally flat");
    ...
    // ── ATQG231: multi-field actualization → a tensor, but a NEW primitive ──────────

    [Fact]
    public void ATQG231_MultiFieldNeedsNewPrimitive()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG231: a rank-2 tensor requires MULTIPLE scalars — a new primitive");
        ...
        sb.AppendLine($"AT has {oneScalar} scalar (the counting measure ρ)");
        ...
        sb.AppendLine("measures whose gradients combine into a tensor). But AT has exactly ONE counting measure ρ —");
    ...
    [Fact]
    public void ATQG232_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG232: is ψ DERIVED, EMERGENT, or a NEW PRIMITIVE?");
        ...
        sb.AppendLine("    statistics, support-rank fluctuations) leaves Weyl = 0 (ATQG230).");
        ...
        sb.AppendLine("    which is a new primitive (ATQG231).");
        ...
        sb.AppendLine("  • This is the definitive answer to the GW arc: AT's conformal gravity is closed under its two");
