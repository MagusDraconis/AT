...
/// <summary>
/// AT-QG Phase 46 — why spin-2? Tests which spin is uniquely selected for the minimal extension.
/// Classify: DERIVED / PREFERRED / POSTULATED.
///
/// Tests: ATQG460 (spin viability), ATQG461 (three constraints), ATQG462 (classification).
/// </summary>
public class ATQG_Phase46_WhySpin2Tests : ResearchTestBase
{
    public ATQG_Phase46_WhySpin2Tests(ITestOutputHelper o) : base(o) { }

    // ── ATQG460: only spin-2 survives all three constraints ─────────────────────────

    [Fact]
    public void ATQG460_OnlySpin2Survives()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG460: which spin passes polarization + attraction + light bending?");
    ...
    // ── ATQG461: the three independent constraints ───────────────────────────────────

    [Fact]
    public void ATQG461_ThreeConstraints()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG461: three independent arguments force spin-2");
    ...
    // ── ATQG462: classification ───────────────────────────────────────────────────────

    [Fact]
    public void ATQG462_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG462: DERIVED / PREFERRED / POSTULATED?");

        sb.AppendLine($"CLASSIFICATION: {WhySpin2.Classify()}");
        sb.AppendLine();
        sb.AppendLine("  • NOT DERIVED: ψ is a new primitive (QG23/24); its spin is not derived from AT's scalar sector.");
