...
/// <summary>
/// AT-QG Phase 41 — derive the TRM acceleration law. Tests whether the √(g_N·a0) term emerges from Q-event
/// saturation. Classify: DERIVED / PARTIAL MATCH / IMPORTED.
///
/// Tests: ATQG410 (saturation has a core, no √ regime), ATQG411 (opposite sign), ATQG412 (classification).
/// </summary>
public class ATQG_Phase41_TRMAccelerationOriginTests : ResearchTestBase
{
    public ATQG_Phase41_TRMAccelerationOriginTests(ITestOutputHelper o) : base(o) { }

    // ── ATQG410: saturation has a regular core, no √ regime ─────────────────────────

    [Fact]
    public void ATQG410_SaturationHasCoreNoSqrt()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG410: saturation acceleration g_sat = g_N(1-e^(-r^3/rc^3))");
    ...
    // ── ATQG411: opposite sign / regime ──────────────────────────────────────────────

    [Fact]
    public void ATQG411_OppositeSign()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG411: saturation suppresses; MOND enhances");
    ...
    // ── ATQG412: classification ───────────────────────────────────────────────────────

    [Fact]
    public void ATQG412_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG412: DERIVED / PARTIAL MATCH / IMPORTED?");
        ...
        sb.AppendLine("  • AT DOES derive flat rotation curves, but via the log-deficit (α=0 scale-free) profile (G4-ME Phases 3–4),");
