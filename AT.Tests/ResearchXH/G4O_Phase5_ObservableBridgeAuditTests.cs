...
public class G4O_Phase5_ObservableBridgeAuditTests : ResearchTestBase
{
    ...
    public void G4_O50_FullGeodesicMotion()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            double a = PhysicalObservables.AtAcceleration(u => PhysicalObservables.Gaussian(u), x, D);
            v += a * dt;
            x += v * dt;
            traj.Add(x);
        }
    ...
    public void G4_O51_WeakFieldAndCurvatureAcrossProfiles()
    {
        ...
        sb.AppendLine($"{"Gaussian peak",-16} {PhysicalObservables.AtAcceleration(u => PhysicalObservables.Gaussian(u), x, D),20:F4}");
        sb.AppendLine($"{"NFW-like",-16} {PhysicalObservables.AtAcceleration(u => PhysicalObservables.Nfw(u), x, D),20:F4}");
        sb.AppendLine($"{"exponential",-16} {PhysicalObservables.AtAcceleration(u => PhysicalObservables.Exponential(u), x, D),20:F4}");
        sb.AppendLine($"{"uniform sphere",-16} {PhysicalObservables.AtAcceleration(u => PhysicalObservables.UniformSphere(u), x, D),20:F4}");
    ...
    public void G4_O52_RhoVsMatterAndClassification()
    {
        ...
        double x = 0.4;
        // AT (conformal map): a = −(1/d)∇lnρ (repulsive).
        double aAt = PhysicalObservables.AtAcceleration(u => PhysicalObservables.Gaussian(u), x, D);
        // Newtonian (matter map): a = −∫ρ (attractive).
        double aNewt = PhysicalObservables.GrAcceleration(u => PhysicalObservables.Gaussian(u), x);

        sb.AppendLine($"AT map (ρ as conformal factor): a = {aAt:F4} (repulsive)");
        sb.AppendLine($"Newton map (ρ as matter density):  a = {aNewt:F4} (attractive)");
        sb.AppendLine($"opposite sign: {Math.Sign(aAt) != Math.Sign(aNewt)}");
        ...
        sb.AppendLine();
        sb.AppendLine("CLASSIFICATION: A) real AT prediction, AND C) ρ ≠ observable matter density — the repulsive");
        ...
        Assert.True(aAt > 0, "AT acceleration should be repulsive");
        Assert.True(aNewt < 0, "Newtonian acceleration should be attractive");
        Assert.True(Math.Sign(aAt) != Math.Sign(aNewt), "conformal and matter maps should disagree");
    }
}
