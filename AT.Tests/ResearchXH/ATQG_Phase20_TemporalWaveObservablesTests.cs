...
/// <summary>
/// AT-QG Phase 20 — temporal-wave interpretation of GW observations. Tests whether propagating time-rate
/// oscillations can generate the same detector observables as GR tensor waves. Classify: MATCH / PARTIAL MATCH /
/// NO MATCH.
///
/// Tests: ATQG200 (round-trip time conformal invariance), ATQG201 (differential strain: breathing vs tensor),
///        ATQG202 (classification).
/// </summary>
public class ATQG_Phase20_TemporalWaveObservablesTests : ResearchTestBase
{
    public ATQG_Phase20_TemporalWaveObservablesTests(ITestOutputHelper o) : base(o) { }

    // ── ATQG200: round-trip light travel time is conformally invariant ──────────────

    [Fact]
    public void ATQG200_RoundTripConformalInvariance()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG200: a temporal wave (δρ) does NOT change light travel time (conformal invariance)");
    ...
    // ── ATQG201: breathing (common-mode) vs tensor (differential) arm strain ────────

    [Fact]
    public void ATQG201_DifferentialStrain()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG201: LIGO measures differential strain — blind to breathing, sensitive to tensor");
    ...
    [Fact]
    public void ATQG202_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG202: can temporal waves generate the LIGO/Virgo observables?");
        ...
        sb.AppendLine("    in light round-trip times (ATQG200) — an interferometer sees nothing.");
        sb.AppendLine("  • Even ignoring that, the breathing (scalar) mode is COMMON-MODE: it stretches both arms equally, so");
        sb.AppendLine("    the differential phase (the LIGO observable) is ZERO (ATQG201).");
