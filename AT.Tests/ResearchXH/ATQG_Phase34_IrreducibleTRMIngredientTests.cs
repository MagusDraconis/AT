...
/// <summary>
/// AT-QG Phase 34 — identify the irreducible TRM ingredient. Removes each of four candidate ingredients in turn
...
///
/// Tests: ATQG340 (identity of the three aliases), ATQG341 (removal analysis), ATQG342 (classification).
/// </summary>
public class ATQG_Phase34_IrreducibleTRMIngredientTests : ResearchTestBase
{
    public ATQG_Phase34_IrreducibleTRMIngredientTests(ITestOutputHelper o) : base(o) { }

    // ── ATQG340: M_eff / kernel / temporal-rate are ONE object ───────────────────────

    [Fact]
    public void ATQG340_ThreeAliasesOneObject()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG340: M_eff, the kernel, and the temporal rate are the same object");
    ...
    // ── ATQG341: removal analysis ─────────────────────────────────────────────────────

    [Fact]
    public void ATQG341_RemovalAnalysis()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG341: remove each ingredient in turn — which successes die?");
        ...
        sb.AppendLine($"redshift requires ψ?            : {redshiftNeedsPsi}  (AT g_00 = −ρ^(2/d) already gives it)");
    ...
    // ── ATQG342: classification ───────────────────────────────────────────────────────

    [Fact]
    public void ATQG342_Classification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG342: ESSENTIAL / SECONDARY / REDUNDANT");
