...
/// <summary>
/// AT-QG Phase 26 — non-tensor explanation of lensing. Tests whether apparent lensing (image shift, magnification,
/// time delay) can emerge from the scalar machinery, using only observable quantities. Classify: MATCH / PARTIAL
/// MATCH / NO MATCH.
///
/// Tests: ATQG260 (deflection & magnification), ATQG261 (time delay & the redshift survivor), ATQG262
///        (mechanism census → classification).
/// </summary>
public class ATQG_Phase26_NonTensorLensingTests : ResearchTestBase
{
    public ATQG_Phase26_NonTensorLensingTests(ITestOutputHelper o) : base(o) { }

    // ── ATQG260: image shift & magnification ─────────────────────────────────────────

    [Fact]
    public void ATQG260_ImageShiftAndMagnification()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG260: image shift & magnification (γ = −1 vs GR γ = +1)");
        ...
        sb.AppendLine();
        sb.AppendLine($"image shift vanishes in conformal AT: {noShift}");
    ...
    // ── ATQG261: time delay & the redshift survivor ─────────────────────────────────

    [Fact]
    public void ATQG261_TimeDelayAndRedshiftSurvivor()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG261: Shapiro time delay (vanishes) vs gravitational redshift (survives)");
        ...
        sb.AppendLine();
        sb.AppendLine($"arrival-time delay vanishes in conformal AT: {noDelay}");
    ...
    // ── ATQG262: mechanism census → classification ───────────────────────────────────

    [Fact]
    public void ATQG262_MechanismCensus()
    {
        Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
        var sb = new StringBuilder();
        PrintHeader("ATQG262: can any scalar mechanism produce apparent lensing?");
        ...
        sb.AppendLine("only the gravitational redshift (a frequency effect) survives as a genuine AT prediction.");
        Output.WriteLine(sb.ToString());
